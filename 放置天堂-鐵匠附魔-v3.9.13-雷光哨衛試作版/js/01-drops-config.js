...
// 卓越以上的流派詞綴：四屬性穿透各 30%、異常增傷 40%、頭目減傷 25% 封頂。
function d2rCombatAffixTotals(owner) {
    let t=d2rEquipTotals(owner), cap={fpen:30,wpen:30,epen:30,apen:30,sdm:40,bdr:25}, out={};
    Object.keys(cap).forEach(k=>out[k]=Math.max(0,Math.min(cap[k],Number(t[k])||0)));
    return out;
}
...
function d2rAffixText(row) {
    ...
    if (['mc','rc','gc','mcd','rcd','gcd','hpp','mpp','pot','abr','gf','xf','nd','bd','fpen','wpen','epen','apen','sdm','bdr'].includes(r[0])) return `${D2R_AFFIX_LABEL[r[0]]} +${r[1]}%`;
...
const D2R_PREFIX_PREFERRED = new Set(['as','ff','fw','fa','fe','ph','pm','md','rd','mg','mh','rh','gh','rf','rw','re','ra','rn','mf','fpen','wpen','epen','apen','sdm']);
