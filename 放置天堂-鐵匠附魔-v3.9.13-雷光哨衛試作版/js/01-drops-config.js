...
// ===== D2R 裝備詞綴（第一階段）=====
// 實體只儲存精簡三元組 [代碼, 數值, 階級]；舊存檔沒有 d2 時即視為普通裝備。
const D2R_AFFIX_LABEL = {
    md:'近距離傷害', rd:'遠距離傷害', mg:'魔法傷害', mh:'近距離命中', rh:'遠距離命中', gh:'魔法命中',
    ...
    kx:'擊殺爆炸', ts:'受擊護盾', pi:'完美一擊', ks:'擊殺加速', sp:'近戰濺射', fh:'滿血獵手',
    fpen:'火焰穿透', wpen:'寒冰穿透', epen:'大地穿透', apen:'風雷穿透', sdm:'異常增傷', bdr:'頭目減傷',
    und:'不死獵殺', dem:'惡魔獵殺', dra:'屠龍', hsk:'破硬皮', opn:'先制增傷', exe:'處決增傷',
    ...
    fbr:'烈焰催化', wfr:'霜潮共鳴', eps:'腐土共生', wbl:'風刃放血',
    fsi:'熾焰刻印', wsi:'寒潮刻印', esi:'腐毒刻印', asi:'裂風刻印',
    ...
    ld:'躍擊傷害', ls:'躍擊暈眩時間', lc:'躍擊冷卻縮短', lr:'躍擊怒氣消耗降低',
    bh:'祝福之鎚傷害', bn:'祝福之鎚數量', bt:'祝福之鎚持續時間', bc:'祝福之鎚冷卻縮短',
    shd:'暗影分身傷害', shn:'暗影分身數量', sht:'暗影分身持續時間', shc:'暗影分身冷卻縮短',
    vwd:'范德震地傷害', vwn:'范德震地目標數量', vws:'范德震地暈眩時間', vwc:'范德震地冷卻縮短',
    fdd:'冰龍追擊傷害', fdn:'冰龍追擊目標數量', fdf:'冰龍追擊冰凍機率', fdc:'冰龍追擊冷卻縮短',
    tjd:'雷霆標槍連鎖傷害', tjn:'雷霆標槍彈跳次數', tjs:'雷霆標槍衰減降低', tjc:'雷霆標槍冷卻縮短',
    med:'心靈共振傷害', men:'心靈共振目標數量', mes:'心靈共振衰減降低', mec:'心靈共振冷卻縮短',
    rbd:'裂界衝擊傷害', rbn:'裂界衝擊目標數量', rbf:'裂界衝擊暈眩機率', rbc:'裂界衝擊冷卻縮短',
    ard:'多重箭雨傷害', arn:'多重箭雨箭矢數量', arc:'多重箭雨暴擊機率', acd:'多重箭雨冷卻縮短',
    csd:'充能一擊傷害', csn:'充能一擊電束數量', csp:'充能一擊風抗穿透', csc:'充能一擊冷卻縮短',
    mzd:'馬賽克終結傷害', mzn:'馬賽克終結目標數量', mzt:'馬賽克蓄能保存時間', mzc:'馬賽克終結冷卻縮短',
    trd:'雷光哨衛傷害', trn:'雷光哨衛電束數量', trt:'雷光哨衛持續時間', trc:'雷光哨衛冷卻縮短',
    qmd:'王旗號令傷害', qmn:'王旗額外號令數量', qmt:'王旗號令持續時間', qmc:'王旗號令間隔縮短',
    srd:'盾反傷害', srb:'盾反格擋機率', srg:'盾反格擋幅度', src:'盾反間隔縮短',
    bfd:'火龍化身傷害', bfn:'火龍灼燒時間', bft:'火龍化身持續時間', bfc:'火龍攻擊間隔縮短',
    kvd:'王者劍氣傷害', kvn:'王者劍氣數量', kvp:'王者劍氣機率', kvc:'王者劍氣間隔縮短',
    ufd:'不屈震波傷害', ufs:'不屈堡壘蓄能加速', ufg:'不屈堡壘護盾', ufc:'不屈堡壘冷卻縮短',
    dld:'雷龍風暴傷害', dln:'雷龍風暴電束數量', dlt:'雷龍風暴持續時間', dlc:'雷龍風暴間隔縮短'
};
const D2R_QUALITY = {
    magic:    { n:'魔法', c:'text-blue-300',       min:1, max:2,  t1max:1 },
    rare:     { n:'稀有', c:'text-yellow-300',     min:2, max:3,  t1max:1 },
    excellent:{ n:'卓越', c:'text-orange-300',     min:3, max:4,  t1max:1 },
    epic:     { n:'史詩', c:'text-purple-300',     min:4, max:5,  t1max:1 },
    legend:   { n:'傳說', c:'text-red-400',        min:5, max:6,  t1max:2 },
    mythic:   { n:'神話', c:'text-cyan-300',       min:6, max:7,  t1max:2 },
    immortal: { n:'不朽', c:'text-green-300',      min:7, max:8,  t1max:3 },
    primal:   { n:'太古', c:'d2q-primal',          min:9, max:10, t1max:4 }
};
...
// 💎 六色五階寶石：只補數值；技能與玩法改變保留給後續符文系統。
const GEM_COLORS = {
    red:{n:'紅寶石',icon:'🔴'}, green:{n:'綠寶石',icon:'🟢'}, blue:{n:'藍寶石',icon:'🔵'},
    white:{n:'白寶石',icon:'⚪'}, yellow:{n:'黃寶石',icon:'🟡'}, purple:{n:'紫寶石',icon:'🟣'}
};
const GEM_RANKS = [
    {n:'碎裂',general:1,power:1,special:.5,value:10000},
    {n:'普通',general:2,power:2,special:1,value:30000},
    {n:'無瑕',general:3,power:4,special:1.5,value:90000},
    {n:'完美',general:5,power:7,special:2,value:270000},
    {n:'皇家',general:8,power:11,special:3,value:810000}
];
const GEM_IDS = [];
...
function gemDef(id) {
    let d=DB.items[id];
    return d&&d.gem ? {id,color:d.gemColor,rank:d.gemRank,colorDef:GEM_COLORS[d.gemColor],rankDef:GEM_RANKS[d.gemRank-1]} : null;
}
function gemEffectForItem(gemId,item) {
    let g=gemDef(gemId), d=item&&DB.items[item.id]; if(!g||!d)return null;
    let v=g.rankDef.general,p=g.rankDef.power,s=g.rankDef.special,code='',label='';
    if(d.type==='wpn'){
        if(g.color==='red'){code='md';v=p;label=`近距離傷害 +${v}`;}
        else if(g.color==='green'){code='rd';v=p;label=`遠距離傷害 +${v}`;}
        else if(g.color==='blue'){code='mg';v=p;label=`魔法傷害 +${v}`;}
        else if(g.color==='white'){code=d.ranged?'rh':((d.mdmg||d.magic||d.qigu)?'gh':'mh');v=p;label=`${D2R_AFFIX_LABEL[code]} +${v}`;}
        else if(g.color==='yellow'){code='as';v=s;label=`攻擊速度 +${v}%`;}
        else {code='pm';v=p;label=`命中回復 MP +${v}`;}
    } else if(d.type==='arm'){
        let map={red:['str','力量'],green:['dex','敏捷'],blue:['int','智力'],white:['con','體質'],yellow:['mf','物品發現率'],purple:['wis','精神']};
        [code,label]=map[g.color]; v=g.color==='yellow'?s:v; label+=` +${v}${g.color==='yellow'?'%':''}`;
    } else return null;
...
const RUNE_DEFS = {
    ...
    rune_ral: {n:'Ral 符文',tier:2, slot:'arm', code:'burnRed',value:20,cap:60,p:120000,d:'燃燒持續時間縮短 20%。'},
    rune_ort: {n:'Ort 符文',tier:3, slot:'arm', code:'paralyzeRed',value:20,cap:60,p:250000,d:'麻痺持續時間縮短 20%。'},
    rune_thul:{n:'Thul 符文',tier:3, slot:'arm', code:'freezeRed',value:20,cap:60,p:250000,d:'冰凍持續時間縮短 20%。'},
    rune_amn: {n:'Amn 符文',tier:3, slot:'wpn', code:'lifeSteal',value:2,cap:10,p:300000,d:'一般攻擊吸取造成傷害的 2% 為 HP。'},
    rune_sol: {n:'Sol 符文',tier:4, slot:'arm', code:'flatDr',value:2,cap:20,p:600000,d:'每次受到傷害固定減少 2。'},
    rune_dol: {n:'Dol 符文',tier:4, slot:'arm', code:'hpR',value:5,cap:20,p:800000,d:'HP 自然恢復量 +5。'},
    rune_ko:  {n:'Ko 符文', tier:4, slot:'all', code:'dex',value:2,cap:10,p:900000,d:'敏捷 +2。'},
    rune_lem: {n:'Lem 符文',tier:4, slot:'all', code:'goldFind',value:10,cap:30,p:1000000,d:'金幣發現量 +10%。'},
    rune_lo:  {n:'Lo 符文', tier:5, slot:'arm', code:'resWind',value:10,cap:30,p:2500000,d:'風屬性抗性 +10。'},
    rune_pul: {n:'Pul 符文',tier:5, slot:'wpn', code:'bossDmg',value:5,cap:20,p:2800000,d:'對頭目的一般攻擊傷害提高 5%。'},
    rune_um:  {n:'Um 符文', tier:5, slot:'arm', code:'allRes',value:8,cap:24,p:3200000,d:'四屬性抗性 +8。'},
    rune_mal: {n:'Mal 符文',tier:5, slot:'wpn', code:'ignoreDef',value:5,cap:20,p:3800000,d:'物理攻擊忽略目標 5% 傷害減免與硬皮。'},
    rune_ist: {n:'Ist 符文',tier:5, slot:'all', code:'mf',value:5,cap:20,p:4500000,d:'物品發現率 +5%。'},
    rune_gul: {n:'Gul 符文',tier:6, slot:'wpn', code:'bossDmg',value:7,cap:20,p:5500000,d:'對頭目的一般攻擊傷害提高 7%。'},
    rune_vex: {n:'Vex 符文',tier:6, slot:'wpn', code:'killMp',value:5,cap:20,p:6000000,d:'擊殺時恢復 5% 最大 MP。'},
    rune_ohm: {n:'Ohm 符文',tier:6, slot:'wpn', code:'basicDmg',value:10,cap:30,p:7000000,d:'一般攻擊傷害 +10%。'},
    rune_ber: {n:'Ber 符文',tier:6, slot:'arm', code:'flatDr',value:5,cap:20,p:8000000,d:'每次受到傷害固定減少 5。'},
...
function runeFitsItem(id,item){
    ...
    let r=runeDef(id),d=item&&DB.items[item.id];if(!r||!d)return false;
    return (d.type==='wpn'&&!d.isArrow)||d.type==='arm';
}
function runeSingleEffectFitsItem(id,item){
    let r=runeDef(id),d=item&&DB.items[item.id];if(!r||!d)return false;
    if(r.slot==='wpn')return d.type==='wpn'&&!d.isArrow;
...
const RUNEWORDS = [
    ...
    {id:'stealth',n:'隱匿',seq:['rune_tal','rune_eth'],kind:'armor',effects:{as:5,allStatusRed:10},d:'攻擊速度 +5%、四種異常時間縮短 10%。'},
    {id:'ancient_pledge',n:'先祖誓言',seq:['rune_ral','rune_ort','rune_tal'],kind:'shield',effects:{allRes:15,flatDr:3},d:'四屬性抗性 +15、直接傷害減少 3。'},
    {id:'leaf',n:'葉子',seq:['rune_tir','rune_ral'],kind:'magicwpn',effects:{magicDmg:10,mpCost:5},d:'魔法傷害 +10、技能 MP 消耗降低 5%。'},
    {id:'lore',n:'知識',seq:['rune_ort','rune_sol'],kind:'helm',effects:{int:3,wis:3,mpPct:10},d:'智力 +3、精神 +3、MP 上限 +10%。'},
    {id:'strength',n:'力量',seq:['rune_amn','rune_tir'],kind:'meleewpn',effects:{lifeSteal:5,bossDmg:5},d:'一般攻擊吸血 +5%、對頭目一般攻擊傷害 +5%。'},
    {id:'rhyme',n:'韻律',seq:['rune_thul','rune_ort'],kind:'shield',effects:{block:5,freezeRed:20,paralyzeRed:20},d:'格擋率 +5%、冰凍與麻痺時間縮短 20%。'},
    {id:'smoke',n:'煙霧',seq:['rune_nef','rune_sol'],kind:'armor',effects:{flatDr:5,allRes:10,allResNone:10},d:'直接傷害減少 5、五屬性抗性 +10。'},
    // ===== v3.8.92 新增符文之語 =====
    {id:'malice',n:'惡意',seq:['rune_ith','rune_el','rune_eth'],kind:'meleewpn',effects:{basicDmg:15,ignoreDef:8,killHp:3},d:'一般攻擊傷害 +15%、忽略防禦 8%、擊殺恢復 HP +3%。'},
    {id:'zephyr',n:'和風',seq:['rune_ort','rune_eth'],kind:'rangedwpn',effects:{as:10,basicDmg:10,ignoreDef:5},d:'攻擊速度 +10%、一般攻擊傷害 +10%、忽略防禦 5%。'},
    {id:'radiance',n:'光輝',seq:['rune_nef','rune_sol','rune_ith'],kind:'helm',effects:{flatDr:5,bossDmg:5,allRes:10},d:'直接傷害減少 5、對頭目一般攻擊傷害 +5%、四屬性抗性 +10。'},
    {id:'insight',n:'洞察',seq:['rune_ral','rune_tir','rune_tal','rune_sol'],kind:'magicwpn',effects:{magicDmg:15,mpCost:10,killMp:5},d:'魔法傷害 +15、技能 MP 消耗降低 10%、擊殺恢復 MP +5%。'},
    {id:'spirit',n:'精神',seq:['rune_tal','rune_thul','rune_ort','rune_amn'],kind:'magicorshield',effects:{magicDmg:12,mpPct:15,allRes:10},d:'魔法傷害 +12、MP 上限 +15%、四屬性抗性 +10。'},
    {id:'edge',n:'邊緣',seq:['rune_tir','rune_tal','rune_amn'],kind:'rangedwpn',effects:{basicDmg:12,bossDmg:8,killMp:3},d:'一般攻擊傷害 +12%、對頭目一般攻擊傷害 +8%、擊殺恢復 MP +3%。'},
    {id:'kings_grace',n:'王者恩典',seq:['rune_amn','rune_ral','rune_thul'],kind:'meleewpn',effects:{basicDmg:18,lifeSteal:5,bossDmg:5},d:'一般攻擊傷害 +18%、吸血 +5%、對頭目一般攻擊傷害 +5%。'},
    {id:'lionheart',n:'獅心',seq:['rune_el','rune_sol','rune_ith'],kind:'armor',effects:{allRes:15,allResNone:15,flatDr:4,killHp:3},d:'五屬性抗性 +15、直接傷害減少 4、擊殺恢復 HP +3%。'},
    {id:'fortitude',n:'剛毅',seq:['rune_el','rune_sol','rune_dol','rune_lo'],kind:'armor',effects:{basicDmg:30,ac:10,hpPct:20,allRes:20,allResNone:20,flatDr:7},d:'一般攻擊傷害 +30%、AC -10、HP 上限 +20%、五屬性抗性 +20、直接傷害減少 7。'},
    {id:'enigma',n:'謎團',seq:['rune_jah','rune_ith','rune_ber'],kind:'armor',grantSkills:['sk_teleport'],effects:{ac:10,hpPct:15,allStat:5,magicDmg:10,mf:20,flatDr:5,mpCost:10},d:'可額外使用原有「傳送術」；AC -10、HP 上限 +15%、全能力 +5、魔法傷害 +10、物品發現率 +20%、直接傷害減少 5、技能 MP 消耗降低 10%。'},
    // ===== v3.9.0 跨流派符文之語 =====
    {id:'honor',n:'榮耀',seq:['rune_amn','rune_el','rune_ith','rune_tir','rune_sol'],kind:'meleewpn',effects:{basicDmg:25,as:5,lifeSteal:5,bossDmg:10,killHp:5,killMp:5},d:'一般攻擊傷害 +25%、攻擊速度 +5%、吸血 +5%、對頭目傷害 +10%、擊殺恢復 HP／MP +5%。'},
    {id:'grief',n:'悔恨',seq:['rune_eth','rune_tir','rune_lo','rune_mal','rune_ral'],kind:'meleewpn',effects:{basicDmg:40,as:15,ignoreDef:20,bossDmg:10},d:'一般攻擊傷害 +40%、攻擊速度 +15%、忽略防禦 20%、對頭目傷害 +10%。'},
    {id:'heart_of_the_oak',n:'橡樹之心',seq:['rune_ko','rune_vex','rune_pul','rune_thul'],kind:'magicwpn',effects:{magicDmg:25,allStat:4,mpPct:20,allRes:20,allResNone:20,mpCost:10},d:'魔法傷害 +25、全能力 +4、MP 上限 +20%、五屬性抗性 +20、技能 MP 消耗降低 10%。'},
    {id:'chains_of_honor',n:'榮耀之鍊',seq:['rune_dol','rune_um','rune_ber','rune_ist'],kind:'armor',effects:{allRes:30,allResNone:30,flatDr:8,hpPct:15,mf:10},d:'五屬性抗性 +30、直接傷害減少 8、HP 上限 +15%、物品發現率 +10%。'},
    {id:'faith',n:'信心',seq:['rune_ohm','rune_jah','rune_lem','rune_eld'],kind:'rangedwpn',effects:{as:20,basicDmg:30,bossDmg:15,allStat:3},d:'攻擊速度 +20%、一般攻擊傷害 +30%、對頭目傷害 +15%、全能力 +3。'},
    {id:'call_to_arms',n:'戰爭召喚',seq:['rune_amn','rune_ral','rune_mal','rune_ist','rune_ohm'],kind:'wpn',effects:{basicDmg:20,as:5,allStat:5,hpPct:15,mpPct:15},d:'一般攻擊傷害 +20%、攻擊速度 +5%、全能力 +5、HP／MP 上限 +15%。'},
    {id:'phoenix',n:'鳳凰',seq:['rune_vex','rune_vex','rune_lo','rune_jah'],kind:'shield',effects:{basicDmg:20,hpR:10,mpR:5,allRes:20,allResNone:20,flatDr:5},d:'一般攻擊傷害 +20%、HP 恢復 +10、MP 恢復 +5、五屬性抗性 +20、直接傷害減少 5。'}
    ,{id:'mosaic',n:'馬賽克',seq:['rune_mal','rune_gul','rune_amn'],kind:'mosaicclaw',effects:{as:10,basicDmg:15,mosaic:1},d:'黑暗妖精鋼爪／雙刀專用。雙重破壞期間普攻命中累積元素蓄能；滿 3 層後，原本會心一擊追加火、水、風三段元素終結。蓄能不消耗，但有保存時間與獨立冷卻。'}
];
const RUNEWORD_EFFECT_META={
    basicDmg:['一般攻擊傷害','%'],as:['攻擊速度','%'],ignoreDef:['忽略防禦','%'],bossDmg:['對頭目一般攻擊傷害','%'],lifeSteal:['一般攻擊吸血','%'],block:['格擋率','%'],
    magicDmg:['魔法傷害',''],mpCost:['技能 MP 消耗','%',true],killHp:['擊殺恢復 HP','%'],killMp:['擊殺恢復 MP','%'],flatDr:['直接傷害減少',''],
    hpPct:['HP 上限','%'],mpPct:['MP 上限','%'],hpR:['HP 恢復',''],mpR:['MP 恢復',''],mf:['物品發現率','%'],
    allRes:['火／水／地／風抗性',''],allResNone:['無屬性抗性',''],allStatusRed:['中毒／燃燒／麻痺／冰凍時間','%',true],freezeRed:['冰凍時間','%',true],paralyzeRed:['麻痺時間','%',true],poisonRed:['中毒時間','%',true],burnRed:['燃燒時間','%',true],
    str:['力量',''],dex:['敏捷',''],con:['體質',''],int:['智力',''],wis:['精神',''],cha:['魅力',''],allStat:['全能力',''],ac:['防禦（AC）','',true],knock:['攻擊延遲機率','%']
};
function runewordEffectLines(word){
    if(!word)return [];
    let out=[];
    if(Array.isArray(word.grantSkills))word.grantSkills.forEach(id=>out.push(`授予技能：${(DB.skills[id]&&DB.skills[id].n)||id}`));
    Object.entries(word.effects||{}).forEach(([key,value])=>{
        if(key==='mosaic'){out.push('啟動馬賽克蓄能：雙重破壞普攻集氣，會心一擊釋放三屬性終結');return;}
        let meta=RUNEWORD_EFFECT_META[key];
        if(!meta){out.push(`${key} +${value}`);return;}
        out.push(`${meta[0]} ${meta[2]?'-':'+'}${value}${meta[1]}`);
    });
    return out;
}
function runewordBaseFits(word,item){
    let d=item&&DB.items[item.id];if(!word||!d)return false;
    if(word.kind==='wpn')return d.type==='wpn'&&!d.isArrow;
    if(word.kind==='meleewpn')return d.type==='wpn'&&!d.isArrow&&!d.ranged&&!d.isBow&&!d.qigu&&!(typeof isWandWeapon==='function'&&isWandWeapon(d));
    if(word.kind==='magicwpn')return d.type==='wpn'&&!d.isArrow&&!!(d.magic||d.mdmg||d.qigu||(typeof isWandWeapon==='function'&&isWandWeapon(d)));
    if(word.kind==='rangedwpn')return d.type==='wpn'&&!d.isArrow&&!!(d.ranged||d.isBow);
    if(word.kind==='mosaicclaw')return d.type==='wpn'&&!d.isArrow&&String(d.req||'').split(',').includes('dark')&&(d.eff==='combo'||/claw|dual/.test(String(item.id||'')));
    if(word.kind==='magicorshield')return (d.type==='arm'&&d.slot==='shield')||(d.type==='wpn'&&!d.isArrow&&!!(d.magic||d.mdmg||d.qigu||(typeof isWandWeapon==='function'&&isWandWeapon(d))));
    if(word.kind==='armor')return d.type==='arm'&&d.slot==='armor';
...
function runewordSocketState(item){
    ...
    for(let socket of sockets){
        if(!socket){gap=true;continue;}
        if(socket.kind!=='rune')return {sockets,runes,invalid:'gem'};
        if(gap)return {sockets,runes,invalid:'gap'};
        runes.push(socket);
    }
    return {sockets,runes,invalid:''};
}
function activeRuneword(item){
    if(!item||d2rAffixRows(item).length)return null; // 白板專用：任何八色詞綴都不成立。
    // v3.9.5：恢復指定孔數；所有孔都必須依序填滿，總孔數必須與配方完全相同。
    let state=runewordSocketState(item);
    if(!state.sockets.length||state.invalid||state.runes.length!==state.sockets.length)return null;
    return RUNEWORDS.find(w=>w.seq.length===state.sockets.length&&runewordBaseFits(w,item)&&w.seq.every((id,i)=>state.runes[i].id===id))||null;
}
...
function equippedRunewordRows(owner){
    ...
    rows.forEach((row,i)=>row.enabled=i<RUNEWORD_EQUIP_LIMIT);
    return rows;
}
function effectiveRuneword(owner,item){
    let row=equippedRunewordRows(owner).find(r=>r.item===item);
    return row&&row.enabled?row.word:null;
}
function runewordPaused(owner,item){
    let row=equippedRunewordRows(owner).find(r=>r.item===item);
...
function runewordHint(item){
    ...
    if(d2rAffixRows(item).length)return '八色裝備不能形成符文之語';
    let state=runewordSocketState(item),sockets=state.sockets,runes=state.runes;
    if(state.invalid==='gem')return '混入寶石，無法形成';
    if(state.invalid==='gap')return '符文必須從第 1 孔開始連續依序鑲嵌';
    let candidates=RUNEWORDS.filter(w=>runewordBaseFits(w,item)&&w.seq.length===sockets.length&&runes.length<=w.seq.length&&runes.every((s,i)=>w.seq[i]===s.id));
    if(candidates.length){let w=candidates[0],next=w.seq[runes.length];return next?`可形成「${w.n}」，尚缺 ${DB.items[next].n}`:`可形成「${w.n}」`;}
...
function runeEquipTotals(owner){
    let out={};if(!owner||!owner.eq)return out;
    ...
    if(typeof alchemyAccessoryTotals==='function'){
        ...
        Object.keys(_alchemy).forEach(code=>{
            let cap=typeof alchemyCodeCap==='function'?alchemyCodeCap(code):Number.MAX_SAFE_INTEGER;
            out[code]=Math.min(cap,(out[code]||0)+(Number(_alchemy[code])||0));
        });
    }
    let rw=runewordEquipTotals(owner);
    ['killHp','killMp','mpCost','knock','ignoreDef','bossDmg','poisonRed','burnRed','paralyzeRed','freezeRed','lifeSteal','flatDr'].forEach(k=>{if(rw[k])out[k]=(out[k]||0)+rw[k];});
    if(rw.allStatusRed){out.poisonRed=(out.poisonRed||0)+rw.allStatusRed;out.burnRed=(out.burnRed||0)+rw.allStatusRed;out.paralyzeRed=(out.paralyzeRed||0)+rw.allStatusRed;out.freezeRed=(out.freezeRed||0)+rw.allStatusRed;}
    return out;
}
function runeIncomingDamage(owner,dmg){let r=runeEquipTotals(owner);return Math.max(0,Math.floor(Number(dmg)||0)-(r.flatDr||0));}
function runeOutgoingDamage(owner,target,dmg){let r=runeEquipTotals(owner),rw=runewordEquipTotals(owner),v=Number(dmg)||0;v=v*(100+(r.basicDmg||0)+(rw.basicDmg||0))/100;if(target&&target.boss)v=v*(100+(r.bossDmg||0))/100;return Math.max(1,Math.floor(v));}
function runeOnDamaged(owner,attacker){let r=runeEquipTotals(owner);if(r.knock&&attacker&&Math.random()*100<r.knock){attacker._delayTicks=Math.max(attacker._delayTicks||0,10);logCombat('<span class="text-amber-300 font-bold">【Nef】</span>受擊衝擊使敵人延遲行動。','player-special');}}
function runeOnKill(owner){
    let r=runeEquipTotals(owner);if(!owner)return;
    if(r.killHp){let key=owner===player?'hp':'curHp';owner[key]=Math.min(owner.mhp||1,(owner[key]||0)+Math.max(1,Math.floor((owner.mhp||1)*r.killHp/100)));}
    if(r.killMp)owner.mp=Math.min(owner.mmp||0,(owner.mp||0)+Math.max(1,Math.floor((owner.mmp||1)*r.killMp/100)));
}
function rollRuneDropForMob(mob){
    if(!mob||mob.race==='建築')return null;
    let groups=mob.boss?[.06,.025,.008,.004,.0015,.0003,.00005]:[.002,.0008,.00025,.0001,.000025,.000003,.0000005],roll=lootRng('rune-drop'),tier=0,sum=0;
    for(let i=groups.length-1;i>=0;i--){sum+=groups[i];if(roll<sum){tier=i+1;break;}}
    if(!tier)return null;
    let pool=RUNE_IDS.filter(id=>RUNE_DEFS[id].tier===tier),id=pool[Math.floor(lootRng('rune-kind')*pool.length)];
    gainItem(id,1,true,true);logSys(`<span class="text-amber-300 font-bold">ᚱ ${mob.n} 掉落了 ${DB.items[id].n}！</span>`);return id;
}
const D2R_AFFIX_RANGES = {
    md:[[7,10],[5,7],[3,5],[2,3],[1,2]], rd:[[7,10],[5,7],[3,5],[2,3],[1,2]], mg:[[6,8],[4,6],[3,4],[2,3],[1,2]],
    mh:[[11,15],[8,10],[5,7],[3,4],[1,2]], rh:[[11,15],[8,10],[5,7],[3,4],[1,2]], gh:[[9,12],[6,8],[4,6],[2,4],[1,2]],
    str:[[4,5],[3,4],[2,3],[1,2],[1,1]], dex:[[4,5],[3,4],[2,3],[1,2],[1,1]], con:[[4,5],[3,4],[2,3],[1,2],[1,1]],
    int:[[4,5],[3,4],[2,3],[1,2],[1,1]], wis:[[4,5],[3,4],[2,3],[1,2],[1,1]], cha:[[4,5],[3,4],[2,3],[1,2],[1,1]],
    hp:[[111,160],[71,110],[41,70],[21,40],[10,20]], mp:[[56,80],[36,55],[21,35],[11,20],[5,10]],
    rf:[[17,22],[12,16],[8,11],[5,7],[2,4]], rw:[[17,22],[12,16],[8,11],[5,7],[2,4]],
    re:[[17,22],[12,16],[8,11],[5,7],[2,4]], ra:[[17,22],[12,16],[8,11],[5,7],[2,4]], rn:[[14,18],[10,13],[7,9],[4,6],[2,3]],
    ...
    ,ac:[[7,8],[5,6],[3,4],[2,2],[1,1]], mr:[[17,20],[13,16],[9,12],[5,8],[2,4]], er:[[7,8],[5,6],[3,4],[2,2],[1,1]], dr:[[7,8],[5,6],[3,4],[2,2],[1,1]]
    ,mc:[[5,5],[4,4],[3,3],[2,2],[1,1]], rc:[[5,5],[4,4],[3,3],[2,2],[1,1]], gc:[[5,5],[4,4],[3,3],[2,2],[1,1]]
    ,mcd:[[26,30],[20,25],[14,19],[8,13],[5,7]], rcd:[[26,30],[20,25],[14,19],[8,13],[5,7]], gcd:[[26,30],[20,25],[14,19],[8,13],[5,7]]
    ,hpp:[[9,10],[7,8],[5,6],[3,4],[1,2]], mpp:[[9,10],[7,8],[5,6],[3,4],[1,2]], pot:[[17,20],[13,16],[9,12],[5,8],[2,4]], abr:[[17,20],[13,16],[9,12],[5,8],[2,4]]
    ,gf:[[9,10],[7,8],[5,6],[3,4],[1,2]], xf:[[5,5],[4,4],[3,3],[2,2],[1,1]]
    ,nd:[[9,10],[7,8],[5,6],[3,4],[1,2]], bd:[[6,6],[5,5],[4,4],[3,3],[2,2]]
    ,kh:[[16,20],[11,15],[7,10],[4,6],[2,3]], km:[[5,5],[4,4],[3,3],[2,2],[1,1]]
    ,kx:[[6,6],[5,5],[4,4],[3,3],[2,2]], ts:[[6,6],[5,5],[4,4],[3,3],[2,2]]
    ,pi:[[50,50],[44,44],[38,38],[31,31],[25,25]], ks:[[10,10],[8,8],[6,6],[4,4],[2,2]]
    ,sp:[[30,30],[25,25],[20,20],[15,15],[10,10]], fh:[[30,30],[25,25],[20,20],[15,15],[10,10]]
    ,fpen:[[10,12],[8,9],[6,7],[4,5],[2,3]], wpen:[[10,12],[8,9],[6,7],[4,5],[2,3]]
    ,epen:[[10,12],[8,9],[6,7],[4,5],[2,3]], apen:[[10,12],[8,9],[6,7],[4,5],[2,3]]
    ,sdm:[[13,15],[10,12],[7,9],[5,6],[3,4]], bdr:[[7,8],[5,6],[4,4],[3,3],[2,2]]
    ,und:[[12,14],[10,11],[7,9],[5,6],[3,4]], dem:[[12,14],[10,11],[7,9],[5,6],[3,4]], dra:[[12,14],[10,11],[7,9],[5,6],[3,4]]
    ,hsk:[[10,12],[8,9],[6,7],[4,5],[2,3]], opn:[[10,12],[8,9],[6,7],[4,5],[2,3]], exe:[[13,15],[10,12],[7,9],[5,6],[3,4]]
    ,pdr:[[6,7],[5,5],[4,4],[3,3],[2,2]], mdr:[[6,7],[5,5],[4,4],[3,3],[2,2]]
    ,ldr:[[7,8],[5,6],[4,4],[3,3],[2,2]], adr:[[7,8],[5,6],[4,4],[3,3],[2,2]]
    ,udr:[[7,8],[5,6],[4,4],[3,3],[2,2]], ddr:[[7,8],[5,6],[4,4],[3,3],[2,2]]
    ,blk:[[6,7],[5,5],[4,4],[3,3],[2,2]], mgd:[[9,10],[7,8],[5,6],[4,4],[3,3]]
    ,rcv:[[4,5],[3,4],[3,3],[2,2],[1,1]], sav:[[8,10],[6,7],[4,5],[3,3],[2,2]]
    ,bar:[[22,25],[18,21],[14,17],[10,13],[6,9]], rip:[[12,15],[9,11],[7,8],[5,6],[3,4]]
    ...
    ,fbr:[[10,12],[8,9],[6,7],[4,5],[2,3]], wfr:[[10,12],[8,9],[6,7],[4,5],[2,3]]
    ,eps:[[10,12],[8,9],[6,7],[4,5],[2,3]], wbl:[[10,12],[8,9],[6,7],[4,5],[2,3]]
    ,fsi:[[7,9],[6,7],[4,5],[3,4],[2,3]], wsi:[[4,5],[3,4],[2,3],[2,2],[1,1]]
    ,esi:[[7,9],[6,7],[4,5],[3,4],[2,3]], asi:[[7,9],[6,7],[4,5],[3,4],[2,3]]
    ...
    ,gd:[[26,30],[21,25],[16,20],[11,15],[6,10]], scd:[[9,10],[7,8],[5,6],[3,4],[1,2]]
    ,ww:[[26,30],[21,25],[16,20],[11,15],[6,10]], wd:[[26,30],[21,25],[16,20],[11,15],[6,10]]
    ,wr:[[26,30],[21,25],[16,20],[11,15],[6,10]], wc:[[9,10],[7,8],[5,6],[3,4],[1,2]]
    ,ld:[[26,30],[21,25],[16,20],[11,15],[6,10]], ls:[[26,30],[21,25],[16,20],[11,15],[6,10]]
    ,lc:[[9,10],[7,8],[5,6],[3,4],[1,2]], lr:[[9,10],[7,8],[5,6],[3,4],[1,2]]
    ,bh:[[26,30],[21,25],[16,20],[11,15],[6,10]], bn:[[2,2],[1,1],[1,1],[1,1],[1,1]]
    ,bt:[[26,30],[21,25],[16,20],[11,15],[6,10]], bc:[[9,10],[7,8],[5,6],[3,4],[1,2]]
    ,shd:[[26,30],[21,25],[16,20],[11,15],[6,10]], shn:[[2,2],[1,1],[1,1],[1,1],[1,1]]
    ,sht:[[26,30],[21,25],[16,20],[11,15],[6,10]], shc:[[9,10],[7,8],[5,6],[3,4],[1,2]]
    ,vwd:[[26,30],[21,25],[16,20],[11,15],[6,10]], vwn:[[2,2],[1,1],[1,1],[1,1],[1,1]]
    ,vws:[[26,30],[21,25],[16,20],[11,15],[6,10]], vwc:[[9,10],[7,8],[5,6],[3,4],[1,2]]
    ,fdd:[[26,30],[21,25],[16,20],[11,15],[6,10]], fdn:[[2,2],[1,1],[1,1],[1,1],[1,1]]
    ,fdf:[[11,12],[9,10],[7,8],[5,6],[2,4]], fdc:[[9,10],[7,8],[5,6],[3,4],[1,2]]
    ,tjd:[[26,30],[21,25],[16,20],[11,15],[6,10]], tjn:[[2,2],[1,1],[1,1],[1,1],[1,1]]
    ,tjs:[[11,12],[9,10],[7,8],[5,6],[2,4]], tjc:[[9,10],[7,8],[5,6],[3,4],[1,2]]
    ,med:[[26,30],[21,25],[16,20],[11,15],[6,10]], men:[[2,2],[1,1],[1,1],[1,1],[1,1]]
    ,mes:[[11,12],[9,10],[7,8],[5,6],[2,4]], mec:[[9,10],[7,8],[5,6],[3,4],[1,2]]
    ,rbd:[[26,30],[21,25],[16,20],[11,15],[6,10]], rbn:[[2,2],[1,1],[1,1],[1,1],[1,1]]
    ,rbf:[[11,12],[9,10],[7,8],[5,6],[2,4]], rbc:[[9,10],[7,8],[5,6],[3,4],[1,2]]
    ,ard:[[26,30],[21,25],[16,20],[11,15],[6,10]], arn:[[4,4],[3,3],[2,2],[1,1],[1,1]]
    ,arc:[[11,12],[9,10],[7,8],[5,6],[2,4]], acd:[[9,10],[7,8],[5,6],[3,4],[1,2]]
    ,csd:[[26,30],[21,25],[16,20],[11,15],[6,10]], csn:[[2,2],[1,1],[1,1],[1,1],[1,1]]
    ,csp:[[11,12],[9,10],[7,8],[5,6],[2,4]], csc:[[9,10],[7,8],[5,6],[3,4],[1,2]]
    ,mzd:[[26,30],[21,25],[16,20],[11,15],[6,10]], mzn:[[2,2],[1,1],[1,1],[1,1],[1,1]]
    ,mzt:[[26,30],[21,25],[16,20],[11,15],[6,10]], mzc:[[9,10],[7,8],[5,6],[3,4],[1,2]]
    ,trd:[[26,30],[21,25],[16,20],[11,15],[6,10]], trn:[[2,2],[1,1],[1,1],[1,1],[1,1]]
    ,trt:[[26,30],[21,25],[16,20],[11,15],[6,10]], trc:[[9,10],[7,8],[5,6],[3,4],[1,2]]
    ,qmd:[[26,30],[21,25],[16,20],[11,15],[6,10]], qmn:[[2,2],[1,1],[1,1],[1,1],[1,1]]
    ,qmt:[[26,30],[21,25],[16,20],[11,15],[6,10]], qmc:[[9,10],[7,8],[5,6],[3,4],[1,2]]
    ,srd:[[26,30],[21,25],[16,20],[11,15],[6,10]], srb:[[7,9],[6,7],[4,5],[3,4],[2,3]]
    ,srg:[[7,9],[6,7],[4,5],[3,4],[2,3]], src:[[9,10],[7,8],[5,6],[3,4],[1,2]]
    ,bfd:[[26,30],[21,25],[16,20],[11,15],[6,10]], bfn:[[2,2],[1,1],[1,1],[1,1],[1,1]]
    ,bft:[[26,30],[21,25],[16,20],[11,15],[6,10]], bfc:[[9,10],[7,8],[5,6],[3,4],[1,2]]
    ,kvd:[[26,30],[21,25],[16,20],[11,15],[6,10]], kvn:[[2,2],[1,1],[1,1],[1,1],[1,1]]
    ,kvp:[[11,12],[9,10],[7,8],[5,6],[2,4]], kvc:[[9,10],[7,8],[5,6],[3,4],[1,2]]
    ,ufd:[[26,30],[21,25],[16,20],[11,15],[6,10]], ufs:[[2,2],[1,1],[1,1],[1,1],[1,1]]
    ,ufg:[[7,9],[6,7],[4,5],[3,4],[2,3]], ufc:[[9,10],[7,8],[5,6],[3,4],[1,2]]
    ,dld:[[26,30],[21,25],[16,20],[11,15],[6,10]], dln:[[2,2],[1,1],[1,1],[1,1],[1,1]]
    ,dlt:[[26,30],[21,25],[16,20],[11,15],[6,10]], dlc:[[9,10],[7,8],[5,6],[3,4],[1,2]]
};
function d2rAffixRows(item) {
    if (!item || !Array.isArray(item.d2)) return [];
    return item.d2.slice(0, 10).filter(r => Array.isArray(r) && D2R_AFFIX_LABEL[r[0]] && Number.isFinite(Number(r[1])) && Number(r[1]) > 0)
        .map(r => [r[0], Math.floor(Number(r[1])), Math.max(1, Math.min(5, Math.floor(Number(r[2])) || 5))]);
}
// 每 +3 強化使裝備原有的八色詞綴效果提高 5%，+15 時封頂 +25%。
// 僅在結算時放大，不改寫詞綴、不增加詞綴數，也不影響寶石、符文或符文之語。
function d2rEnhanceAffixMult(item) {
    if (!d2rAffixRows(item).length) return 1;
    let en = Math.max(0, Math.min(15, Math.floor(Number(item && item.en) || 0)));
    return 1 + Math.floor(en / 3) * .05;
}
function d2rEquipTotals(owner) {
    let out = {}, gemAs=0, gemMf=0;
    if (!owner || !owner.eq) return out;
    Object.values(owner.eq).forEach(it => {
        let mult = d2rDurabilityMult(it) * d2rEnhanceAffixMult(it);
        d2rAffixRows(it).forEach(r => { out[r[0]] = (out[r[0]] || 0) + r[1] * mult; });
        equipSocketRows(it).filter(Boolean).forEach(socket => {
            if(!socket||socket.kind!=='gem')return;
            let eff=gemEffectForItem(socket.id,it);if(!eff)return;
            if(eff.code==='as')gemAs+=eff.value;
            else if(eff.code==='mf')gemMf+=eff.value;
            else out[eff.code]=(out[eff.code]||0)+eff.value;
        });
    });
    let rw=runewordEquipTotals(owner);
    if(rw.as)out.as=(out.as||0)+rw.as;
    if(rw.magicDmg)out.mg=(out.mg||0)+rw.magicDmg;
    if(rw.int)out.int=(out.int||0)+rw.int;if(rw.wis)out.wis=(out.wis||0)+rw.wis;
    if(rw.allStat)['str','dex','con','int','wis','cha'].forEach(k=>out[k]=(out[k]||0)+rw.allStat);
    if(rw.ac)out.ac=(out.ac||0)+rw.ac;
    if(rw.allRes){out.rf=(out.rf||0)+rw.allRes;out.rw=(out.rw||0)+rw.allRes;out.re=(out.re||0)+rw.allRes;out.ra=(out.ra||0)+rw.allRes;}
    if(rw.allResNone)out.rn=(out.rn||0)+rw.allResNone;
    let singleRune=runeEquipTotals(owner);
    if(singleRune.hpR)out.hpr=(out.hpr||0)+singleRune.hpR;
    if(singleRune.resWind)out.ra=(out.ra||0)+singleRune.resWind;
    if(singleRune.dex)out.dex=(out.dex||0)+singleRune.dex;
    if(singleRune.goldFind)out.gf=(out.gf||0)+singleRune.goldFind;
    if(singleRune.allRes){out.rf=(out.rf||0)+singleRune.allRes;out.rw=(out.rw||0)+singleRune.allRes;out.re=(out.re||0)+singleRune.allRes;out.ra=(out.ra||0)+singleRune.allRes;}
    if(rw.hpR)out.hpr=(out.hpr||0)+rw.hpR;
    if(rw.mpR)out.mpr=(out.mpr||0)+rw.mpR;
    if(singleRune.mf)out.mf=(out.mf||0)+singleRune.mf;
    if(rw.mf)out.mf=(out.mf||0)+rw.mf;
    out.as=(out.as||0)+Math.min(15,gemAs);
    out.mf=(out.mf||0)+Math.min(30,gemMf);
    return out;
}
function d2rHuntTotals(owner,rawTotals) {
    let t=rawTotals||d2rEquipTotals(owner), cap={gf:50,xf:25,nd:30,bd:20,kh:60,km:15}, out={};
...
// 元素刻印是「裝備技能」：本次命中先結算傷害，成功後施加狀態，因此連攜增傷從下一擊開始生效。
function d2rElementImprintTry(owner,target,damage,ele,t) {
    if(!owner||!target||!damage||!t||Math.max(0,Number(target.curHp)||0)<=damage)return '';
    let code={fire:'fsi',water:'wsi',earth:'esi',wind:'asi'}[ele],chance=code&&(Number(t[code])||0);
    if(!chance||Math.random()*100>=chance)return '';
    let st=target.st||(target.st=typeof newMobStatus==='function'?newMobStatus():{}), src=owner._allyName&&typeof _dpsAllySrc==='function'?_dpsAllySrc(owner):'player';
    if(code==='fsi'){
        let old=target._burnDot||{},dot=Math.max(1,Math.floor(damage*.05));
        target._burnDot={left:50,dmg:Math.max(Number(old.dmg)||0,dot),tick:10,src:dot>=(Number(old.dmg)||0)?src:(old.src||src)};
    }else if(code==='wsi'){
        if(target.boss)return '';
        st.freeze=Math.max(Number(st.freeze)||0,20);
    }else if(code==='esi'){
        let dot=Math.max(1,Math.floor(damage*.04)),oldUnit=Number(st.poisonUnit)||0;
        st.poison=60;st.poisonTick=10;st.poisonStacks=1;st.poisonUnit=Math.max(oldUnit,dot);st.poisonDmg=st.poisonUnit;st.poisonSrc=dot>=oldUnit?src:(st.poisonSrc||src);
    }else if(code==='asi'){
        target.bleeds=target.bleeds||[];target._bleedCap=Math.max(Number(target._bleedCap)||0,5);
        while(target.bleeds.length>=target._bleedCap)target.bleeds.shift();
        target.bleeds.push({dmg:Math.max(1,Math.floor(damage*.05)),ticksLeft:80});target._bleedSrc=src;
    }
    let now=typeof state==='object'&&state?state.ticks:0,key='_d2rImprintLog_'+code;
    if(typeof logCombat==='function'&&now>=(Number(target[key])||0)){
        target[key]=now+30;let name={fsi:'熾焰刻印',wsi:'寒潮刻印',esi:'腐毒刻印',asi:'裂風刻印'}[code];
        logCombat(`<span class="font-bold text-amber-300">【${name}】</span>${target.n||'目標'}被元素刻印侵蝕。`,'player-special');
    }
    return code;
}
function d2rOutgoingDamage(owner,target,dmg,ele,hadAbnormal) {
    ...
    let t=d2rCombatAffixTotals(owner), pen={fire:t.fpen,water:t.wpen,earth:t.epen,wind:t.apen}[ele]||0;
    let race=target&&String(target.race||''), hp=target&&Math.max(0,Number(target.curHp)||0), mhp=target&&Math.max(1,Number(target.hp)||1);
    let pct=pen+((hadAbnormal||d2rTargetHasAbnormal(target))?t.sdm:0);
    if(target&&(target.un||race==='不死'||race==='殭屍'))pct+=t.und;
    if(race==='惡魔')pct+=t.dem;
    if(race&&race.includes('龍'))pct+=t.dra;
    ...
    if(ele==='fire'&&t.fbr&&d2rTargetCondition(target,'brn'))pct+=t.fbr;
    if(ele==='water'&&t.wfr&&(hadAbnormal||d2rTargetCondition(target,'frz')))pct+=t.wfr;
    if(ele==='earth'&&t.eps&&d2rTargetCondition(target,'psn'))pct+=t.eps;
    if(ele==='wind'&&t.wbl&&d2rTargetCondition(target,'bld'))pct+=t.wbl;
    ...
    if(t.shp&&(Number(owner._d2rShield)||0)>0)pct+=t.shp;
...
function d2rHuntDamage(owner,target,dmg,ele,hadAbnormal) {
    let t=d2rHuntTotals(owner), pct=target&&target.boss?t.bd:t.nd;
    dmg=Math.max(1,Math.floor((Number(dmg)||0)*(1+pct/100)));
    return d2rOutgoingDamage(owner,target,dmg,ele,hadAbnormal);
}
function d2rHuntOnKill(owner) {
    if(!owner)return;
    let raw=d2rEquipTotals(owner),t=d2rHuntTotals(owner,raw),ct=d2rCombatAffixTotals(owner,raw),hpKey=owner===player?'hp':'curHp',mhp=Math.max(1,Number(owner.mhp)||1),now=(typeof state==='object'&&state?state.ticks:0);
    if(t.kh)owner[hpKey]=Math.min(owner.mhp||1,(owner[hpKey]||0)+Math.floor(t.kh));
    if(t.km)owner.mp=Math.min(owner.mmp||0,(owner.mp||0)+Math.floor(t.km));
    if(ct.krc)owner[hpKey]=Math.min(mhp,(owner[hpKey]||0)+Math.max(1,Math.floor(mhp*ct.krc/100)));
    if(ct.ksh)owner._d2rShield=Math.max(owner._d2rShield||0,Math.max(1,Math.floor(mhp*ct.ksh/100)));
    if(ct.kfu){let fresh=now>=(owner._d2rKillFuryUntil||0);owner._d2rKillFuryPct=ct.kfu;owner._d2rKillFuryUntil=now+50;if(fresh)logCombat(`<span class="text-orange-300 font-bold">【擊殺戰意】</span>傷害 +${ct.kfu}%（5 秒）。`,'player-special',owner===player?'player':'mercenary');}
}
function d2rTriggerTotals(owner) {
    let out={kx:0,ts:0,pi:0,piRate:0,ks:0};
    if(!owner||!owner.eq)return out;
    Object.values(owner.eq).filter(Boolean).forEach(it=>{
        let mult=d2rDurabilityMult(it)*d2rEnhanceAffixMult(it);
        d2rAffixRows(it).forEach(r=>{
            if(r[0]==='pi'){out.pi=Math.max(out.pi,r[1]*mult);out.piRate+=(6-r[2])*.5+.5;}
            else if(Object.prototype.hasOwnProperty.call(out,r[0]))out[r[0]]+=r[1]*mult;
        });
    });
    out.kx=Math.min(6,out.kx);out.ts=Math.min(6,out.ts);out.pi=Math.min(50,out.pi);out.piRate=Math.min(8,out.piRate);out.ks=Math.min(15,out.ks);
    return out;
}
function d2rPerfectStrike(owner,dmg) {
    let t=d2rTriggerTotals(owner);
    if(!t.piRate||Math.random()*100>=t.piRate)return {dmg:Math.max(1,Math.floor(dmg)),proc:false};
    return {dmg:Math.max(1,Math.floor(dmg*(1+t.pi/100))),proc:true};
}
function d2rTriggerIncoming(owner,dmg,source,kind) {
    dmg=Math.max(0,Math.floor(Number(dmg)||0));if(!owner||!dmg)return dmg;
    let ct=d2rCombatAffixTotals(owner), race=source&&String(source.race||''), hp=owner.curHp!=null?owner.curHp:owner.hp, mhp=Math.max(1,Number(owner.mhp)||1), reduce=0;
    if(source&&source.boss)reduce+=ct.bdr;
    if(kind==='physical')reduce+=ct.pdr;
    if(kind==='magic')reduce+=ct.mdr;
    if((Number(hp)||0)/mhp<=.35)reduce+=ct.ldr;
    if(d2rTargetHasAbnormal(owner))reduce+=ct.adr;
    if(source&&(source.un||race==='不死'||race==='殭屍'))reduce+=ct.udr;
    if(race==='惡魔')reduce+=ct.ddr;
    reduce=Math.min(60,reduce);if(reduce)dmg=Math.max(1,Math.floor(dmg*(1-reduce/100)));
    if(dmg>0&&ct.blk&&Math.random()*100<ct.blk){dmg=Math.max(1,Math.floor(dmg*.5));logCombat(`<span class="text-sky-300 font-bold">【堅守格擋】</span>擋下 50% 傷害。`,'player-special',owner===player?'player':'mercenary');}
    ...
    if(dmg>0&&t.ts&&now>=(owner._d2rShieldCd||0)&&Math.random()<.05){
        ...
        owner._d2rShieldCd=now+100;
        logCombat(`<span class="text-cyan-300 font-bold">【受擊護盾】</span>獲得 ${owner._d2rShield} 點護盾（10 秒冷卻）。`,'player-special',owner===player?'player':'mercenary');
    }
    if(dmg>0&&ct.sav&&dmg>=Math.max(1,Number(hp)||0)&&now>=(owner._d2rSaviorCd||0)){
        dmg=Math.max(0,(Number(hp)||1)-1);owner._d2rSaviorCd=now+600;
        let guard=Math.max(1,Math.floor(mhp*ct.sav/100));owner._d2rShield=Math.max(owner._d2rShield||0,guard);
        logCombat(`<span class="text-amber-300 font-bold">【致命守護】</span>保留 1 HP 並獲得 ${guard} 點護盾（60 秒冷卻）。`,'player-special',owner===player?'player':'mercenary');
    }
    return dmg;
}
function d2rAfterIncoming(owner,dmg) {
    dmg=Math.max(0,Math.floor(Number(dmg)||0));if(!owner||!dmg)return;
    let hpKey=owner===player?'hp':'curHp',hp=Number(owner[hpKey])||0;if(hp<=0)return;
    let ct=d2rCombatAffixTotals(owner),mhp=Math.max(1,Number(owner.mhp)||1),now=(typeof state==='object'&&state?state.ticks:0),channel=owner===player?'player':'mercenary';
...
function d2rTriggerOnTeamKill(mob) {
    ...
    let members=[player].concat((player.allies||[]).filter(a=>a&&!a._downed));
    members.forEach(o=>{let t=d2rTriggerTotals(o);if(t.ks){o._d2rKillHastePct=t.ks;o._d2rKillHasteUntil=(state.ticks||0)+50;}});
    if(mob.boss||window._d2rExplosionGuard)return;
    let best=members.map(o=>({o,t:d2rTriggerTotals(o)})).sort((a,b)=>b.t.kx-a.t.kx)[0];
    if(!best||!best.t.kx)return;
    let dmg=Math.min((best.o.lv||1)*20,Math.max(1,Math.floor((mob.hp||1)*best.t.kx/100))), dead=[], hit=0;
    mapState.mobs.forEach((m,i)=>{if(m&&m!==mob&&!m._dead&&m.curHp>0){hit++;m.curHp-=dmg;m.justHit='magic';mobWake(m);if(m.curHp<=0)dead.push(i);}});
    if(hit)logCombat(`<span class="text-orange-300 font-bold">【擊殺爆炸】</span>對 ${hit} 名周圍敵人造成 ${dmg} 點傷害。`,'player-special');
    window._d2rExplosionGuard=true;try{dead.sort((a,b)=>b-a).forEach(i=>{if(mapState.mobs[i]&&!mapState.mobs[i]._dead)killMob(i);});}finally{window._d2rExplosionGuard=false;}
}
function d2rGameplayTotals(owner) {
    let out={sp:0,fh:0};if(!owner||!owner.eq)return out;
    Object.values(owner.eq).filter(Boolean).forEach(it=>{let mult=d2rDurabilityMult(it)*d2rEnhanceAffixMult(it);d2rAffixRows(it).forEach(r=>{if(r[0]==='sp'||r[0]==='fh')out[r[0]]+=r[1]*mult;});});
    out.sp=Math.min(30,out.sp);out.fh=Math.min(30,out.fh);return out;
}
function d2rGameplayMainDamage(owner,dmg) {
    let t=d2rGameplayTotals(owner), hp=owner===player?owner.hp:owner.curHp;
    if(t.fh&&hp>=(owner.mhp||1))dmg=(Number(dmg)||0)*(1+t.fh/100);
    return Math.max(1,Math.floor(dmg));
}
function d2rGameplaySplash(owner,target,mainDmg,isRanged) {
    if(isRanged||!owner||!target||window._d2rSplashGuard)return;
    let pct=d2rGameplayTotals(owner).sp;if(!pct)return;
    let dmg=Math.max(1,Math.floor((Number(mainDmg)||0)*pct/100)), hit=0, dead=[];
    mapState.mobs.forEach((m,i)=>{if(hit>=2||!m||m===target||m._dead||m.curHp<=0)return;hit++;m.curHp-=dmg;m.justHit='phys';mobWake(m);if(m.curHp<=0)dead.push(i);});
    if(hit)logCombat(`<span class="text-red-300 font-bold">【近戰濺射】</span>波及 ${hit} 名敵人，各造成 ${dmg} 點傷害。`,'player-special',owner===player?'player':'mercenary');
    // 濺射擊殺正常結算怪物獎勵，但禁止引發擊殺爆炸與再次濺射。
    window._d2rSplashGuard=true;window._d2rExplosionGuard=true;
    try{dead.sort((a,b)=>b-a).forEach(i=>{if(mapState.mobs[i]&&!mapState.mobs[i]._dead)killMob(i);});}
    finally{window._d2rExplosionGuard=false;window._d2rSplashGuard=false;}
}
function d2rDurabilityMax(item) {
    let d = item && DB.items[item.id];
    if (!d || !d2rAffixRows(item).length || isRelic(d)) return 0;
    return d.type === 'wpn' ? 100 : (d.type === 'arm' ? 120 : 0);
}
function d2rDurabilityNow(item) {
    let max = d2rDurabilityMax(item);
    return max ? Math.max(0, Math.min(max, item.dur == null ? max : Math.floor(Number(item.dur) || 0))) : 0;
}
function d2rDurabilityMult(item) {
    let max = d2rDurabilityMax(item);
    if (!max) return 1;
    let now = d2rDurabilityNow(item);
    return now <= 0 ? 0.7 : (now < max * 0.25 ? 0.9 : 1);
}
// 💎 開孔第一階段：sockets 的陣列長度＝已開孔數；元素暫為 null，未來依順序保存寶石／符文實體。
// socketMax 為該實體可開上限，與已開孔數分離，避免未來改版時舊裝備的上限被資料表變動回溯改寫。
function equipSocketLimit(item) {
    let d = item && item.id ? DB.items[item.id] : item;
    if (!d || isRelic(d) || d.isArrow || d.doll || d.virtual) return 0;
    if (d.type === 'wpn') return 6;
    if (d.type !== 'arm') return 0;
    if (d.slot === 'armor') return 6;
    if (d.slot === 'helm' || d.slot === 'shield') return 4;
    if (d.slot === 'gloves' || d.slot === 'boots' || d.slot === 'cloak') return 2;
    return 0;
}
function equipSocketMax(item) {
    let cap = equipSocketLimit(item);
    if (!cap) return 0;
    return Math.max(0, Math.min(cap, Math.floor(Number(item && item.socketMax) || cap)));
}
function equipSocketRows(item) {
    if (!item || !Array.isArray(item.sockets)) return [];
    return item.sockets.slice(0, equipSocketMax(item)).map(x => x == null ? null : x);
}
function equipSocketOpenCount(item) { return equipSocketRows(item).length; }
function equipSocketSignature(item) {
    return equipSocketRows(item).map(x => {
        if (x == null) return '_';
        if (typeof x === 'string') return x;
        return [x.kind || '', x.id || '', x.uid || '', x.rank || 0].join(':');
    }).join(',');
}
function d2rRollSockets(def, quality, options) {
    options = options || {};
    let cap = equipSocketLimit(def), qi = D2R_QUALITY_KEYS.indexOf(quality), white=quality==='white';
    if (!cap || (qi < 0&&!white)) return null;
    let rates = [.05,.08,.12,.16,.20,.25,.30,.40];
    let chance = (white?.30:rates[qi]) * (options.boss ? 1.5 : 1) * (Number.isFinite(options.chanceMult) ? Math.max(0,options.chanceMult) : 1);
    if (lootRng('d2socket-chance') >= Math.min(.95,chance)) return null;
    // 高孔極稀有：60%/22%/10%/5%/2%/1%，並受裝備部位上限裁切。
    let roll=lootRng('d2socket-count'), count=roll<.60?1:(roll<.82?2:(roll<.92?3:(roll<.97?4:(roll<.99?5:6))));
    count=Math.min(cap,count);
    return { socketMax:cap, sockets:Array(count).fill(null) };
}
function rollGemDropForMob(mob) {
    if(!mob||mob.race==='建築')return null;
    let boss=!!mob.boss, rates=boss?[.12,.04,.005]:[.006,.0015,0], roll=lootRng('gem-drop'), rank=0;
    if(roll<rates[2])rank=3;
    else if(roll<rates[2]+rates[1])rank=2;
    else if(roll<rates[2]+rates[1]+rates[0])rank=1;
    if(!rank)return null;
    let lv=Math.max(1,Number(mob.lv)||1),upgrade=lv>=80?.20:(lv>=60?.10:0);
    if(rank<3&&lootRng('gem-level-up')<upgrade)rank++;
    let colors=Object.keys(GEM_COLORS),color=colors[Math.floor(lootRng('gem-color')*colors.length)],id=`gem_${color}_${rank}`;
    gainItem(id,1,true,true);
    logSys(`<span class="text-fuchsia-300 font-bold">💎 ${mob.n} 掉落了 ${DB.items[id].n}！</span>`);
    return id;
}
function d2rEquipRequirement(item) {
    let rows = d2rAffixRows(item), qi = D2R_QUALITY_KEYS.indexOf(item && item.d2q);
    if (!rows.length || qi < 0) return null;
    // v3.8.89：全面降低八色裝備需求，讓流派在中期成形；品質差距、T1 加成與轉生減免仍保留。
    let best = Math.min(...rows.map(r => r[2])), tierAdd = ({1:8,2:5,3:3,4:1,5:0})[best] || 0;
    let levels = [1,10,18,26,34,42,50,60], attrs = [10,12,15,18,22,26,30,35];
    let level = Math.max(1, levels[qi] + tierAdd - Math.max(0, Number(player && player.rebirthCount) || 0) * 5);
    let d = DB.items[item.id], stat = 'str';
    if (d && d.type === 'wpn') stat = d.ranged ? 'dex' : ((d.mdmg || d.magic) ? 'int' : 'str');
    else if (d && d.type === 'arm') stat = (d.slot === 'armor' || d.slot === 'shield') ? 'con' : 'dex';
    else if (d && d.type === 'acc') stat = (d.mdmg || d.mmp || d.mpR) ? 'wis' : 'cha';
    return { level, stat, value:attrs[qi] };
}
function d2rRequirementStatus(item, owner) {
    let req = d2rEquipRequirement(item), p = owner || player;
    if (!req) return {ok:true, req:null};
    let value = Number(p && p.d && p.d[req.stat]);
    if (!Number.isFinite(value)) value = (Number(p.base && p.base[req.stat])||0) + (Number(p.alloc && p.alloc[req.stat])||0) + (Number(p.panacea && p.panacea[req.stat])||0) + (Number(p.rebirthBonus && p.rebirthBonus[req.stat])||0);
    return {ok:(Number(p.lv)||1) >= req.level && value >= req.value, req, current:value};
}
function d2rRollTier(level, qualityIndex) {
    let best = level >= 80 ? 1 : (level >= 60 ? 2 : (level >= 40 ? 3 : (level >= 20 ? 4 : 5)));
    best = Math.max(1, best - Math.floor((qualityIndex || 0) / 2));
    let span = 6 - best, roll = Math.floor(lootRng('d2tier') * span);
    return Math.min(5, best + roll);
}
function d2rEligibleAffixCodes(def, qualityIndex) {
    let common=['str','dex','con','int','wis','cha','hp','mp','mf','gf','xf','nd','bd','kh','km'];
    let magicWpn=!!(def&&(def.mdmg||def.magic||def.qigu||(typeof isWandWeapon==='function'&&isWandWeapon(def))));
    let pool=def&&def.type==='wpn'
        ?common.concat(def.ranged?['rd','rh','rc','rcd']:(magicWpn?['mg','gh','gc','gcd']:['md','mh','mc','mcd']))
        :common.concat(['rf','rw','re','ra','rn','hpr','mpr','wt','ac','mr','dr','abr','pot'],
            (def&&(def.slot==='boots'||def.slot==='gloves'||def.slot==='cloak'))?['er']:[],
    ...
    let warriorGear=def&&def.type!=='wpn'&&((def.type==='arm'&&req.split(',').includes('warrior'))||def.type==='acc');
    if(qualityIndex>=2&&warriorGear)pool.push('ww','wd','wr','wc','ld','ls','lc','lr');
    let royalGear=def&&def.type!=='wpn'&&((def.type==='arm'&&req.split(',').includes('royal'))||def.type==='acc');
    if(qualityIndex>=2&&royalGear)pool.push('bh','bn','bt','bc','qmd','qmn','qmt','qmc','kvd','kvn','kvp','kvc');
    let darkGear=def&&def.type!=='wpn'&&((def.type==='arm'&&req.split(',').includes('dark'))||def.type==='acc');
    if(qualityIndex>=2&&darkGear)pool.push('shd','shn','sht','shc','mzd','mzn','mzt','mzc','trd','trn','trt','trc');
    let knightGear=def&&def.type!=='wpn'&&((def.type==='arm'&&req.split(',').includes('knight'))||def.type==='acc');
    if(qualityIndex>=2&&knightGear)pool.push('vwd','vwn','vws','vwc','srd','srb','srg','src','ufd','ufs','ufg','ufc');
    let dragonGear=def&&def.type!=='wpn'&&((def.type==='arm'&&req.split(',').includes('dragon'))||def.type==='acc');
    if(qualityIndex>=2&&dragonGear)pool.push('fdd','fdn','fdf','fdc','bfd','bfn','bft','bfc','dld','dln','dlt','dlc');
    let elfGear=def&&def.type!=='wpn'&&((def.type==='arm'&&req.split(',').includes('elf'))||def.type==='acc');
    if(qualityIndex>=2&&elfGear)pool.push('tjd','tjn','tjs','tjc','ard','arn','arc','acd','csd','csn','csp','csc');
    let illusionGear=def&&def.type!=='wpn'&&((def.type==='arm'&&req.split(',').includes('illusion'))||def.type==='acc');
    if(qualityIndex>=2&&illusionGear)pool.push('med','men','mes','mec','rbd','rbn','rbf','rbc');
    // 卓越以上的新流派詞綴：穿透限武器／飾品，異常增傷限武器／手套，頭目減傷限盔甲／盾牌。
    if(qualityIndex>=2&&(def.type==='wpn'||def.type==='acc'))pool.push('fpen','wpen','epen','apen');
    if(qualityIndex>=2&&(def.type==='wpn'||(def.type==='arm'&&def.slot==='gloves')))pool.push('sdm');
    if(qualityIndex>=2&&def.type==='arm'&&(def.slot==='armor'||def.slot==='shield'))pool.push('bdr');
    // 第二批：族群獵殺／先制限武器與飾品；破硬皮／處決限武器與手套。
    if(qualityIndex>=2&&(def.type==='wpn'||def.type==='acc'))pool.push('und','dem','dra','opn');
    if(qualityIndex>=2&&(def.type==='wpn'||(def.type==='arm'&&def.slot==='gloves')))pool.push('hsk','exe');
    // 第三批防禦詞綴：依防具部位分流，避免所有減傷集中在單一部位。
    if(qualityIndex>=2&&def.type==='arm'&&(def.slot==='armor'||def.slot==='shield'))pool.push('pdr','ldr');
    if(qualityIndex>=2&&((def.type==='arm'&&def.slot==='cloak')||def.type==='acc'))pool.push('mdr');
    if(qualityIndex>=2&&((def.type==='arm'&&def.slot==='helm')||def.type==='acc'))pool.push('adr');
    if(qualityIndex>=2&&def.type==='arm'&&(def.slot==='cloak'||def.slot==='shield'))pool.push('udr','ddr');
    // 第四批受擊反應詞綴：盾與手套偏格擋／反擊，胸甲與飾品偏續戰，頭盔與斗篷偏魔力防護。
    if(qualityIndex>=2&&def.type==='arm'&&(def.slot==='shield'||def.slot==='gloves'))pool.push('blk');
    if(qualityIndex>=2&&((def.type==='arm'&&(def.slot==='helm'||def.slot==='cloak'))||def.type==='acc'))pool.push('mgd');
    if(qualityIndex>=2&&((def.type==='arm'&&(def.slot==='armor'||def.slot==='boots'))||def.type==='acc'))pool.push('rcv');
    if(qualityIndex>=2&&def.type==='arm'&&(def.slot==='armor'||def.slot==='shield'))pool.push('sav','bar');
    if(qualityIndex>=2&&((def.type==='arm'&&(def.slot==='helm'||def.slot==='gloves'))||def.type==='acc'))pool.push('rip');
    // 第五批資源／擊殺節奏詞綴：武器偏條件輸出，防具與飾品負責護盾、回春及連殺戰意。
    if(qualityIndex>=2&&(def.type==='wpn'||(def.type==='arm'&&def.slot==='gloves')))pool.push('ber');
    if(qualityIndex>=2&&(def.type==='wpn'||(def.type==='arm'&&def.slot==='helm')||def.type==='acc'))pool.push('fmp');
    if(qualityIndex>=2&&(def.type==='wpn'||(def.type==='arm'&&def.slot==='shield')))pool.push('shp');
    ...
    // 第七批元素連攜詞綴：需同時命中指定元素與狀態，比單一追擊更專門。
    if(qualityIndex>=2&&(def.type==='wpn'||(def.type==='arm'&&def.slot==='gloves')))pool.push('fbr');
    if(qualityIndex>=2&&(def.type==='wpn'||def.type==='acc'))pool.push('wfr');
    if(qualityIndex>=2&&(def.type==='wpn'||(def.type==='arm'&&def.slot==='cloak')))pool.push('eps');
    if(qualityIndex>=2&&(def.type==='wpn'||(def.type==='arm'&&def.slot==='boots')))pool.push('wbl');
    // 第八批配套裝備技能：各刻印能自行製造元素連攜所需的狀態。
    if(qualityIndex>=2&&(def.type==='wpn'||(def.type==='arm'&&def.slot==='gloves')))pool.push('fsi');
    if(qualityIndex>=2&&(def.type==='wpn'||def.type==='acc'))pool.push('wsi');
    if(qualityIndex>=2&&(def.type==='wpn'||(def.type==='arm'&&def.slot==='cloak')))pool.push('esi');
    if(qualityIndex>=2&&(def.type==='wpn'||(def.type==='arm'&&def.slot==='boots')))pool.push('asi');
    return pool;
}
function d2rRerollSingleAffix(item,index,mode) {
    let rows=d2rAffixRows(item),qi=D2R_QUALITY_KEYS.indexOf(item&&item.d2q),def=item&&DB.items[item.id];
    index=Math.floor(Number(index));if(!def||qi<0||index<0||index>=rows.length)return false;
    ...
    if(mode==='type'){
        ...
        ['qmd','qmn','qmt','qmc','srd','srb','srg','src','bfd','bfn','bft','bfc','kvd','kvn','kvp','kvc','ufd','ufs','ufg','ufc','dld','dln','dlt','dlc'].forEach(c=>{skill.add(c);special.add(c)});
    ...
    let value=range[0]+Math.floor(lootRng('d2-enchant-value')*(range[1]-range[0]+1));
    item.d2=rows.map((r,i)=>i===index?[code,value,tier]:r);item.d2q=D2R_QUALITY_KEYS[qi];
    return true;
}
function d2rRollAffixes(def, options) {
    options = options || {};
    if (!def || (!_lootMobInfo && !options.quality)) return null;
    if (isRelic(def) || def.noAffix || def.isArrow || !['wpn','arm','acc'].includes(def.type)) return null;
    if(!options.quality&&_lootMobInfo&&lootRng('d2-white-base')<.15)return null;
    let mf = Math.max(0, Number(player && player.d && player.d.d2MagicFind) || 0);
    let effectiveMf = options.ignoreMf ? 0 : mf * 250 / (mf + 250), boss = !!(_lootMobInfo && _lootMobInfo.boss);
    // 八色品質：物品發現率以遞減收益偏移至較高品質，但不增加裝備本身掉落率。
    let weights = (boss ? [30,30,18,10,6,3,2,1] : [55,25,10,5,2.5,1.5,0.7,0.3])
        .map((w, i) => w * (1 + effectiveMf / 100 * i / 2));
    let total = weights.reduce((s,w) => s+w, 0), qr = lootRng('d2quality') * total, qi = 0;
    for (; qi < weights.length - 1 && qr >= weights[qi]; qi++) qr -= weights[qi];
    let quality = options.quality && D2R_QUALITY[options.quality] ? options.quality : D2R_QUALITY_KEYS[qi];
    ...
    ['qmd','qmn','qmt','qmc','srd','srb','srg','src','bfd','bfn','bft','bfc','kvd','kvn','kvp','kvc','ufd','ufs','ufg','ufc','dld','dln','dlt','dlc'].forEach(c=>{skillCodes.add(c);specialCodes.add(c)});
    const gameplayCodes = new Set(['sp','fh']);
    let specialMax = [0,0,1,1,2,2,3,3][qi], specialCount = 0;
    let qdef = d2rQualityDef(quality);
    let count = qdef.min + Math.floor(lootRng('d2count') * (qdef.max - qdef.min + 1)), t1count = 0;
    let rows = [];
    while (rows.length < count && pool.length) {
        ...
        if (tier === 1) t1count++;
        let range = D2R_AFFIX_RANGES[code][tier - 1], value = range[0] + Math.floor(lootRng('d2value') * (range[1] - range[0] + 1));
        rows.push([code, value, tier]);
    }
    return { quality, rows };
}
function d2rAffixText(row) {
    ...
    if(r[0]==='ts')return `受擊護盾：受傷 5% 獲得最大 HP ${r[1]}% 護盾`;
    if(r[0]==='pi')return `完美一擊：一般攻擊有機率額外造成 ${r[1]}% 傷害`;
    if(r[0]==='ks')return `擊殺加速：擊殺後攻擊速度 +${r[1]}%，持續 5 秒`;
    if(r[0]==='sp')return `近戰濺射：一般近戰攻擊波及兩名敵人，造成主傷害 ${r[1]}%`;
    if(r[0]==='fh')return `滿血獵手：HP 全滿時一般攻擊傷害 +${r[1]}%`;
    if(['fpen','wpen','epen','apen'].includes(r[0]))return `${D2R_AFFIX_LABEL[r[0]]} +${r[1]}%（對應屬性最終傷害提高）`;
    if(r[0]==='sdm')return `異常增傷：攻擊帶有異常狀態的敵人時傷害 +${r[1]}%`;
    if(r[0]==='bdr')return `頭目減傷：受到頭目傷害 -${r[1]}%`;
    if(r[0]==='und')return `不死獵殺：對不死族傷害 +${r[1]}%`;
    if(r[0]==='dem')return `惡魔獵殺：對惡魔族傷害 +${r[1]}%`;
    if(r[0]==='dra')return `屠龍：對龍族傷害 +${r[1]}%`;
    if(r[0]==='hsk')return `破硬皮：對硬皮敵人傷害 +${r[1]}%`;
    if(r[0]==='opn')return `先制增傷：目標 HP 90% 以上時傷害 +${r[1]}%`;
    if(r[0]==='exe')return `處決增傷：目標 HP 30% 以下時傷害 +${r[1]}%`;
    if(r[0]==='pdr')return `物理減傷：受到物理傷害 -${r[1]}%`;
    if(r[0]==='mdr')return `魔法減傷：受到魔法傷害 -${r[1]}%`;
    if(r[0]==='ldr')return `瀕死減傷：自身 HP 35% 以下時受到傷害 -${r[1]}%`;
    if(r[0]==='adr')return `異常減傷：自身帶有異常狀態時受到傷害 -${r[1]}%`;
    if(r[0]==='udr')return `不死減傷：受到不死族敵人傷害 -${r[1]}%`;
    if(r[0]==='ddr')return `惡魔減傷：受到惡魔族敵人傷害 -${r[1]}%`;
    if(r[0]==='blk')return `堅守格擋：受傷時 ${r[1]}% 機率擋下 50% 傷害`;
    if(r[0]==='mgd')return `魔力護體：以 MP 吸收所受傷害的 ${r[1]}%`;
    if(r[0]==='rcv')return `傷勢回流：受傷後恢復實際傷害 ${r[1]}% 的 HP`;
    if(r[0]==='sav')return `致命守護：致命傷時保留 1 HP 並獲得最大 HP ${r[1]}% 護盾（60 秒冷卻）`;
    if(r[0]==='bar')return `傷勢壁壘：受傷後獲得該次傷害 ${r[1]}% 護盾（5 秒冷卻，最多最大 HP 15%）`;
    if(r[0]==='rip')return `反擊意志：受傷後傷害 +${r[1]}%，持續 5 秒`;
    if(r[0]==='ber')return `浴血增傷：自身 HP 35% 以下時傷害 +${r[1]}%`;
    if(r[0]==='fmp')return `滿魔增傷：自身 MP 90% 以上時傷害 +${r[1]}%`;
    if(r[0]==='shp')return `護盾猛攻：擁有八色詞綴護盾時傷害 +${r[1]}%`;
    ...
    if(r[0]==='fbr')return `烈焰催化：火屬性命中燃燒敵人時傷害 +${r[1]}%`;
    if(r[0]==='wfr')return `霜潮共鳴：水屬性命中冰凍敵人時傷害 +${r[1]}%`;
    if(r[0]==='eps')return `腐土共生：地屬性命中中毒敵人時傷害 +${r[1]}%`;
    if(r[0]==='wbl')return `風刃放血：風屬性命中流血敵人時傷害 +${r[1]}%`;
    if(r[0]==='fsi')return `裝備技能・熾焰刻印：火屬性傷害命中時 ${r[1]}% 機率附加灼燒 5 秒`;
    if(r[0]==='wsi')return `裝備技能・寒潮刻印：水屬性傷害命中時 ${r[1]}% 機率冰凍 2 秒（頭目無效）`;
    if(r[0]==='esi')return `裝備技能・腐毒刻印：地屬性傷害命中時 ${r[1]}% 機率附加中毒 6 秒`;
    if(r[0]==='asi')return `裝備技能・裂風刻印：風屬性傷害命中時 ${r[1]}% 機率附加流血 8 秒`;
    if(r[0]==='hy')return `九頭蛇傷害 +${r[1]}%`;
    if(r[0]==='hd')return `九頭蛇持續時間 +${r[1]}%`;
    if(r[0]==='st')return `靜電立場削減效果 +${r[1]}%`;
    if(r[0]==='os')return `冰封球每次發射冰片 +${r[1]}`;
    if(r[0]==='cb')return `連鎖雷光彈跳次數 +${r[1]}`;
    if(r[0]==='mcx')return `隕星數量 +${r[1]}`;
    if(r[0]==='gd')return `燃燒地面持續時間 +${r[1]}%`;
    if(r[0]==='scd')return `延伸技能冷卻時間 -${r[1]}%`;
    if(r[0]==='ww')return `炫風斬傷害 +${r[1]}%`;
    if(r[0]==='wd')return `炫風斬持續時間 +${r[1]}%`;
    if(r[0]==='wr')return `一般攻擊怒氣取得 +${r[1]}%`;
    if(r[0]==='wc')return `炫風斬冷卻時間 -${r[1]}%`;
    if(r[0]==='ld')return `躍擊傷害 +${r[1]}%`;
    if(r[0]==='ls')return `躍擊暈眩時間 +${r[1]}%`;
    if(r[0]==='lc')return `躍擊冷卻時間 -${r[1]}%`;
    if(r[0]==='lr')return `躍擊怒氣消耗 -${r[1]}%`;
    if(r[0]==='bh')return `祝福之鎚傷害 +${r[1]}%`;
    if(r[0]==='bn')return `祝福之鎚數量 +${r[1]}`;
    if(r[0]==='bt')return `祝福之鎚持續時間 +${r[1]}%`;
    if(r[0]==='bc')return `祝福之鎚冷卻時間 -${r[1]}%`;
    if(r[0]==='shd')return `暗影分身傷害 +${r[1]}%`;
    if(r[0]==='shn')return `暗影分身數量 +${r[1]}`;
    if(r[0]==='sht')return `暗影分身持續時間 +${r[1]}%`;
    if(r[0]==='shc')return `暗影分身冷卻時間 -${r[1]}%`;
    if(r[0]==='vwd')return `范德震地傷害 +${r[1]}%`;
    if(r[0]==='vwn')return `范德震地目標數量 +${r[1]}`;
    if(r[0]==='vws')return `范德震地暈眩時間 +${r[1]}%`;
    if(r[0]==='vwc')return `范德震地冷卻時間 -${r[1]}%`;
    if(r[0]==='fdd')return `冰龍追擊傷害 +${r[1]}%`;
    if(r[0]==='fdn')return `冰龍追擊目標數量 +${r[1]}`;
    if(r[0]==='fdf')return `冰龍追擊冰凍機率 +${r[1]}%`;
    if(r[0]==='fdc')return `冰龍追擊冷卻時間 -${r[1]}%`;
    if(r[0]==='tjd')return `雷霆標槍連鎖傷害 +${r[1]}%`;
    if(r[0]==='tjn')return `雷霆標槍彈跳次數 +${r[1]}`;
    if(r[0]==='tjs')return `雷霆標槍每次彈跳傷害衰減 -${r[1]}%`;
    if(r[0]==='tjc')return `雷霆標槍冷卻時間 -${r[1]}%`;
    if(r[0]==='med')return `心靈共振傷害 +${r[1]}%`;
    if(r[0]==='men')return `心靈共振目標數量 +${r[1]}`;
    if(r[0]==='mes')return `心靈共振每次擴散傷害衰減 -${r[1]}%`;
    if(r[0]==='mec')return `心靈共振冷卻時間 -${r[1]}%`;
    if(r[0]==='rbd')return `裂界衝擊傷害 +${r[1]}%`;
    if(r[0]==='rbn')return `裂界衝擊目標數量 +${r[1]}`;
    if(r[0]==='rbf')return `裂界衝擊暈眩機率 +${r[1]}%`;
    if(r[0]==='rbc')return `裂界衝擊冷卻時間 -${r[1]}%`;
    if(r[0]==='ard')return `多重箭雨傷害 +${r[1]}%`;
    if(r[0]==='arn')return `多重箭雨箭矢數量 +${r[1]}`;
    if(r[0]==='arc')return `多重箭雨額外暴擊機率 +${r[1]}%`;
    if(r[0]==='acd')return `多重箭雨冷卻時間 -${r[1]}%`;
    if(r[0]==='csd')return `充能一擊傷害 +${r[1]}%`;
    if(r[0]==='csn')return `充能一擊電束數量 +${r[1]}`;
    if(r[0]==='csp')return `充能一擊風抗穿透 +${r[1]}%`;
    if(r[0]==='csc')return `充能一擊冷卻時間 -${r[1]}%`;
    if(r[0]==='mzd')return `馬賽克終結傷害 +${r[1]}%`;
    if(r[0]==='mzn')return `馬賽克終結目標數量 +${r[1]}`;
    if(r[0]==='mzt')return `馬賽克蓄能保存時間 +${r[1]}%`;
    if(r[0]==='mzc')return `馬賽克終結冷卻時間 -${r[1]}%`;
    if(r[0]==='trd')return `雷光哨衛傷害 +${r[1]}%`;
    if(r[0]==='trn')return `雷光哨衛電束數量 +${r[1]}`;
    if(r[0]==='trt')return `雷光哨衛持續時間 +${r[1]}%`;
    if(r[0]==='trc')return `雷光哨衛冷卻時間 -${r[1]}%`;
    if(r[0]==='qmd')return `王旗號令傷害 +${r[1]}%`;
    if(r[0]==='qmn')return `王旗額外號令數量 +${r[1]}`;
    if(r[0]==='qmt')return `王旗號令持續時間 +${r[1]}%`;
    if(r[0]==='qmc')return `王旗號令間隔 -${r[1]}%`;
    if(r[0]==='srd')return `盾反傷害 +${r[1]}%`;
    if(r[0]==='srb')return `盾反格擋機率 +${r[1]}%`;
    if(r[0]==='srg')return `盾反格擋幅度 +${r[1]}%`;
    if(r[0]==='src')return `盾反間隔 -${r[1]}%`;
    if(r[0]==='bfd')return `火龍化身傷害 +${r[1]}%`;
    if(r[0]==='bfn')return `火龍灼燒時間 +${r[1]}秒`;
    if(r[0]==='bft')return `火龍化身持續時間 +${r[1]}%`;
    if(r[0]==='bfc')return `火龍攻擊間隔 -${r[1]}%`;
    if(r[0]==='kvd')return `王者劍氣傷害 +${r[1]}%`;
    if(r[0]==='kvn')return `王者劍氣數量 +${r[1]}`;
    if(r[0]==='kvp')return `王者劍氣機率 +${r[1]}%`;
    if(r[0]==='kvc')return `王者劍氣間隔 -${r[1]}%`;
    if(r[0]==='ufd')return `不屈震波傷害 +${r[1]}%`;
    if(r[0]==='ufs')return `不屈堡壘所需受擊 -${r[1]}次`;
    if(r[0]==='ufg')return `不屈堡壘護盾 +${r[1]}%最大生命`;
    if(r[0]==='ufc')return `不屈堡壘冷卻時間 -${r[1]}%`;
    if(r[0]==='dld')return `雷龍風暴傷害 +${r[1]}%`;
    if(r[0]==='dln')return `雷龍風暴電束數量 +${r[1]}`;
    if(r[0]==='dlt')return `雷龍風暴持續時間 +${r[1]}%`;
    if(r[0]==='dlc')return `雷龍風暴間隔 -${r[1]}%`;
    if (r[0] === 'ac') return `AC -${r[1]}`;
    if (['mc','rc','gc','mcd','rcd','gcd','hpp','mpp','pot','abr','gf','xf','nd','bd','fpen','wpen','epen','apen','sdm','bdr','und','dem','dra','hsk','opn','exe','pdr','mdr','ldr','adr','udr','ddr','blk','mgd','rcv','sav','bar','rip','ber','fmp','shp','ksh','krc','kfu','frz','brn','psn','bld','ctl','deb','fbr','wfr','eps','wbl','fsi','wsi','esi','asi'].includes(r[0])) return `${D2R_AFFIX_LABEL[r[0]]} +${r[1]}%`;
    return `${D2R_AFFIX_LABEL[r[0]]} +${r[1]}`;
}
const D2R_NAME_PREFIX = {
    md:'殘暴的', rd:'鷹眼的', mg:'秘法的', mh:'精準的', rh:'神射的', gh:'專注的',
    ...
    fpen:'熔穿的',wpen:'破潮的',epen:'裂地的',apen:'破風的',sdm:'處刑的',bdr:'屹立的',
    und:'驅邪的',dem:'獵魔的',dra:'屠龍的',hsk:'粉碎的',opn:'先鋒的',exe:'終結的',
    ...
    fbr:'催焰的',wfr:'霜潮的',eps:'腐土的',wbl:'風血的',
    fsi:'熾印的',wsi:'寒印的',esi:'毒印的',asi:'風印的',
    hy:'蛇焰的',hd:'長燃的',st:'靜電的',os:'碎冰的',cb:'雷鏈的',mcx:'星落的',gd:'焦土的',scd:'迅詠的',
    ww:'旋刃的',wd:'不息的',wr:'狂怒的',wc:'疾旋的',
    ld:'震地的',ls:'鎮壓的',lc:'飛躍的',lr:'輕躍的',
    bh:'聖鎚的',bn:'多重聖鎚的',bt:'長禱的',bc:'迅禱的',
    shd:'影襲的',shn:'影群的',sht:'長影的',shc:'迅影的',
    vwd:'裂地的',vwn:'廣震的',vws:'鎮魂的',vwc:'迅震的',
    fdd:'冰龍的',fdn:'群襲的',fdf:'霜縛的',fdc:'迅霜的',
    tjd:'雷霆的',tjn:'躍雷的',tjs:'不衰的',tjc:'迅雷的',
    med:'共鳴的',men:'群心的',mes:'不衰心念的',mec:'迅念的',
    rbd:'裂界的',rbn:'廣裂的',rbf:'震魂的',rbc:'迅裂的',
    ard:'箭雨的',arn:'萬箭的',arc:'穿心的',acd:'迅弦的',
    csd:'充能的',csn:'裂電的',csp:'破風的',csc:'迅電的',
    mzd:'萬象的',mzn:'廣域的',mzt:'凝息的',mzc:'迅終的',
    trd:'雷哨的',trn:'裂電的',trt:'長鳴的',trc:'迅哨的',
    qmd:'統御的',qmn:'萬軍的',qmt:'長令的',qmc:'疾令的',
    srd:'盾反的',srb:'鏡守的',srg:'壁壘的',src:'迅反的',
    bfd:'焚龍的',bfn:'長燃的',bft:'龍化的',bfc:'迅焰的',
    kvd:'霸劍的',kvn:'萬刃的',kvp:'勇決的',kvc:'迅刃的',
    ufd:'不屈的',ufs:'蓄勢的',ufg:'鋼心的',ufc:'迅壁的',
    dld:'雷龍的',dln:'裂空的',dlt:'長雷的',dlc:'迅霆的'
};
const D2R_NAME_SUFFIX = {
    str:'之巨力', dex:'之敏捷', con:'之活力', int:'之智慧', wis:'之精神', cha:'之魅力',
    ...
    fpen:'之熔穿',wpen:'之破潮',epen:'之裂地',apen:'之破風',sdm:'之處刑',bdr:'之屹立',
    und:'之淨化',dem:'之誅魔',dra:'之龍殤',hsk:'之破壁',opn:'之先制',exe:'之斷命',
    ...
    fbr:'之催焰',wfr:'之霜潮',eps:'之腐土',wbl:'之風血',
    fsi:'之熾焰刻印',wsi:'之寒潮刻印',esi:'之腐毒刻印',asi:'之裂風刻印',
    hy:'之九頭蛇',hd:'之蛇群長駐',st:'之靜電',os:'之冰片',cb:'之雷鏈',mcx:'之星雨',gd:'之焦土',scd:'之迅詠',
    ww:'之炫風',wd:'之不息旋舞',wr:'之狂怒',wc:'之疾旋',
    ld:'之震地',ls:'之鎮壓',lc:'之飛躍',lr:'之輕躍',
    bh:'之祝福之鎚',bn:'之鎚陣',bt:'之長禱',bc:'之迅禱',
    shd:'之影襲',shn:'之影群',sht:'之長影',shc:'之迅影',
    vwd:'之裂地',vwn:'之廣震',vws:'之鎮魂',vwc:'之迅震',
    fdd:'之冰龍',fdn:'之群襲',fdf:'之霜縛',fdc:'之迅霜',
    tjd:'之雷霆標槍',tjn:'之躍雷',tjs:'之不衰電流',tjc:'之迅雷',
    med:'之心靈共振',men:'之群心',mes:'之不衰心念',mec:'之迅念',
    rbd:'之裂界衝擊',rbn:'之廣裂',rbf:'之震魂',rbc:'之迅裂',
    ard:'之箭雨',arn:'之萬箭',arc:'之穿心',acd:'之迅弦',
    csd:'之充能一擊',csn:'之裂電',csp:'之破風',csc:'之迅電',
    mzd:'之馬賽克',mzn:'之廣域終結',mzt:'之凝息',mzc:'之迅終',
    trd:'之雷光哨衛',trn:'之多重電束',trt:'之長鳴',trc:'之迅哨',
    qmd:'之王旗號令',qmn:'之萬軍',qmt:'之長令',qmc:'之疾令',
    srd:'之盾反',srb:'之鏡守',srg:'之壁壘',src:'之迅反',
    bfd:'之焚龍',bfn:'之長燃',bft:'之火龍化身',bfc:'之迅焰',
    kvd:'之王者劍氣',kvn:'之萬刃',kvp:'之勇決',kvc:'之迅刃',
    ufd:'之不屈震波',ufs:'之蓄勢',ufg:'之鋼心',ufc:'之迅壁',
    dld:'之雷龍風暴',dln:'之裂空雷束',dlt:'之長雷',dlc:'之迅霆'
};
...
function d2rNameAffixes(item) {
    ...
    let pre = rows.find(x => D2R_PREFIX_PREFERRED.has(x.r[0])) || rows[0];
    if (rows.length <= 2) return { prefix:D2R_NAME_PREFIX[pre.r[0]] || '', suffix:'' };
    let suf = rows.find(x => x !== pre && D2R_SUFFIX_PREFERRED.has(x.r[0])) || rows.find(x => x !== pre);
    return { prefix:D2R_NAME_PREFIX[pre.r[0]] || '', suffix:suf ? (D2R_NAME_SUFFIX[suf.r[0]] || '') : '' };
}
function itemSig(it) { let _ams = Math.max(1, Math.min(3, Math.floor(Number(it.attrMagicStar) || 1))), _d2 = d2rAffixRows(it).map(r => r.join(':')).join(','), _dur = d2rDurabilityMax(it) ? d2rDurabilityNow(it) : '', _sock = typeof equipSocketSignature === 'function' ? equipSocketSignature(it) : ''; return it.id + '|' + (it.en || 0) + '|' + (it.bless === true ? 'B' : (it.bless ? 'C' : 0)) + '|' + (it.anc === true ? 'A' : (it.anc || 0)) + '|' + (it.attr || '') + '|' + (it.seteff || '') + (it.attrMagic ? '|' + it.attrMagic + (_ams > 1 ? '@' + _ams : '') : '') + '|D2:' + (it.d2q || '') + ':' + _d2 + '|DC:' + (Math.max(0,Math.floor(Number(it.d2CraftCount)||0))) + '|DUR:' + _dur + '|GP:' + (Number(it.gamblePaid)||0) + '|SOCK:' + (Number(it.socketMax)||0) + ':' + _sock; }   // 改造次數、孔數與孔內順序進簽章，避免不同培養成本的裝備被合併。
...
const CLASS_SKILL_REQ_FIELD = Object.freeze({
    royal:'reqRoy', knight:'reqK', mage:'reqM', elf:'reqE',
    dark:'reqD', illusion:'reqI', dragon:'reqDk', warrior:'reqW'
});
function isOriginalClassSkill(sk, owner, skId) {
    owner = owner || player;
    if (!sk || !owner) return false;
    let origins = Array.isArray(sk.originClasses) ? sk.originClasses : [];
    if (origins.includes(owner.cls)) return true;
    let field = CLASS_SKILL_REQ_FIELD[owner.cls];
    return !!(field && sk[field] !== undefined);
}
function classSkillEquipMult(sk, owner, skId) {
    owner = owner || player;
    if (!isOriginalClassSkill(sk, owner, skId) || !owner.eq) return 1;
    let mult = 1;
...
let player = {
    ...
    base: { str:0, dex:0, con:0, int:0, wis:0, cha:8 }, bonus: 0, alloc: { str:0, dex:0, con:0, int:0, wis:0, cha:0 }, panacea: { str:0, dex:0, con:0, int:0, wis:0, cha:0 }, panaceaUsed: 0, junkPrefs: {}, buildFlowFocus: {}, buildFlowFocusManual: {}, bloodPledge: null, magicShieldCd: 0, lastMapByCat: {}, tracking: null, sherineWorld: false, sherineMad: false, sherineCrystalPity: { world:0, mad:0 }, masteryQuest: null, mastery: null, masteryChangeCnt: 0, siege: { active:false, city:'kent', gateKilled:false, towerKilled:false, endTime:0, kills:0, result:null, cooldownUntil:0, accCdUntil:0 }, dragonWar: null,
...
// 📱 外網手機偶爾會在大量素材同時載入時留下破圖；物品圖示採分段重試並加查詢碼避開失敗快取。
function retryIconImage(img) {
    if (!img) return;
    let attempt = Math.max(0, parseInt(img.dataset.iconRetry || '0', 10) || 0);
    if (!img.dataset.iconOriginal) {
        img.dataset.iconOriginal = img.getAttribute('src') || '';
        img.dataset.iconOpacity = img.style.opacity || '';
    }
    if (attempt >= 3 || !img.dataset.iconOriginal) {
        img.style.opacity = '0';
        if (img.parentElement) img.parentElement.classList.add('icon-load-failed');
        return;
    }
    attempt += 1;
    img.dataset.iconRetry = String(attempt);
    img.style.opacity = '0';
    const delay = [350, 1200, 3000][attempt - 1];
    setTimeout(function () {
        if (!img.isConnected) return;
        const original = img.dataset.iconOriginal;
        const separator = original.includes('?') ? '&' : '?';
        img.onload = function () {
            img.style.opacity = img.dataset.iconOpacity || '';
            if (img.parentElement) img.parentElement.classList.remove('icon-load-failed');
        };
        img.src = original + separator + 'icon_retry=' + attempt + '_' + Date.now();
    }, delay);
}
...
function getStrMeleeDmg(str) {
    if (str > 100) return 57 + Math.floor((str - 100) / 2);
...
function getStrMeleeCrit(str) {
    if (str > 100) return 14 + Math.floor((str - 100) / 5);
    if (str <= 39) return 0;
    if (str <= 44) return 1;
    if (str <= 49) return 2;
    if (str <= 59) return 3;
...
function getDexRangedDmg(dex) {
    if (dex > 100) return 47 + Math.floor((dex - 100) / 2);
...
function getDexRangedCrit(dex) {
    if (dex > 100) return 12 + Math.floor((dex - 100) / 5);
    if (dex <= 39) return 0;
    if (dex <= 44) return 1;
    if (dex <= 49) return 2;
    if (dex <= 59) return 3;
...
function getDexAC(dex) {
    if (dex > 100) return -32 - Math.floor((dex - 100) / 3);
    // 回傳 AC 變化量（負值代表防禦提升）
    return lookupStep(dex, [
...
function getDexER(dex) {
    return Math.floor(dex / 2); // v3.8.40：ER 隨敏捷無限成長
}

// ---------- 智力 INT：魔法傷害 / 魔法命中 / 魔法爆擊率 / 額外魔法點數 / MP消耗減少 ----------
function getIntMagicDmg(int) {
    if (int > 100) return 35 + Math.floor((int - 100) / 2);
...
function getIntMagicHit(int) {
    if (int > 100) return 32 + Math.floor((int - 100) / 3);
    return lookupStep(int, [
...
function getIntMagicCrit(int) {
    if (int > 100) return 15 + Math.floor((int - 100) / 5);
    if (int <= 34) return 0;
    if (int <= 39) return 1;
    if (int <= 44) return 2;
    if (int <= 49) return 4;
...
function getIntMpReduce(int) {
    if (int > 100) return Math.min(80, 30 + Math.floor((int - 100) / 10));
    // MP消耗減少 %
    return lookupStep(int, [
...
function getConGrowth(con, cls) {
    ...
    if (pts > 92) effective += (pts - 92) * 0.03125;                        // CON 101+：持續成長
    return effective * per;
}
function getConHpRegenMax(con) {
    if (con > 100) return 55 + Math.floor((con - 100) / 3);
    if (con < 11) return 0;
...
function getConPotionPct(con) {
    if (con > 100) return 18 + Math.floor((con - 100) / 5);
    // 藥水額外恢復量 %
    if (con <= 19) return 0;
    if (con <= 24) return 1;
    if (con <= 30) return 2;
...
function getWisMpRegen(wis) {
    if (wis > 100) return 34 + Math.floor((wis - 100) / 3);
...
function getWisMR(wis) {
    // 精神7~10 = 0；11 起每精神 +4 MR，無上限
    if (wis <= 10) return 0;
    return (wis - 10) * 4;
}
function getWisBlueBonus(wis) {
    if (wis > 100) return 49 + Math.floor((wis - 100) / 3);
    // 藍色藥水：提升MP自然恢復量
    return lookupStep(wis, [
...
const WEIGHT_COUNT_SLOTS = ['wpn','offwpn','shield','helm','armor','shin','tshirt','cloak','gloves','boots','amulet','ear1','ear2','ring1','ring2','ring3','ring4','belt','eye','charm','pet','doll'];   // 🦵 shin=脛甲（盔甲下方·額外防具）   // ⚔️ offwpn=迅猛雙斧副手武器，計入負重；ear1/ear2=耳環；eye=眼睛；charm=大型護符；doll=魔法娃娃
...
Object.assign(ITEM_WEIGHTS, {"喬丹之石":3});   // 💍 v3.9.2 喬丹之石：傳說唯一戒指
...
function sanitizeState() {
    ...
    // v3.8.39：六項角色屬性與萬能藥累計皆無上限，只排除負值、NaN、Infinity 與小數，避免損壞存檔。
    const statKeys = ['str','dex','con','int','wis','cha'];
    ['base','alloc','panacea'].forEach(group => {
        if (!player[group] || typeof player[group] !== 'object') player[group] = {};
        statKeys.forEach(s => { player[group][s] = Math.max(0, Math.floor(fin(player[group][s], 0))); });
    });
    player.bonus = Math.max(0, Math.floor(fin(player.bonus, 0)));
    player.panaceaUsed = Math.max(0, Math.floor(fin(player.panaceaUsed, 0)));
...
// ===== 🪄 核心流派裝備指定掉落 =====
// 每件獨立判定；由 killMob 的正式掉落管線取得，因此保留頭目裝備的詞綴、洞數與寶石鑲嵌能力。
[
    ['炎魔的巴列斯', 'wpn_hydra_staff_test', 5],
    ['黑長者',       'wpn_static_field_staff_test', 5],
    ['冰之女王',     'wpn_frozen_orb_staff_test', 5],
    ['克特',         'wpn_chain_lightning_staff_test', 5],
    ['不死鳥',       'wpn_meteor_rain_staff_test', 5]
    ,['古代巨人',    'wpn_whirlwind_axe', 5]
    ,['曼波兔',      'bot_leap_storm', 5]
    ,['死亡騎士',    'shd_holy_reliquary', 5]
    ,['暗殺軍王史雷佛','clk_shadow_doppelganger', 5]
    ,['林德拜爾',    'wpn_thunder_javelin', 5]
    ,['不幸的幻象眼魔','wpn_nightmare_qigu', 5]
    ,['邪惡的鐮刀死神','wpn_rift_qigu', 5]
    ,['夢幻之島風精靈王','wpn_gale_hunter_bow', 5]
    ,['闇精靈王',    'wpn_lightning_sentry_claw', 5]
].forEach(function (r) {
    const list = MOB_DROPS[r[0]] = MOB_DROPS[r[0]] || [];
    if (!list.some(function (e) { return e[0] === r[1]; })) list.push([r[1], r[2]]);
});

// ===== 🔥❄️⚡ v3.9.12 馬賽克支援裝備指定掉落 =====
// 由正式掉落管線產生，防具保留詞綴、指定洞數、打洞與寶石系統；護符保留詞綴但不額外產生裝備孔。
[
    ['混沌',           'hlm_mosaic_gaze',  3],
    ['暗殺軍王史雷佛', 'glv_mosaic_talon', 3],
    ['死亡',           'bot_mosaic_dance', 3],
    ['吉爾塔斯',       'amu_mosaic_prism', 1]
].forEach(function (r) {
    const list = MOB_DROPS[r[0]] = MOB_DROPS[r[0]] || [];
    if (!list.some(function (e) { return e[0] === r[1]; })) list.push([r[1], r[2]]);
});

Object.assign(ITEM_WEIGHTS, {
    '萬象凝視頭飾': 5,
    '鳳凰聚氣手甲': 10,
    '龍爪幻舞長靴': 15,
    '稜光萬象護符': 5
...
// ===== 💍 v3.9.2 喬丹之石指定掉落 =====
// 固定白板唯一戒指：noAffix 會讓正式掉落管線跳過祝福與 D2R 八色詞綴。
[
    ['吉爾塔斯',   'acc_stone_of_jordan', 0.01],
    ['不滅的巫妖', 'acc_stone_of_jordan', 0.005],
    ['巴風特',     'acc_stone_of_jordan', 0.001],
    ['黑長者',     'acc_stone_of_jordan', 0.001]
].forEach(function (r) {
    const list = MOB_DROPS[r[0]] = MOB_DROPS[r[0]] || [];
    if (!list.some(function (e) { return e[0] === r[1]; })) list.push([r[1], r[2]]);
});
...
// ===== 🔥 v3.9.3 地獄火炬三鑰匙 =====
// 表內為基礎 1%；正式版全域掉落倍率 0.5 仍會照常作用。
[
    ['吉爾塔斯', 'mat_key_terror',      1],
    ['安塔瑞斯', 'mat_key_hatred',      1],
    ['巴拉卡斯', 'mat_key_destruction', 1]
].forEach(function (r) {
    const list = MOB_DROPS[r[0]] = MOB_DROPS[r[0]] || [];
    if (!list.some(function (e) { return e[0] === r[1]; })) list.push([r[1], r[2]]);
});
