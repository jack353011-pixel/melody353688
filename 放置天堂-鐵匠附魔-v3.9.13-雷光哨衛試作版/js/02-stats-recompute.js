...
function recomputeStats() {
    ...
    d.cha = (b.cha || 0) + (a.cha || 0) + (pn.cha||0);   // 魅力：第六屬性，無上限
    // ♾️ 轉生永久屬性：每次轉生獲得的自由點數由成長中心分配，所有加成參與完整衍生公式。
    if (p.rebirthBonus && typeof p.rebirthBonus === 'object') {
        ['str','dex','con','int','wis','cha'].forEach(s => { d[s] += Math.max(0, Math.floor(Number(p.rebirthBonus[s]) || 0)); });
    }
    ...
    ['str','dex','con','int','wis','cha'].forEach(_s => { d[_s] += (_d2[_s] || 0); });   // D2R 六維先進衍生公式；玩家與傭兵共用
    for (let k in p.eq) {
        let e = p.eq[k];
        if (!e || k === 'wpn') continue;
    ...
    // 🎯 v3.8.39：六維最終屬性不再夾至 100，資訊欄會顯示完整實際數值。
    ...
    d.getMpCost = function(baseMp, tier) {
        let _rt=typeof runeEquipTotals==='function'?runeEquipTotals(p):{};
        let c = Math.max(1, Math.ceil(baseMp * (1 - Math.min(95,d.mpReduce+(_rt.mpCost||0)) / 100)));
    ...
    // D2R 第一階段固定能力；在基礎 HP/MP 建立後、一般裝備加成前套用。
    p.mhp += (_d2.hp || 0); p.mmp += (_d2.mp || 0);
    d.meleeDmg += (_d2.md || 0); d.rangedDmg += (_d2.rd || 0); d.magicDmg += (_d2.mg || 0);
    d.meleeHit += (_d2.mh || 0); d.rangedHit += (_d2.rh || 0); d.magicHit += (_d2.gh || 0);
    d.resFire += (_d2.rf || 0); d.resWater += (_d2.rw || 0); d.resEarth += (_d2.re || 0); d.resWind += (_d2.ra || 0); d.resNone += (_d2.rn || 0);
    d.ac -= (_d2.ac || 0); d.mr += (_d2.mr || 0); d.er += (_d2.er || 0); d.dr += (_d2.dr || 0);
    d.meleeCrit += (_d2.mc || 0); d.rangedCrit += (_d2.rc || 0); d.magicCrit += (_d2.gc || 0);
    d.meleeCritDmg += (_d2.mcd || 0); d.rangedCritDmg += (_d2.rcd || 0); d.magicCritDmg += (_d2.gcd || 0);
    d.hpR += (_d2.hpr || 0); d.mpR += (_d2.mpr || 0);
    d.d2MagicFind = (_d2.mf || 0);   // 只提高未來裝備成為魔法／稀有品質的機率，不提高基礎掉落率
    d.atkSpdPct += Math.min(30, (_d2.as || 0));   // D2R 攻速總上限 +30%；不影響施法速度
...
d.mr += (baseMr + bonusMr);
        ...
        let _runeword = typeof effectiveRuneword === 'function' ? effectiveRuneword(p, e) : (typeof activeRuneword === 'function' ? activeRuneword(e) : null);
        if (_runeword && Array.isArray(_runeword.grantSkills)) {
            _runeword.grantSkills.forEach(sk => {
                if (!player.grantedSkills.includes(sk)) player.grantedSkills.push(sk);
                if (!player.skills.includes(sk)) player.skills.push(sk);
            });
        }
    ...
    // ❄️ 蕾雅魔杖＋蕾雅戒指：王室寒勳只在兩件遺物重合時顯現。
    let _laiaPair = p.eq.wpn && p.eq.wpn.id === 'wpn_laia_wand' && Object.values(p.eq).some(e => e && e.id === 'acc_laia_ring');
    if (_laiaPair) {
    ...
    if (masteryEquipmentTagActive(p, 'e_sword')) spdMult *= (1/1.5);   // 🏅 劍術精通：[單手]＋[近戰] 裝備標籤相符時攻速+50%
    ...
    if (masteryEquipmentTagActive(p, 'k_royal_sword')) spdMult *= (1/1.5);   // 👑 劍術精通：[單手劍]／[雙手劍] 標籤相符時攻速+50%
    if (masteryEquipmentTagActive(p, 'i_qigu') || masteryEquipmentTagActive(p, 'i_magicsword')) spdMult *= (1/1.3);   // 🔮 奇古獸／魔劍精通依裝備標籤啟動攻速+30%
    ...
    if(_equipMmpPct)p.mmp=Math.floor(p.mmp*(1+_equipMmpPct/100));
    ...
    { let _rt=typeof runeEquipTotals==='function'?runeEquipTotals(p):{},_rw=typeof runewordEquipTotals==='function'?runewordEquipTotals(p):{},_hpPct=(_rt.hpPct||0)+(_rw.hpPct||0);if(_hpPct)p.mhp=Math.floor(p.mhp*(1+_hpPct/100));if(_rw.mpPct)p.mmp=Math.floor(p.mmp*(1+_rw.mpPct/100)); }
