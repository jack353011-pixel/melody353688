...
function rollHealingSpell(sk, dStats, caster, target) {
    ...
    let _classSkillMult = (typeof classSkillEquipMult === 'function') ? classSkillEquipMult(sk, caster) : 1;
    ...
    if (sk.classicHeal) {
        ...
        return Math.max(1, Math.floor(rolled * mult * _jm * _fw * _classSkillMult));
    }
    // 尚未轉換的治癒來源保留舊資料相容性。
    if (sk.healDice) return Math.max(1, Math.floor((rollDice(sk.healDice[0], sk.healDice[1]) + (sk.healBase || 0)) * (1 + 3 * (Number(dStats && dStats.magicDmg) || 0) / 32) * _jm * _fw * _classSkillMult));
    if (sk.valDice) return Math.max(1, Math.floor(((sk.valBase || 0) + roll(sk.valDice[0], sk.valDice[1]) + (Number(dStats && dStats.magicDmg) || 0)) * _jm * _fw * _classSkillMult));
    return 0;
}
...
function playerAttackIntervalTicks(includeTemporarySlow) {
    ...
    if((player._d2rKillHasteUntil||0)>state.ticks&&player._d2rKillHastePct>0)ticks=Math.max(1,ticks/(1+player._d2rKillHastePct/100));
...
function tick() {
    ...
    for(let k in player.statuses) {
        if (player.statuses[k] > 0 && k !== 'poisonDmg' && k !== 'poisonTick' && k !== 'burnDmg' && k !== 'burnTick' && k !== 'scaldDmg' && k !== 'scaldTick' && k !== 'bleedDmg' && k !== 'bleedTick' && k !== 'armorBreakPct') {   // 😤 v3.6.20 armorBreakPct＝伴隨值（%數）非時長，不遞減
            let _rr=typeof runeEquipTotals==='function'?runeEquipTotals(player):{},_red=({poison:_rr.poisonRed,burn:_rr.burnRed,paralyze:_rr.paralyzeRed,freeze:_rr.freezeRed})[k]||0;
            player.statuses[k]-=1/Math.max(.4,1-_red/100);
...
function getPhysicalDmg(diceStr, target, wpn, arrowData, forceHeavy, forceHit, forceLand, forceCrit, wpnInst, forceGraze, probe) {
    ...
    if (!probe && typeof d2rHuntDamage === 'function') _outDmg = d2rHuntDamage(player, target, _outDmg, _wAff ? _wAff.ele : getWpnEle(null, DB.items[_swingId]));
...
function dualWieldOffhandAttack(t) {
    ...
    dmg = Math.max(1, dmg);
    if(typeof d2rGameplayMainDamage==='function')dmg=d2rGameplayMainDamage(player,dmg);
    if(typeof d2rPerfectStrike==='function'){let _pso=d2rPerfectStrike(player,dmg);dmg=_pso.dmg;if(_pso.proc)logCombat('<span class="text-yellow-200 font-bold">【完美一擊】</span>副手攻擊造成額外傷害！','player-heavy');}
    dmg=runeOutgoingDamage(player,t,dmg); t.curHp -= dmg; d2rOnHit(player, t, dmg); if(typeof d2rGameplaySplash==='function')d2rGameplaySplash(player,t,dmg,false); if (typeof terrorVisageOnDamage === 'function') terrorVisageOnDamage(t, dmg, 'melee'); t.justHit = getWpnEle(player.eq.offwpn, owpn); mobWake(t);   // 🌅 巨大骷髏：副手視為近距離
...
// D2R 稀有命中特效：玩家／傭兵共用。固定附傷吃屬性剋制；耐久同步縮放發動率與數值。
function d2rOnHit(owner, target, dealtDamage) {
    if (!owner || !owner.eq || !target || target.curHp <= 0) return;
    let eleMap={ff:'fire',fw:'water',fa:'earth',fe:'wind'}, nameMap={ff:'烈焰',fw:'寒霜',fa:'大地',fe:'風雷'};
    Object.values(owner.eq).forEach(it => {
        let durabilityMult=d2rDurabilityMult(it), valueMult=durabilityMult*d2rEnhanceAffixMult(it);
        d2rAffixRows(it).forEach(r => {
            if (!['ff','fw','fa','fe','ph','pm'].includes(r[0])) return;
            // 強化只放大特效數值；發動率仍只受耐久影響，避免高強裝備雙重成長。
            let rate=(9-r[2])*durabilityMult;
            ...
            if (eleMap[r[0]] && target.curHp>0) {
                ...
                if (owner===player) owner.hp=Math.min(owner.mhp,(owner.hp||0)+val);
                else owner.curHp=Math.min(owner.mhp||1,(owner.curHp||0)+val);
            } else if (r[0]==='pm') owner.mp=Math.min(owner.mmp||0,(owner.mp||0)+val);
        });
        // 紫寶石：一般攻擊命中時每顆獨立 5% 回復 MP；不受裝備耐久衰減。
        equipSocketRows(it).filter(Boolean).forEach(socket => {
            if(!socket||socket.kind!=='gem')return;
            let eff=gemEffectForItem(socket.id,it);
            if(eff&&eff.code==='pm'&&Math.random()<.05)owner.mp=Math.min(owner.mmp||0,(owner.mp||0)+eff.value);
        });
    });
    let _r=typeof runeEquipTotals==='function'?runeEquipTotals(owner):{};
    if(_r.lifeSteal&&dealtDamage>0){let heal=Math.max(1,Math.floor(dealtDamage*_r.lifeSteal/100));if(owner===player)owner.hp=Math.min(owner.mhp,(owner.hp||0)+heal);else owner.curHp=Math.min(owner.mhp||1,(owner.curHp||0)+heal);}
}
