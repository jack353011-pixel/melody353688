function playerAttack() {
    ...
    if (result.hit) {
        ...
        if(typeof d2rGameplayMainDamage==='function')result.dmg=d2rGameplayMainDamage(player,result.dmg);
        ...
        if (target.curHp > 0 && typeof hellfireTorchTryProc === 'function') hellfireTorchTryProc(target);
...
function stormBuffTick(sk, noMageBonus) {
    ...
    targets.forEach((t, _illusionIdx) => {
        ...
        d = illusionMagicDmg(d, true, _illusionIdx === 0); if (typeof d2rHuntDamage === 'function') d = d2rHuntDamage(player, t, d, sk.ele || 'none'); t.curHp -= d; t.justHit = (sk.ele && sk.ele !== 'none') ? sk.ele : 'magic'; if (typeof moonShatterOnDamage === 'function') moonShatterOnDamage(player, t, d); mobWake(t);   // 🔮 幻覺2件每次法術僅回一次MP；5件仍逐目標二次傷害；八色流派詞綴統一結算
...
// 🔥 地獄火炬：一般攻擊或造成傷害的主動技能命中時 5% 引發；觸發後 10 秒內置冷卻。
// 此效果是裝備 proc，不建立技能、不加入自動施法清單，也不會改變九頭蛇／火球的既有延伸關係。
function hellfireTorchTryProc(target) {
    if (!player || !player.eq || !target || target.curHp <= 0 || target._dead) return false;
    let item = player.eq.charm, def = item && DB.items[item.id], proc = def && def.hellfireProc;
    if (!proc) return false;
    let now = (typeof state !== 'undefined' && Number.isFinite(state.ticks)) ? state.ticks : 0;
    if ((player._hellfireTorchCdUntil || 0) > now) return false;
    if (Math.random() * 100 >= (Number(proc.rate) || 5)) return false;
    player._hellfireTorchCdUntil = now + (Number(proc.cdSec) || 10) * 10;
    let effMr = (target.st && target.st.mrhalf > 0) ? (target.mr || 0) / 2 : (target.mr || 0);
    let base = 30 + Math.floor((player.lv || 1) / 2) + Math.max(0, Number(player.d && player.d.magicDmg) || 0);
    let coef = (typeof magicDamageCoef === 'function') ? magicDamageCoef(player.d, magicAttrDefense(target, 'fire'), 3) : 1;
    let resist = (typeof mrMult === 'function') ? mrMult(effMr) : 1;
    let dmg = Math.max(1, Math.floor(base * coef * resist * elementCounterMult('fire', target.e) * fragileMult(target)));
    target.curHp -= dmg; target.justHit = 'fire'; target._spellHurt = true; mobWake(target);
    if (typeof playSpellFx === 'function') { try { playSpellFx('地獄火', target); } catch (e) {} }
    logCombat(`<span class="font-bold" style="color:#fb923c;text-shadow:0 0 7px #dc2626;">【地獄火炬】</span>地獄火焰吞噬 <span class="${getMobColor(target.lv)}">${target.n}</span>，造成 ${dmg} 點火屬性魔法傷害！（內置冷卻 10 秒）`, 'player-special');
    return true;
}
...
function equipSkillDmgMult(sk, skId, who) {
    ...
    if (typeof classSkillEquipMult === 'function') m *= classSkillEquipMult(sk, _w, skId);
    return m;
}
...
function playerStatusResisted(kind) {
    ...
    if(typeof d2rEquipTotals==='function')pct+=(d2rEquipTotals(player).abr||0);
...
function allyStatusResisted(ally, kind) {
    ...
    if(typeof d2rEquipTotals==='function')pct+=(d2rEquipTotals(ally).abr||0);
...
function _procWeaponSpellHit(t, sp, en, illusionRecoverMp) {
    ...
    if (typeof d2rHuntDamage === 'function') d = d2rHuntDamage(player, t, d, sp.ele || 'none');
    t.curHp -= d;
...
function laiaWandHitProc(t) {
    ...
    d = Math.max(1, Math.floor(d * rlFuryMult()));   // 🔮 紅獅5/5＋😡狂怒5/5 最終傷害
    if (t.st && t.st.mrhalf > 0) t.st.mrhalf = 0;
    if (typeof d2rHuntDamage === 'function') d = d2rHuntDamage(player, t, d, sp.ele || 'none', wasFrozen);
...
function _enemyPhysicalAttackInner(mob, idx, stunChance = 0, atkDmg = null, atkDb = null, isBasicAttack = false) {   // atkDmg/atkDb：連擊技覆寫骰子/加值（如鐮刀劍氣斬 9×3D70+99，與一般攻擊不同）
    ...
    if (hit) {
        ...
        if(player.eq.shield && (!player.classicMode || _lancePair)) {   // 🎮 經典模式：盾牌無格檔（長槍＋塔盾成對＝例外放行）
            ...
            let _rwBlock=(typeof runewordEquipTotals==='function'?runewordEquipTotals(player):{}).block||0;
            let _blockChance = _lancePair ? 100 : (((_sh && _sh.block) ? (heavy ? _sh.block : _sh.block * 0.3) : 0)+_rwBlock);   // 🛡️ 非重擊基礎率＋「韻律」固定格擋率。
