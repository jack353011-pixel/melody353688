...
function renderAuditTab() {
    ...
    let _backupRows = (typeof autoBackupList === 'function' ? autoBackupList() : []).map(b => {
        let dt = b.at ? new Date(b.at).toLocaleString() : '未知時間';
        return `<div class="flex items-center justify-between gap-2 bg-slate-800/60 rounded px-2 py-1.5">`
            + `<span class="min-w-0 truncate text-slate-300" title="${dt}">${dt}・Lv.${b.lv}</span>`
            + `<button onclick="restoreAutoBackup(${b.index})" class="btn shrink-0 px-2 py-1 text-xs bg-emerald-900 border-emerald-700 text-emerald-200">還原</button>`
            + `</div>`;
    }).join('');
    el.innerHTML = `
...
function killMob(idx) {
    ...
    let _playerHunt = typeof d2rHuntTotals === 'function' ? d2rHuntTotals(player) : {};
    let _petExpGain = Math.floor(_expEach * (1 + dollFieldVal('expBonus') / 100) * _balanceExp);   // 🐾 每隻存活寵物各得完整玩家份額；玩家滿等不影響養寵
    let _playerPetExpGain = Math.floor(_petExpGain * (1 + (_playerHunt.xf || 0) / 100));
    let _playerExpGain = Math.floor(_playerPetExpGain * getExpGainMult(player.lv));   // 八色經驗詞綴只提高主玩家自身，不複製寵物經驗。
    ...
    if (player.allies && player.allies.length && mob.exp) {
        player.allies.forEach(a => {
            ...
            // 來源角色正在自己的分頁遊玩或已建立離線掛網時，傭兵仍照常作戰，但不再累積第二份角色經驗。
            if (typeof mercenarySourceOwnProgressActive === 'function' && mercenarySourceOwnProgressActive(a)) return;
            let _ah = typeof d2rHuntTotals === 'function' ? d2rHuntTotals(a) : {};
            let _gain = Math.floor(_expEach * getExpGainMult(a.lv || 1) * _balanceExp * (1 + (_ah.xf || 0) / 100));
            if (_gain <= 0) return;
            a.exp = (a.exp || 0) + _gain;
    ...
    if (!_kbNoReward && !mob.noGold && Math.random() < _goldDropRate) {
        ...
        g = Math.floor(g * (1 + dollFieldVal('goldBonus') / 100) * partyRewardMult() * (typeof balanceMult === 'function' ? balanceMult('gold') : 1) * (1 + (_playerHunt.gf || 0) / 100));   // 金幣為隊伍共用資源，只採主玩家裝備且全身最高 +50%。
        player.gold += g;
    ...
    // 第二階段狩獵詞綴：隊伍成功擊殺時，各角色只依自己穿著的裝備回復；倒地傭兵不觸發。
    if(typeof d2rHuntOnKill==='function'){
        if(!player.dead&&player.hp>0)d2rHuntOnKill(player);
        if(player.allies&&player.allies.length)player.allies.forEach(a=>{if(a&&!a._downed)d2rHuntOnKill(a);});
    }
    if(typeof d2rTriggerOnTeamKill==='function')d2rTriggerOnTeamKill(mob);
