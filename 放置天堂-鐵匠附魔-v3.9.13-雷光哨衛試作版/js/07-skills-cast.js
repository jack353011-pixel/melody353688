...
function summonDamageMult(sm, owner, magicBased, teamMagicDmg) {
    ...
    if (typeof classSkillEquipMult === 'function') mult *= classSkillEquipMult(DB.skills[sm.skId], owner, sm.skId);
...
function castSkill(skId) {
    let r, _beforeEnemyHp = 0;
    if (typeof mapState !== 'undefined' && mapState.mobs) _beforeEnemyHp = mapState.mobs.reduce((s, m) => s + (m && !m._dead && m.curHp > 0 ? m.curHp : 0), 0);
    ...
    if (r && typeof hellfireTorchTryProc === 'function' && typeof mapState !== 'undefined' && mapState && mapState.mobs) {
        let _afterEnemyHp = mapState.mobs.reduce((s, m) => s + (m && !m._dead && m.curHp > 0 ? m.curHp : 0), 0);
        if (_afterEnemyHp < _beforeEnemyHp) {
            let _torchTarget = getTarget();
            if (_torchTarget && hellfireTorchTryProc(_torchTarget)) {
                let _torchIdx = mapState.mobs.findIndex(m => m && m.uid === _torchTarget.uid);
                if (_torchTarget.curHp <= 0 && _torchIdx !== -1) killMob(_torchIdx); else renderMobs();
            }
        }
    }
...
function castSkillInner(skId) {
    ...
    if (sk.darkCrit) {
        ...
        if (typeof classSkillEquipMult === 'function') dmg = Math.max(1, Math.floor(dmg * classSkillEquipMult(sk, player, skId)));
    ...
    if(sk.type === 'atk' && (player.cds.atkSk <= 0 || _echoFree || _royalFreeCast)) {   // 🏅 迴響精通／👑 魔法精通：免費施放不受攻擊冷卻限制
        ...
        if (sk.roarFixed) {
            ...
            if (typeof classSkillEquipMult === 'function') base = Math.max(1, Math.floor(base * classSkillEquipMult(sk, player, skId)));
        ...
        if(sk.dmgType === 'physical') {
            ...
            for(let h = 0; h < hits; h++) {
                ...
                if (typeof classSkillEquipMult === 'function') res.dmg = Math.max(1, Math.floor(res.dmg * classSkillEquipMult(sk, player, skId)));
...
function applyTeamHot(skId, sk, dStats, caster) {
    ...
    try { if (typeof classSkillEquipMult === 'function') _hm *= classSkillEquipMult(sk, caster, skId); } catch (e) {}
