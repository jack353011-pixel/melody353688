...
function gainItem(id, cnt=1, silent=false, forceNormal=false, affixOld=false, deferUi=false, fixedAffixes=null, blessRate=null) {   // ⚠️ 第 7 參供既定詞綴來源；第 8 參供製作等來源指定祝福率
    ...
    if (!forceNormal && !_noAffixCtx && d && !d.noAffix && !isRelic(d) && ((d.type === 'wpn' && !d.isArrow) || d.type === 'arm' || d.type === 'acc')) {   // 🦴 _noAffixCtx／noAffix：固定白板→不附詞綴；🏺 遺物永不附詞綴
    ...
    let seteff = false;
    let _d2roll = (!d.noAffix && fixedAffixes && Array.isArray(fixedAffixes.d2))
        ? { quality: (typeof D2R_QUALITY !== 'undefined' && D2R_QUALITY[fixedAffixes.d2q]) ? fixedAffixes.d2q : 'magic', rows: d2rAffixRows({d2:fixedAffixes.d2}) }
        : ((!forceNormal && !_noAffixCtx && !d.noAffix) ? d2rRollAffixes(d) : null);
    let _socketRoll = (fixedAffixes && Array.isArray(fixedAffixes.sockets))
        ? { socketMax:Math.max(0,Math.floor(Number(fixedAffixes.socketMax)||0)), sockets:fixedAffixes.sockets.slice() }
        : ((_lootMobInfo && d && !isRelic(d) && ((d.type==='wpn'&&!d.isArrow)||d.type==='arm') && typeof d2rRollSockets === 'function') ? d2rRollSockets(d,_d2roll?_d2roll.quality:'white',{boss:!!_lootMobInfo.boss}) : null);

    let _tEn = 0;   // 🏛️ v3.0.83 傳統模式已取消：掉落自帶強化值停用（任何來源恆 +0·手動強化照常）
    let _probe = { id: id, en: _tEn, bless: bless, anc: anc, attr: attr, seteff: seteff, d2q:_d2roll && _d2roll.quality, d2:_d2roll && _d2roll.rows, gamblePaid:fixedAffixes && Number(fixedAffixes.gamblePaid)||0, socketMax:_socketRoll && _socketRoll.socketMax, sockets:_socketRoll && _socketRoll.sockets };
    ...
    else { let _push = { id: id, uid: uid(), cnt: cnt, en: _tEn, bless: bless, anc: anc, attr: attr, seteff: seteff, lock: false, junk: !_d2roll && !!(player.junkPrefs && player.junkPrefs[itemSig(_probe)]) && !(d && d.noJunk) }; if (_d2roll) { _push.d2q=_d2roll.quality; _push.d2=_d2roll.rows; let _dm=d2rDurabilityMax(_push); if(_dm)_push.dur=_dm; } if (_probe.gamblePaid) _push.gamblePaid=_probe.gamblePaid; if (_socketRoll) { _push.socketMax=_probe.socketMax; _push.sockets=equipSocketRows(_probe); } if (_gw) _push.gw = _gw; player.inv.push(_push); if (_fastGainIndex) _rememberCatchupGainItemStack(_push); }   // D2R 詞綴裝備預設不繼承廢品標記，避免新掉落被靜默賣掉。

    // 紀錄這次產生的物品屬性
    let itemInfo = { id: id, cnt: cnt, en: _tEn, bless: bless, anc: anc, attr: attr, seteff: seteff, d2q:_d2roll && _d2roll.quality, d2:_d2roll && _d2roll.rows, gamblePaid:_probe.gamblePaid, socketMax:_probe.socketMax, sockets:_probe.sockets };
...
function getItemFullName(item) {
    let d = DB.items[item.id];
    if(!d) return "未知的物品";
    let _d2name = typeof d2rNameAffixes === 'function' ? d2rNameAffixes(item) : {prefix:'',suffix:''};
    let _attrMagic = getAttrMagicProc(item);
    let segs = _attrMagic ? `<span class="text-yellow-300 font-bold">${'★'.repeat(_attrMagic.star)}</span> ` : '';
    if (item.d2q && typeof d2rQualityDef === 'function') { let _dq=d2rQualityDef(item.d2q); segs += `<span class="${_dq.c} font-bold">${_dq.n} </span>`; }
    let aff = getAttrAffix(item.attr);
    if (aff) {
    ...
    if (_d2name.prefix) segs += `<span class="${d2rQualityDef(item.d2q).c}">${_d2name.prefix}</span>`;
    ...
    let _rw=typeof activeRuneword==='function'?activeRuneword(item):null,_rwStop=_rw&&typeof runewordPaused==='function'&&runewordPaused(player,item),_rwSuffix=_rw?`【${_rw.n}${_rwStop?'・暫停':''}】`:'';
    let _ac=typeof alchemyNormalizeCore==='function'?alchemyNormalizeCore(item.alchemyCore):null,_acState=_ac&&typeof alchemyCoreState==='function'?alchemyCoreState(player,item):null;
    let _acSuffix=_ac?`【♾️${alchemyCoreName(_ac)}${_acState&&_acState.state==='paused'?'・暫停':''}】`:'';
    let setPrefix = item.seteff ? item.seteff.slice(0, 2) : "";   // 🔮 席琳套裝：套裝名冠在裝備名稱前（如「紅獅環甲」）；顏色沿用 getItemColor（規則同前）
    let _nameColor = item.d2q ? d2rQualityDef(item.d2q).c : getItemColor(item);
    return `${segs}<span class="${_nameColor}">${en}${setPrefix}${d.n}${_d2name.suffix}${_rwSuffix}${_acSuffix}${cnt}</span>`;
}
...
function playerEquipPotionBonusPct() {
    ...
    try { if(typeof d2rEquipTotals==='function')sum+=(d2rEquipTotals(player).pot||0); } catch(e){}
    return sum;
}
...
function batchUseItem(u) {
    ...
    // 💊 v3.8.39 萬能藥無使用總量與屬性上限；批量使用只受持有數限制
    if (d.eff === 'panacea') {
        const STAT_CN = { str:'力量', dex:'敏捷', con:'體質', int:'智力', wis:'精神', cha:'魅力' };
        let st = d.pstat;
        let maxN = Math.max(1, Math.floor(item.cnt || 1));
        let rawP = prompt(`要使用幾瓶 ${d.n}？（持有 ${item.cnt} 瓶·本次最多 ${maxN} 瓶）`, maxN);
        if (rawP === null) return;
        ...
        calcStats();
        logSys(`使用了 <span class="${d.c || 'text-pink-300'} font-bold">${d.n}</span> ×${nP}，${STAT_CN[st]} 永久 +${nP}！（累計使用 ${player.panaceaUsed} 瓶）`);
...
function useItem(u, silent = false) {
    ...
    // 🔥 未鑑定的地獄火炬：本角色第一次必定本職，之後以 committed RNG 隨機八職業。
    if (d.eff === 'hellfire_unseal') {
        if (silent) return;
        const _torchClasses = ['royal','knight','mage','elf','dark','illusion','dragon','warrior'];
        const _torchClassName = { royal:'王族', knight:'騎士', mage:'法師', elf:'妖精', dark:'黑暗妖精', illusion:'幻術士', dragon:'龍騎士', warrior:'戰士' };
        let opened = Math.max(0, Math.floor(Number(player.hellfireTorchOpened) || 0));
        let cls = opened === 0 ? player.cls : _torchClasses[Math.floor(lootRng('hellfire-torch-class-' + opened) * _torchClasses.length)];
        if (!_torchClasses.includes(cls)) cls = 'knight';
        let resultId = 'charm_hellfire_' + cls;
        if (!DB.items[resultId]) { logSys('<span class="text-red-400">地獄火炬鑑定失敗：找不到職業版本。</span>'); return; }
        item.cnt--; if (item.cnt <= 0) player.inv = player.inv.filter(i => i.uid !== item.uid);
        player.hellfireTorchOpened = opened + 1;
        gainItem(resultId, 1, true, true);
        logSys(`<span class="c-legend font-bold">🔥 地獄火照亮了護符上的紋章！</span>你鑑定出 <span class="text-amber-300 font-bold">${DB.items[resultId].n}</span>${opened === 0 ? `（第一次鑑定：固定為本職${_torchClassName[cls]}）` : `（隨機職業：${_torchClassName[cls]}）`}。`);
        renderTabs(); updateUI(); saveGame();
        if (!document.getElementById('item-modal').classList.contains('hidden')) closeModal();
        return;
    }
    ...
    if (d.type === 'pot' || d.eff === 'poly' || d.eff === 'reset' || d.eff === 'magicbarrier' || d.eff === 'teleport_scroll' || d.eff === 'panacea') {   // 變形卷軸(eff:poly)、回憶蠟燭(eff:reset)、魔法卷軸(eff:magicbarrier)、瞬間移動卷軸(eff:teleport_scroll)亦走此消耗品分支
        ...
        if (item.id.includes('potion_heal') || item.id === 'potion_strong' || item.id === 'potion_ult') {
            ...
            if(!silent) logSys(`使用了 ${d.n}，${STAT_CN[st]} 永久 +1！（累計使用 ${player.panaceaUsed} 瓶）`);
...
// D2R 輕量耐久：按全隊擊殺累積，僅磨損主玩家已穿武器／防具；傭兵與遠征裝備不處理。
function d2rWearEquipped(mob) {
    if (!player || !player.eq) return;
    player._d2DurKills = Math.max(0, Number(player._d2DurKills) || 0) + ((mob && mob.boss) ? 10 : 1);
    let wear = Math.floor(player._d2DurKills / 150);
    if (wear <= 0) return;
    player._d2DurKills %= 150;
    Object.values(player.eq).forEach(it => {
        let max = d2rDurabilityMax(it);
        if (!max) return;
        it.dur = Math.max(0, d2rDurabilityNow(it) - wear);
    });
    calcStats();
}
...
function equipItem(item) {
    ...
    if (!checkCanEquip(item)) {
        if (d && d.reqLv && (player.lv || 1) < d.reqLv) {
            logSys(`<span class="text-red-400">無法裝備，需要 Lv.${d.reqLv}（目前 Lv.${player.lv || 1}）。</span>`);
            return;
        }
        let _rs = typeof d2rRequirementStatus === 'function' ? d2rRequirementStatus(item, player) : {ok:true};
        if (!_rs.ok && _rs.req) {
            let _sn={str:'力量',dex:'敏捷',con:'體質',int:'智力',wis:'精神',cha:'魅力'}[_rs.req.stat];
            logSys(`<span class="text-red-400">無法裝備，需要 Lv.${_rs.req.level} 且 ${_sn} ${_rs.req.value}（目前 Lv.${player.lv}／${_sn} ${Math.floor(_rs.current||0)}）。</span>`);
        } else logSys(d.reqAvatar ? `無法裝備，「${d.n}」僅限${d.reqAvatar}。` : `無法裝備，職業不符。`);
        return;
    }
...
function renderStatusEffects() {
    ...
    if(typeof equippedRunewordRows==='function'&&typeof runewordEffectLines==='function'){
        let _rwRows=equippedRunewordRows(player).filter(row=>row.enabled);
        if(_rwRows.length){
            let _rwHtml=_rwRows.map(row=>`<span class="text-orange-300 font-bold">${row.word.n}</span>：${runewordEffectLines(row.word).join('、')}`).join('<br>');
            html+=`<div class="mt-2 border-t border-amber-800/70 pt-1"><span class="text-amber-300 font-bold">符文組新增屬性</span><br>${_rwHtml}</div>`;
        }
    }
