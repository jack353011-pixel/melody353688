...
function _renderMobsImpl() {
    ...
    for(let _k = 0; _k < _order.length; _k++) {
        ...
        if (m) {
            ...
            if(_showMobStatus && m.boss && (m._bossResiliencePct || 0) > 0) _badgeTags += ` <span class="px-1 rounded bg-emerald-950/90 text-emerald-200 text-[10px] font-bold border border-emerald-400/60" title="降低所有來源的最終扣血，包含玩家、傭兵、寵物、召喚物、持續、固定與百分比傷害">韌性 ${Math.round(Number(m._bossResiliencePct) * 100)}%</span>`;
            if(_showMobStatus && m.boss && (m._armyHpMult || 1) > 1) _badgeTags += ` <span class="px-1 rounded bg-indigo-950/90 text-indigo-200 text-[10px] font-bold border border-indigo-400/60" title="依出戰傭兵、寵物與召喚物提高首領生命；攻擊力不變">軍團耐久 ×${Number(m._armyHpMult).toFixed(2).replace(/0+$/,'').replace(/\.$/,'')}</span>`;
