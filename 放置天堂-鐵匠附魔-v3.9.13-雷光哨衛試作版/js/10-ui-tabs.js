...
function renderTabs(force) {
    ...
    slots.forEach(s => {
        ...
        if(eq) {
            ...
            let imgHtml = `<img src="${imgUrl}" loading="lazy" decoding="async" onerror="retryIconImage(this)" class="object-contain pointer-events-none ${glowClass}">`;
...
player.inv.forEach(i => {
    ...
    let imgHtml = `<img src="${imgUrl}" loading="lazy" decoding="async" onerror="retryIconImage(this)" class="w-6 h-6 object-contain pointer-events-none ${glowClass}"${_dimStyle}>`;
...
function weaponPurposeLabels(d) {
    ...
    if (d.core === 'vanderShockwave') out.push('范德震地（衝擊之暈延伸為基礎最多3目標的物理震波，短暫暈眩；基礎冷卻8秒）');
    if (d.core === 'frostDragonChase') out.push('冰龍追擊（屠宰者完成後追加基礎最多3目標的水屬性追擊，具有魔抗判定的冰凍；基礎冷卻10秒）');
    if (d.core === 'mindEcho') out.push('心靈共振（心靈破壞成功後追加基礎最多2目標的無屬性心靈波；基礎冷卻7秒）');
    if (d.core === 'riftBurst') out.push('裂界衝擊（粉碎能量成功後追加基礎最多3目標的武器震波，並有10%基礎暈眩機率；基礎冷卻6秒）');
    if (d.core === 'multiArrowRain') out.push('多重箭雨（三重矢成功後追加基礎6支風箭，分散攻擊場上敵人且不額外消耗箭矢；基礎冷卻7秒）');
    if (d.core === 'thunderJavelin') out.push('雷霆標槍流派（三重矢可觸發清群用連鎖雷電；精準射擊期間可改為單體充能一擊，兩種延伸不會在同一箭同時發動）');
...
function buildItemDescHTML(item) {
    ...
    let _socketRows = typeof equipSocketRows === 'function' ? equipSocketRows(item) : [];
    let _socketBrief = _socketMax ? `｜孔 ${_socketRows.length}/${_socketMax}` : '';
    if (_d2rows.length) {
        let _qd = typeof d2rQualityDef === 'function' ? d2rQualityDef(item.d2q) : {n:'魔法',c:'text-blue-300'};
        let _tiers = _d2rows.map(r => r[2]), _best = Math.min(..._tiers), _worst = Math.max(..._tiers);
        ...
        let _brief = _d2rows.map(r => {let _v=r[0]==='ac'?`-${_effVal(r)}`:`+${_effVal(r)}${['mc','rc','gc','mcd','rcd','gcd','hpp','mpp','pot','abr','gf','xf','nd','bd','kx','ts','pi','ks','sp','fh','fpen','wpen','epen','apen','sdm','bdr','und','dem','dra','hsk','opn','exe','pdr','mdr','ldr','adr','udr','ddr','hy','hd','st','gd','scd','ww','wd','wr','wc','ld','ls','lc','lr','bh','bt','bc','med','mes','mec','rbd','rbf','rbc','ard','arc','acd','csd','csp','csc','mzd','mzt','mzc','trd','trt','trc'].includes(r[0])?'%':''}`;return `<span title="原始：${d2rAffixText(r)}">${D2R_SHORT_LABEL[r[0]] || D2R_AFFIX_LABEL[r[0]]}${_v}<small class="text-emerald-400"> 實效</small><small class="text-slate-500"> T${r[2]}</small></span>`;});
        let _briefLines = [];
        ...
        let _rq = d2rEquipRequirement(item), _sn = _rq && ({str:'力量',dex:'敏捷',con:'體質',int:'智力',wis:'精神',cha:'魅力'}[_rq.stat]);
        if (_rq) desc += `<br><span class="text-amber-300">需求：Lv.${_rq.level}｜${_sn} ${_rq.value}</span>`;
        if (_em > 1) desc += `｜<span class="text-fuchsia-300">強化詞綴 +${Math.round((_em-1)*100)}%</span>`;
        let _dm = d2rDurabilityMax(item);
        let _durBrief = '';
        if (_dm) {
            let _dn = d2rDurabilityNow(item), _mult = d2rDurabilityMult(item);
            let _dc = _dn <= 0 ? 'text-red-400' : (_dn < _dm*.25 ? 'text-orange-300' : 'text-emerald-300');
            desc += `｜<span class="${_dc}">耐久 ${_dn}/${_dm}${_mult < 1 ? `（詞綴效果 ${Math.round(_mult*100)}%）` : ''}</span>`;
            _durBrief = `｜耐久 ${_dn}/${_dm}`;
        }
        let _represent = _d2rows.slice().sort((a,b)=>a[2]-b[2]||b[1]-a[1]).slice(0,2)
            .map(r=>`${D2R_SHORT_LABEL[r[0]] || D2R_AFFIX_LABEL[r[0]]}${r[0]==='ac'?'-':'+'}${_effVal(r)}${['mc','rc','gc','mcd','rcd','gcd','hpp','mpp','pot','abr','gf','xf','nd','bd','kx','ts','pi','ks','sp','fh','fpen','wpen','epen','apen','sdm','bdr','und','dem','dra','hsk','opn','exe','pdr','mdr','ldr','adr','udr','ddr','hy','hd','st','gd','scd','ww','wd','wr','wc','ld','ls','lc','lr','bh','bt','bc','med','mes','mec','rbd','rbf','rbc','ard','arc','acd','csd','csp','csc','mzd','mzt','mzc','trd','trt','trc'].includes(r[0])?'%':''}`).join('｜');
    ...
    if (_socketMax) {
        let _filled = _socketRows.filter(Boolean).length;
        let _slots = _socketRows.length ? _socketRows.map((x,i) => {
            if(!x)return `孔 ${i+1}：空`;
    ...
    if (d.gem && typeof gemDef === 'function') {
        let _g=gemDef(item.id);
        desc += `<br><span class="text-fuchsia-300 font-bold">💎 ${_g.rankDef.n}・${_g.colorDef.n}</span><br><span class="text-slate-300">武器與防具鑲嵌效果不同；請至成長中心查看目標裝備的實際效果。3 顆同色同階可合成下一階。</span>`;
    }
    ...
    if(typeof activeRuneword==='function'){let _rw=activeRuneword(item);if(_rw){
        ...
        desc=`<span class="${_paused?'text-slate-400':'text-orange-300'} font-bold">ᚱ 符文之語：${_rw.n}${_paused?'（暫停）':''}｜${_rw.seq.map(id=>DB.items[id].n.replace(' 符文','')).join('・')}</span><br><span class="${_paused?'text-slate-500':'text-amber-300'} font-bold">符文組新增屬性</span><br>${_rwLines.map(line=>`<span class="${_lineClass}">・${line}</span>`).join('<br>')}<br><span class="${_paused?'text-red-300':'text-slate-400'}">${_stateText}</span><br>`+desc;
    }}
    if(item.alchemyCore&&typeof alchemyNormalizeCore==='function'){
        let _ac=alchemyNormalizeCore(item.alchemyCore);if(_ac){
            item.alchemyCore=_ac;
            let _acs=typeof alchemyCoreState==='function'?alchemyCoreState(player,item):{state:'stored',enabled:false,reason:'尚未裝備'};
            let _acState=_acs.enabled?'已裝備，核心能力正在生效。':_acs.state==='stored'?'裝備後才會依全身上限判定是否生效。':`目前暫停：${_acs.reason||'未生效'}。`;
            let _acClass=_acs.enabled?'text-fuchsia-200':'text-slate-400',_acLines=typeof alchemyCoreLines==='function'?alchemyCoreLines(_ac):[];
            desc=`<span class="${_acs.enabled?'text-fuchsia-300':'text-violet-300'} font-bold">♾️ 鍊金符文：${alchemyCoreName(_ac)}${_acs.state==='paused'?'（暫停）':''}</span><br>${_acLines.map(line=>`<span class="${_acClass}">・${line}</span>`).join('<br>')}<br><span class="${_acs.state==='paused'?'text-red-300':'text-slate-400'}">${_acState}</span><br>`+desc;
        }
    }
    ...
    if (d.type === 'wpn' || d.type === 'arm' || d.type === 'acc') {
        ...
        if (d.classSkillMult && d.classSkillMult > 1) _eff.push(`本職原生技能增幅 +${Math.round((d.classSkillMult - 1) * 100)}%（傷害、治癒與召喚傷害；裝備授予及跨職精通技能不適用）`);
        if (d.mosaicChargeTimePct) _eff.push(`馬賽克蓄能保存時間 +${d.mosaicChargeTimePct}%（仍須裝備馬賽克並開啟雙重破壞）`);
        if (d.mosaicExtraChargeRate) _eff.push(`鳳凰聚氣（馬賽克普攻集氣時 ${d.mosaicExtraChargeRate}% 機率額外獲得 1 層）`);
        if (d.mosaicCooldownPct) _eff.push(`馬賽克元素終結冷卻 -${d.mosaicCooldownPct}%（與詞綴合計，最低 5 秒）`);
        if (d.mosaicPenetration) _eff.push(`馬賽克元素穿透 ${d.mosaicPenetration}%（只影響元素終結，降低目標元素抗性與有效 MR）`);
        if (d.hellfireProc) _eff.push(`地獄火焰 ${d.hellfireProc.rate || 5}%（一般攻擊命中或主動技能造成傷害時觸發火屬性魔法；內置冷卻 ${d.hellfireProc.cdSec || 10} 秒）`);
    ...
    if(d.mmpPct) statsArr.push(`MP上限+${d.mmpPct}%`);
    ...
    if (d.type === 'wpn' || d.type === 'arm' || d.type === 'acc') {
        ...
        let _logos = _EQ_CLASSES
            ...
            .join('');
        if (_logos) _detail.push(`<span class="text-slate-400">適用職業：</span>${_logos}`);
    }

    // ⚖️ 負重：計入負重的裝備（武器/防具/飾品）顯示重量
    if ((d.type === 'wpn' || d.type === 'arm' || d.type === 'acc') && ITEM_WEIGHTS[d.n] !== undefined) {
        _detail.push(`<span class="text-amber-300">重量: ${ITEM_WEIGHTS[d.n]}</span>`);
    }

    // 🔧 安定值 / 無法強化（武器/防具/飾品）
    if (d.type === 'wpn' || d.type === 'arm' || d.type === 'acc') {
        if (d.reqLv) _detail.push(`<span class="text-amber-300 font-bold">需求等級: Lv.${d.reqLv}</span>`);
        if (d.noEnhance) _detail.push(`<span class="text-rose-300 font-bold">無法強化</span>`);
        else _detail.push(`<span class="text-slate-400">安定值: ${d.safe || 0}</span>`);
    }

    if (_detail.length) {
        if (_compactHead) desc += `<br>${_detail.join('<br>')}`;
        else desc += `<details class="item-detail-fold"><summary>展開詳細資訊</summary><div>${_detail.join('<br>')}</div></details>`;
    }
    if (_compactHead) return `${_compactHead}<details class="item-detail-fold item-detail-full"><summary>展開完整能力</summary><div>${desc}</div></details>`;
    return desc;
}
...
function openModal(item, isEq, slot) {
    ...
    if (!isEq && _modalEquipItem && !_modalCanEquip) {
        let _reason = (d.reqLv && (player.lv || 1) < d.reqLv) ? `：需要 Lv.${d.reqLv}（目前 Lv.${player.lv || 1}）` : (d.reqAvatar ? `：僅限${d.reqAvatar}` : '');
        desc += `<br><span class="text-red-400 font-bold">無法裝備${_reason}</span>`;
    }
    ...
    const _modalDesc = document.getElementById('modal-item-desc');
    _modalDesc.innerHTML = desc;
    // 物品操作視窗的目的就是檢查裝備；完整能力預設展開，避免手機玩家只看見摘要與操作按鈕。
    // 背包／商店卡片仍沿用收合顯示，不會因此把清單撐長。
    _modalDesc.querySelectorAll('details.item-detail-full').forEach(_detail => { _detail.open = true; });
    ...
    if (isEq) {
        ...
        if(d.type === 'wpn' || d.type === 'arm' || d.type === 'acc') {
            if (_modalCanEquip) act += `<button class="col-span-2 w-full btn border-blue-700 bg-blue-900 hover:bg-blue-800 text-blue-200 py-3 text-lg font-bold" onclick="equipItem(${JSON.stringify(item).replace(/"/g, '&quot;')})">裝備</button>`;
                ...
                let _reason = (d.reqLv && (player.lv || 1) < d.reqLv) ? `・需要 Lv.${d.reqLv}` : (d.reqAvatar ? `・僅限${d.reqAvatar}` : '');
                act += `<button class="col-span-2 w-full btn border-red-900 bg-red-950 text-red-400 py-3 text-lg font-bold cursor-not-allowed" disabled>無法裝備${_reason}</button>`;
            }
        }
    ...
    if(_cmp) {
        const SLOT_LABEL = { wpn:'武器', offwpn:'副手武器', helm:'頭盔', armor:'盔甲', shin:'脛甲', shield:'副手', cloak:'斗篷', tshirt:'內衣', gloves:'手套', boots:'鞋子', ring1:'戒指 1', ring2:'戒指 2', ring3:'戒指 3', ring4:'戒指 4', amulet:'項鍊', ear1:'耳環 1', ear2:'耳環 2', belt:'腰帶', charm:'大型護符' };   // 🔥 v3.9.3 大型護符獨立欄
...
function getSellPrice(item) {
    ...
    if (item.anc)   mult *= 10;
    let result = price * mult;
    if (Number(item.gamblePaid) > 0) result = Math.min(result, Math.floor(Number(item.gamblePaid) * 0.15));
    return result;
}
...
const AUTOSELL_D2R_SPECIAL_AFFIX_CODES = new Set([
    'kx','ts','pi','ks','sp','fh','fpen','wpen','epen','apen','sdm','bdr','und','dem','dra','hsk','opn','exe','pdr','mdr','ldr','adr','udr','ddr',
...
function _autoSellDecision(i, ruleSnapshot, craftRemain) {   // 🔧 v2.6.77 ruleSnapshot：預覽用「快照複本」判定，不讀（也不寫）live 規則；v2.6.91 craftRemain：製作素材保留額度（逐件扣）
    ...
    if (et) {
        ...
        // 八色詞綴過濾是獨立規則：玩家只要啟用進階過濾就會處理全部八色裝備，不再被上方預設關閉的三個一般裝備開關擋住。
        if (d2rows.length && dr.on === true) {
...
const AUTOSELL_D2R_PRESETS = {
    safe:    {maxQuality:'rare',keepAffixCount:3,protectTopTier:true,topTierMax:2,protectSpecial:true,protectEnhance:true,enhanceAt:1,socketMode:'all'},
    balanced:{maxQuality:'epic',keepAffixCount:5,protectTopTier:true,topTierMax:2,protectSpecial:true,protectEnhance:true,enhanceAt:3,socketMode:'filled'},
    cleanup: {maxQuality:'legend',keepAffixCount:7,protectTopTier:true,topTierMax:1,protectSpecial:true,protectEnhance:true,enhanceAt:6,socketMode:'filled'}
};
function refreshAutoSellD2rSummary(){
    let out=document.getElementById('as-d2-summary');if(!out)return;
    if(!document.getElementById('as-d2-on').checked){out.textContent='目前未啟用：所有八色詞綴裝備都會保留。';return;}
    let q=document.getElementById('as-d2-quality'),quality=q.options[q.selectedIndex]?.text||q.value;
    let guards=[`詞綴達 ${document.getElementById('as-d2-count').value} 條`];
    if(document.getElementById('as-d2-tier-on').checked)guards.push(`含 ${document.getElementById('as-d2-tier').selectedOptions[0]?.text||'高階'} 詞綴`);
    if(document.getElementById('as-d2-special').checked)guards.push('特殊／職業技能詞綴');
    if(document.getElementById('as-d2-enhance').checked)guards.push(`強化 +${document.getElementById('as-d2-enhance-at').value} 以上`);
    let socketText={all:'任何已開孔裝備',filled:'已鑲嵌裝備',none:''}[document.getElementById('as-d2-sockets').value];if(socketText)guards.push(socketText);
    out.textContent=`販賣 ${quality} 以下的八色裝備；遇到「${guards.join('、')}」就保留。`;
}
function applyAutoSellD2rPreset(name){
    let p=AUTOSELL_D2R_PRESETS[name];if(!p)return;
    document.getElementById('as-d2-on').checked=true;
    document.getElementById('as-d2-quality').value=p.maxQuality;
    document.getElementById('as-d2-count').value=p.keepAffixCount;
    document.getElementById('as-d2-tier-on').checked=p.protectTopTier;
    document.getElementById('as-d2-tier').value=String(p.topTierMax);
    document.getElementById('as-d2-special').checked=p.protectSpecial;
    document.getElementById('as-d2-enhance').checked=p.protectEnhance;
    document.getElementById('as-d2-enhance-at').value=p.enhanceAt;
    document.getElementById('as-d2-sockets').value=p.socketMode;
    refreshAutoSellD2rSummary();
}
function openAutoSellRules() {
    ...
    let el=document.createElement('div'); el.id='autosell-rule-modal'; el.innerHTML=`<style>
      ...
      <div class="as-sec as-d2"><div class="as-title">八色裝備快速過濾</div><label class="as-row"><input id="as-d2-on" type="checkbox" ${r.d2r.on?'checked':''} onchange="refreshAutoSellD2rSummary()"> 啟用八色詞綴裝備自動販賣</label><div class="as-presets"><button type="button" onclick="applyAutoSellD2rPreset('safe')">🛡️ 保守</button><button type="button" onclick="applyAutoSellD2rPreset('balanced')">⚖️ 平衡</button><button type="button" onclick="applyAutoSellD2rPreset('cleanup')">🧹 清背包</button></div><div id="as-d2-summary" class="as-summary"></div><div class="as-inline-actions"><button type="button" onclick="previewAutoSellRules()">先看會賣什麼</button><button type="button" class="primary" onclick="saveAutoSellRules()">套用設定</button></div><details class="as-details"><summary>自訂細部條件</summary><label class="as-row">最高可販賣品質：<select id="as-d2-quality" onchange="refreshAutoSellD2rSummary()">${d2QualityRows}</select></label><label class="as-row">詞綴數量達 <input id="as-d2-count" type="number" min="1" max="10" value="${r.d2r.keepAffixCount}" oninput="refreshAutoSellD2rSummary()"> 條以上時保留</label><label class="as-row"><input id="as-d2-tier-on" type="checkbox" ${r.d2r.protectTopTier!==false?'checked':''} onchange="refreshAutoSellD2rSummary()"> 保護含有 <select id="as-d2-tier" onchange="refreshAutoSellD2rSummary()">${d2TierRows}</select> 詞綴的裝備</label><label class="as-row"><input id="as-d2-special" type="checkbox" ${r.d2r.protectSpecial!==false?'checked':''} onchange="refreshAutoSellD2rSummary()"> 保護特殊玩法／職業技能詞綴</label><label class="as-row"><input id="as-d2-enhance" type="checkbox" ${r.d2r.protectEnhance!==false?'checked':''} onchange="refreshAutoSellD2rSummary()"> 保護強化值達 <input id="as-d2-enhance-at" type="number" min="1" max="99" value="${r.d2r.enhanceAt}" oninput="refreshAutoSellD2rSummary()"> 以上的裝備</label><label class="as-row">孔洞保護：<select id="as-d2-sockets" onchange="refreshAutoSellD2rSummary()">${d2SocketRows}</select></label></details><div class="as-help">第一次建議按「保守」再預覽；八色過濾獨立生效，不必設定下面的一般裝備條件。</div></div>
    ...
    document.body.appendChild(el);refreshAutoSellD2rSummary();
}
...
function previewAutoSellRules(){
    ...
    if(!a.length){body.textContent=previewRules.d2r&&previewRules.d2r.on?'目前沒有可販賣的物品；背包裝備可能都被品質、詞綴、強化或孔洞條件保護。可回上一頁改用「平衡」或「清背包」後再預覽。':'目前沒有符合規則的物品；若要清理八色裝備，請先按「保守／平衡／清背包」其中一個方案。';}
    else {
