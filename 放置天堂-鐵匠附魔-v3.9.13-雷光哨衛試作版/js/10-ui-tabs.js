...
function buildItemDescHTML(item) {
    ...
    if (_d2rows.length) {
        ...
        let _represent = _d2rows.slice().sort((a,b)=>a[2]-b[2]||b[1]-a[1]).slice(0,2)
            .map(r=>`${D2R_SHORT_LABEL[r[0]] || D2R_AFFIX_LABEL[r[0]]}${r[0]==='ac'?'-':'+'}${_effVal(r)}${['mc','rc','gc','mcd','rcd','gcd','hpp','mpp','pot','abr','gf','xf','nd','bd','kx','ts','pi','ks','sp','fh','hy','hd','st','gd','scd','ww','wd','wr','wc','ld','ls','lc','lr','bh','bt','bc','med','mes','mec','rbd','rbf','rbc','ard','arc','acd','csd','csp','csc','mzd','mzt','mzc','trd','trt','trc'].includes(r[0])?'%':''}`).join('｜');
