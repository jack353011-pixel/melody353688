...
// 🎲 D2R 賭裝商店：固定裝備改成未知部位；不受 MF 影響、最高只出 T2，避免取代高階狩獵。
const D2_GAMBLE_CATEGORIES = [
    { k:'melee', n:'近戰武器', icon:'⚔️', base:15000, test:d => d.type === 'wpn' && !d.isArrow && !d.ranged && !d.isBow && !d.qigu && !(typeof isWandWeapon === 'function' && isWandWeapon(d)) },
    { k:'ranged', n:'遠程武器', icon:'🏹', base:15000, test:d => d.type === 'wpn' && !d.isArrow && !!(d.ranged || d.isBow) },
    { k:'magic', n:'魔法武器', icon:'🪄', base:15000, test:d => d.type === 'wpn' && !d.isArrow && !!(d.qigu || d.mdmg || d.magic || (typeof isWandWeapon === 'function' && isWandWeapon(d))) },
    { k:'helm', n:'頭盔', icon:'🪖', base:8000, test:d => d.type === 'arm' && d.slot === 'helm' },
    { k:'armor', n:'盔甲', icon:'🥋', base:12000, test:d => d.type === 'arm' && d.slot === 'armor' },
    { k:'gloves', n:'手套', icon:'🧤', base:6000, test:d => d.type === 'arm' && d.slot === 'gloves' },
    { k:'boots', n:'長靴', icon:'🥾', base:6000, test:d => d.type === 'arm' && d.slot === 'boots' },
    { k:'cloak', n:'斗篷', icon:'🧥', base:6000, test:d => d.type === 'arm' && d.slot === 'cloak' },
    { k:'shield', n:'盾牌', icon:'🛡️', base:8000, test:d => d.type === 'arm' && d.slot === 'shield' },
    { k:'accessory', n:'飾品', icon:'💍', base:20000, test:d => d.type === 'acc' && !d.doll }
];
let _d2GambleBusy = false;
function d2GambleEffectiveLevel() {
    return Math.max(1, Math.min(100, Math.floor(Number(player.lv) || 1) + Math.max(0, Math.floor(Number(player.rebirthCount) || 0)) * 10));
}
function d2GamblePool(cat) {
    if (!cat) return [];
    return Object.keys(DB.items).filter(id => {
        let d = DB.items[id];
        if (!d || !cat.test(d) || isRelic(d) || d.legend || d.virtual || d.noSell || !(Number(d.p) > 0) || Number(d.gachaWeight) === 1) return false;
        try { return checkCanEquip({ id:id }); } catch(e) { return false; }
    });
}
function d2GamblePrice(cat, effectiveLevel) {
    let scale = Math.pow(1 + effectiveLevel / 20, 2);
    return Math.max(1, shopPrice(Math.round(cat.base * scale)));
}
function d2GamblePity() {
    if (!player.d2Gamble || typeof player.d2Gamble !== 'object') player.d2Gamble = {};
    ['noExcellent','noEpic','noLegend'].forEach(k => player.d2Gamble[k] = Math.max(0, Math.floor(Number(player.d2Gamble[k]) || 0)));
    return player.d2Gamble;
}
function d2GambleRollQuality(effectiveLevel) {
    let maxIndex = effectiveLevel < 20 ? 1 : (effectiveLevel < 40 ? 2 : (effectiveLevel < 60 ? 3 : (effectiveLevel < 80 ? 4 : 5)));
    let pity = d2GamblePity(), forced = '';
    if (maxIndex >= 4 && pity.noLegend >= 99) forced = 'legend';
    else if (maxIndex >= 3 && pity.noEpic >= 29) forced = 'epic';
    else if (maxIndex >= 2 && pity.noExcellent >= 7) forced = 'excellent';
    let keys = ['magic','rare','excellent','epic','legend','mythic'];
    let weights = [20,32,30,15,2.5,0.5].slice(0, maxIndex + 1);
    let quality = forced;
    if (!quality) {
        let total = weights.reduce((s,w) => s + w, 0), roll = lootRng('d2gamble-quality') * total, index = 0;
        for (; index < weights.length - 1 && roll >= weights[index]; index++) roll -= weights[index];
        quality = keys[index];
    }
    let qi = keys.indexOf(quality);
    pity.noExcellent = qi >= 2 ? 0 : pity.noExcellent + 1;
    pity.noEpic = qi >= 3 ? 0 : pity.noEpic + 1;
    pity.noLegend = qi >= 4 ? 0 : pity.noLegend + 1;
    return quality;
}
function d2GamblePanelHTML() {
    let effectiveLevel = d2GambleEffectiveLevel(), pity = d2GamblePity();
    let cards = D2_GAMBLE_CATEGORIES.map(cat => {
        let price = d2GamblePrice(cat, effectiveLevel), empty = !d2GamblePool(cat).length, poor = Number(player.gold) < price;
        return `<button class="d2-gamble-card" onclick="buyD2Gamble('${cat.k}')" ${empty || poor ? 'disabled' : ''}>
            <span class="d2-gamble-icon">${cat.icon}</span><span class="font-bold">${cat.n}</span>
            <span class="text-yellow-300 text-xs">${price.toLocaleString()} 金幣</span>
        </button>`;
    }).join('');
    return `<section class="d2-gamble-shop">
        <div class="flex justify-between items-start gap-3 mb-2"><div><div class="text-amber-300 font-bold text-lg">🎲 神秘裝備賭局</div>
        <div class="text-slate-400 text-xs">有效等級 ${effectiveLevel}｜隨機基底與八色詞綴｜最高 T2｜不受物品發現率影響</div></div>
        <div class="text-right text-xs text-slate-400 shrink-0">卓越保底 ${Math.min(8,pity.noExcellent)}/8<br>史詩 ${Math.min(30,pity.noEpic)}/30・傳說 ${Math.min(100,pity.noLegend)}/100</div></div>
        <div class="d2-gamble-grid">${cards}</div>
        <div class="text-slate-500 text-xs mt-2">Lv.20／40／60／80 依序開放卓越／史詩／傳說／神話；不朽與太古只能由狩獵取得。賭裝回收價最高為購價 15%。</div>
    </section>`;
}
function buyD2Gamble(categoryKey) {
    if (_d2GambleBusy) return;
    let cat = D2_GAMBLE_CATEGORIES.find(x => x.k === categoryKey);
    if (!cat) return;
    let effectiveLevel = d2GambleEffectiveLevel(), price = d2GamblePrice(cat, effectiveLevel), pool = d2GamblePool(cat);
    if (!pool.length) { logSys('目前沒有適合你的此類裝備。'); return; }
    if (Number(player.gold) < price) { logSys('金幣不足。'); return; }
    _d2GambleBusy = true;
    try {
        let quality = d2GambleRollQuality(effectiveLevel);
        let id = pool[Math.floor(lootRng('d2gamble-base') * pool.length)];
        let rolled = d2rRollAffixes(DB.items[id], { quality:quality, level:effectiveLevel, bestTier:2, ignoreMf:true });
        let socketRoll = typeof d2rRollSockets === 'function' ? d2rRollSockets(DB.items[id],quality,{chanceMult:.5}) : null;
        if (!rolled || !rolled.rows.length) throw new Error('賭裝詞綴生成失敗');
        player.gold -= price;
        let item = gainItem(id, 1, true, false, false, false, { d2q:quality, d2:rolled.rows, gamblePaid:price, socketMax:socketRoll && socketRoll.socketMax, sockets:socketRoll && socketRoll.sockets });
        logSys(`<span class="${d2rQualityDef(quality).c} font-bold">🎲 賭得 ${getItemFullName(item)}</span>`);
        saveGame();
        updateUI();
        renderShopItems();
    } catch(e) {
        logSys(`<span class="text-red-400">賭裝失敗：${String(e && e.message || e)}</span>`);
    } finally {
        _d2GambleBusy = false;
    }
}
...
function getShopItemsForNpc(npcId) {
    ...
    return list.filter(id => {
        let d = DB.items[id];
        if (!d) return false;
        // 固定裝備改由上方 D2R 神秘部位賭局取代；箭矢仍是雜貨商固定商品。
        if (d.type === 'wpn' || d.type === 'arm' || d.type === 'acc') return !!d.isArrow;
...
function renderShopItems() {
    ...
    if(!listDiv) return;
    listDiv.innerHTML = d2GamblePanelHTML();

    let ids = getShopItemsForNpc(_currentShopNpc);
    if(!ids.length) {
        listDiv.insertAdjacentHTML('beforeend', '<div class="text-slate-500 text-sm text-center py-5">此商人沒有其他固定商品。</div>');
        return;
    }
...
// 🌙 v3.9.13 離線掛網已重新啟用：只有玩家在外掛面板明確關閉功能時，才清掉離線快照。
//    未設定開關代表預設開啟；此判斷必須和 js/27 的 `afk_toggle_offline` 規則一致。
function _offlineFeatureEnabledForStorage(){
    try { return typeof localStorage === 'undefined' || localStorage.getItem('afk_toggle_offline') !== '0'; }
    catch(e) { return true; }
}
...
// 🌙 v3.9.13 選角掛網狀態：存檔摘要只在 render 時解一次；之後每 2 秒只讀角色專屬 checkpoint。
//    綠燈直接使用 js/27 真正結算所讀的 eligible/profile/claim，避免畫面說成功、實際卻領不到。
function _slotBadgeMeta(n, sum){
    ...
    return {
        slot: n,
        roleFp: sum.roleFp,
        mercWho: { cls: sum.rawCls, name: sum.name, enSeed: sum.enSeed, classic: !!sum.classic },
        offlineWho: { slot: n, cls: sum.rawCls, name: sum.name, enSeed: sum.enSeed, currentMap: sum.currentMap || '' }
    };
}
...
function _slotRoleActiveNow(meta){
    if(!meta || !meta.roleFp || typeof _roleReadObject !== 'function' || typeof _rolePruneSessions !== 'function') return false;
    try {
        let reg = _rolePruneSessions(_roleReadObject(ROLE_SESSION_REGISTRY_KEY));
        return Object.keys(reg).some(id => id !== _roleSessionId && reg[id] && reg[id].fp === meta.roleFp);
    } catch(e){ return false; }
}
function _slotOfflineStatusNow(meta){
    if(!meta || !meta.offlineWho) return null;
    try {
        if(window.AFK_TOGGLES && !window.AFK_TOGGLES.enabled('slotinfo')) return null;
        if(_slotRoleActiveNow(meta)) return { state:'playing', mapName:'', elapsedMs:0, capMs:0 };
        if(typeof window.offlineHuntStatusForCharacter === 'function') return window.offlineHuntStatusForCharacter(meta.offlineWho);
    } catch(e){}
    return null;
}
function _slotOfflineDuration(ms){
    ...
    if(mins < 1) return '剛開始';
    if(mins < 60) return mins + ' 分鐘';
    let hours = Math.floor(mins / 60), remain = mins % 60;
    if(hours < 24) return hours + ' 小時' + (remain ? ' ' + remain + ' 分' : '');
    let days = Math.floor(hours / 24), remainHours = hours % 24;
    return days + ' 天' + (remainHours ? ' ' + remainHours + ' 小時' : '');
}
function _slotOfflineDetailHtml(status, party){
    if(!status && !party) return '';
    // 傭兵身分由頭頂紫色徽章獨立顯示，不再覆蓋卡片底部的綠色離線掛網狀態。
    let stateName = String(status && status.state || (party ? 'mercenary' : 'none'));
    let label = '未掛網', detail = '前往狩獵區打怪即可建立掛網';
    if(stateName === 'ready'){
        label = '● 掛網中';
        detail = (status.mapName || '狩獵區') + '｜' + _slotOfflineDuration(status.elapsedMs);
        if(status.capMs > 0 && status.elapsedMs >= status.capMs) detail += '（收益已達上限）';
    } else if(stateName === 'sampling'){
        label = '● 尚未掛網成功'; detail = '需戰鬥 15 秒並擊殺至少 3 隻';
    } else if(stateName === 'dead'){
        label = '● 掛網失敗'; detail = '角色已死亡，復活後請重新建立';
    } else if(stateName === 'mercenary'){
        label = '● 擔任傭兵'; detail = '仍可使用自己的角色進行離線掛網';
    } else if(stateName === 'playing'){
        label = '● 遊戲中'; detail = '角色正在其他分頁執行';
    } else if(stateName === 'claimed'){
        label = '● 已完成結算'; detail = '進入狩獵區可重新開始掛網';
    } else if(stateName === 'restricted'){
        label = '● 此地圖不可掛網'; detail = status.mapName || '請更換一般狩獵地圖';
    } else if(stateName === 'blocked'){
        label = '● 掛網資格受限'; detail = '請先解除目前地圖的限制';
    } else if(stateName === 'stale'){
        label = '● 掛網已中斷'; detail = '太久沒有擊殺怪物，請重新戰鬥';
    }
    return `<span class="load-offline-detail load-offline-${loadEsc(stateName)}" data-offline-state="${loadEsc(stateName)}"><strong>${loadEsc(label)}</strong><small>${loadEsc(detail)}</small></span>`;
}
// 頭頂保留傭兵徽章；卡片底部顯示真正離線 checkpoint 的狀態、地圖與累積時間。
function _slotBadgeHtml(party, offline){
    let partyHtml = party ? `<span class="load-slot-badge load-badge-party"><strong>擔任傭兵</strong></span>` : '';
    let offlineHtml = _slotOfflineDetailHtml(offline, party);
    return partyHtml || offlineHtml ? `<span class="load-slot-status">${partyHtml}${offlineHtml}</span>` : '';
}
...
function _summaryFromRaw(s){
    ...
    try { let d = JSON.parse(s); let p = d.p;
        ...
        return {
            ...
            currentMap: d.ms && d.ms.current ? String(d.ms.current) : '',
...
const SAVE_SLOT_MAX = 16;   // 外掛相容：選角、匯入檢查、傭兵、血盟與 PVP 共用的存檔位上限。
...
// 🔄 登入畫面徽章活刷：只重算徽章、原地增刪 span——不整頁重繪（不打斷選取與立繪動畫）。
//    掛網時間、其他分頁遊戲中與傭兵狀態都會在 2 秒內同步。
setInterval(function(){
    ...
    grid.querySelectorAll('.load-slot-card[data-slot]').forEach(btn => {
        ...
        let party = _slotPartyStatusNow(meta);
        let offline = _slotOfflineStatusNow(meta);
        let old = btn.querySelector('.load-slot-status');
        let html = _slotBadgeHtml(party, offline);
        if(html){
...
function returnToCharacterSelect(){
    ...
    // 先由離線模組寫入「可否掛網」快照並脫離目前角色；否則進選角後的關頁事件會以 state.running=false
    // 把剛建立成功的 checkpoint 覆蓋成未掛網。沒有載入離線模組時才退回一般最終存檔。
    if(typeof window.offlinePrepareCharacterSelect === 'function'){
        try { window.offlinePrepareCharacterSelect(); } catch(e){ _flushSaveNow(); }
    } else _flushSaveNow();
...
function renderLoadSelect(){
    ...
    for(let n = start; n <= start + 3; n++){
        ...
        html += `<button type="button" onclick="loadSelectSlot(${n})" data-slot="${n}" data-key="${key}" class="load-slot-card ${selected ? 'selected' : ''} ${empty ? 'empty' : 'filled'}" title="${loadEsc(title)}">`
            ...
            + _slotBadgeHtml(_slotPartyStatusNow(_loadSlotMeta[n]), _slotOfflineStatusNow(_loadSlotMeta[n]))
            + `</button>`;
    }
    grid.innerHTML = html;
...
// 🤝 受僱中角色「經驗只增不減」防護：受僱角色現在也能使用自己的存檔外出練功，
//    同時僱主仍可能把傭兵待領經驗寫回。若另一情境（別分頁／先前一次領取）已把「磁碟上的」
//    經驗墊高，本情境用較舊的記憶體快照存回會覆蓋掉墊高的經驗＝待領經驗遺失。對策：存檔序列化前，
//    若目前正擔任傭兵且磁碟等級/經驗高於記憶體 → 吸收磁碟較高值，
//    保證存回只增不減（不以舊經驗覆蓋主玩家攜帶存回的經驗）。非受僱角色不受影響（閘門快取 false 直接早退）。
function _mercMonotonicExpGuard() {
    try {
        if (typeof currentRoleIsMercenary !== 'function' || !currentRoleIsMercenary()) return;
...
function _autoBackupReadMeta(slot) {
    try {
        ...
        return Array.isArray(meta) ? meta.filter(x => x && Number.isFinite(Number(x.at))).slice(0, AUTO_BACKUP_KEEP) : [];
    } catch(e) { return []; }
}
function _autoBackupCapture(slot) {
    try {
        slot = Math.max(1, Math.floor(Number(slot) || 1));
        let sourceKey = 'lineage_idle_save_' + slot;
        let raw = _lsGet(sourceKey);
        if(!raw) return false;
        let verified = _saveUnwrap(_lzGet(sourceKey));
        ...
        for(let i = AUTO_BACKUP_KEEP - 1; i >= 1; i--) {
            let older = _lsGet(_autoBackupPrefix(slot) + i);
            if(older != null) _lzSetStoredRaw(_autoBackupPrefix(slot) + (i + 1), older);
        }
        _lzSetStoredRaw(_autoBackupPrefix(slot) + '1', raw);
        meta.unshift({ at: now, lv: Math.max(1, Math.floor(Number(parsed.p.lv) || 1)), name: String(parsed.p.name || '') });
        meta = meta.slice(0, AUTO_BACKUP_KEEP);
        _lsSet(_autoBackupMetaKey(slot), JSON.stringify(meta));
        return true;
    } catch(e) {
        try { console.warn('[auto backup] capture failed', e); } catch(_e) {}
        return false;
    }
}
...
function saveGame() {
    ...
    _autoBackupCapture(currentSlot);   // 先保存上一份有效進度，再覆寫主存檔
...
function loadGame() {
    ...
    if (s) {
        ...
        // 🌙 離線功能開啟時保留逐地圖速率與生存樣本；玩家明確關閉功能才移除舊資料。
        if(!_offlineFeatureEnabledForStorage()) delete player.offlineHunt;
        ...
        ['wpn','arrow','helm','armor','shin','shield','cloak','tshirt','gloves','boots','ring1','ring2','ring3','ring4','amulet','belt'].forEach(_sl => {
            ...
            let _ok = true; try { _ok = checkCanEquip(_e, true); } catch(err) { _ok = true; }
            if (!_ok) {
...
// 配點/萬能藥的「自然屬性值」：基礎+配點+萬能藥（不含裝備與 buff）；v3.8.39 起不設上限
