...
// 👇 新增海音與歐瑞製作 NPC 的配方
    ...
    // 🔷🔶 象牙塔・神秘的魔法師：三鑰匙合成地獄火炬＋既有魔杖客製配方（見 MYSTICWAND_RECIPES）
    'npc_mystic_mage': [
        { result: 'item_hellfire_torch_unidentified', req: [{ id: 'mat_key_terror', cnt: 1 }, { id: 'mat_key_hatred', cnt: 1 }, { id: 'mat_key_destruction', cnt: 1 }] }
    ],
...
function pandoraStockBless(id) {
    let d = DB.items[id];
    if (!d || d.noAffix || (typeof isRelic === 'function' ? isRelic(d) : d.relic)) return false;
...
(function(){
    ...
    const STAT_LABEL = { ac:'AC', mr:'魔防(MR)', dr:'傷害減免', er:'迴避(ER)', str:'力量', dex:'敏捷', con:'體質', int:'智力', wis:'精神', cha:'魅力', mhp:'HP上限', mmp:'MP上限', mmpPct:'MP上限%', hpR:'HP恢復', mpR:'MP恢復', resFire:'火屬性抗性', resWater:'水屬性抗性', resEarth:'地屬性抗性', resWind:'風屬性抗性', meleeHit:'近距離命中', rangedHit:'遠距離命中', meleeDmg:'近距離傷害', rangedDmg:'遠距離傷害', mdmg:'魔法傷害', extraHit:'額外命中', extraDmg:'額外傷害' };
    ...
    function buildItemTipHTML(id, hidePrice){
        ...
        if(d.type === 'wpn'){
            ...
            let st = [];
            ['ac','mr','dr','er','str','dex','con','int','wis','cha','mhp','mmp','mmpPct','hpR','mpR','resFire','resWater','resEarth','resWind','meleeHit','rangedHit','meleeDmg','rangedDmg','mdmg','extraHit','extraDmg'].forEach(k => {
                if(d[k] !== undefined && d[k] !== 0) st.push(`${STAT_LABEL[k]||k} ${sgn(k === 'ac' ? -d[k] : d[k])}${k === 'mmpPct' ? '%' : ''}`);   // 🔧 AC 顯示取負（ac:3 ＝ 防禦 AC-3，越低越好），與背包資訊欄一致
            });
