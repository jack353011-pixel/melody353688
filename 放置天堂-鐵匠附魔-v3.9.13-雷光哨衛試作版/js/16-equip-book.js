...
function equipCatKey(id, d) {
    ...
    if (d.type === 'acc') {
        ...
        if (d.slot === 'charm') return 'charm';   // 🔥 地獄火炬；不設全收集加成，避免八職業版本再膨脹免費戰力
...
function buildEquipIndex() {
    Object.keys(EQUIP_CAT_ITEMS).forEach(k => delete EQUIP_CAT_ITEMS[k]);
    Object.keys(EQUIP_ITEM_CAT).forEach(k => delete EQUIP_ITEM_CAT[k]);
...
buildEquipIndex();
window.addEventListener('load', buildEquipIndex);
