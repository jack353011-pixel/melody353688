...
(function () {
    const PAGE_SLOTS = [
        ...
        [
            ...
            { k: 'charm',     x: 80.1, y: 15.81, w: 19.67, h: 8.82 },   // 🔥 v3.9.3 大型護符欄（地獄火炬專用；不占戒指／項鍊）
    ...
    function renderSlots() {
        ...
        PAGE_SLOTS[page].forEach(pos => {
            ...
            if (item && data) {
                ...
                img.loading = 'lazy';
                img.decoding = 'async';
                img.onerror = function () { retryIconImage(this); };
    ...
    function init() {
        ...
        const mobileBack = el('equipment-mobile-back');
        if (mobileBack) mobileBack.onclick = function () {
            const statsButton = el('tab-btn-stats');
            if (statsButton && typeof switchTab === 'function') switchTab('stats', statsButton);
            requestAnimationFrame(function () {
                if (typeof mobileJumpTo === 'function') mobileJumpTo('col-right');
                else document.getElementById('col-right')?.scrollIntoView({ block: 'start' });
            });
        };
