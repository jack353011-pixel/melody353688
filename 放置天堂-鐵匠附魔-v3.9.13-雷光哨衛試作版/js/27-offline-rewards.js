...
(function () {
    ...
    // 外掛面板的「離線快速結算」開關。AFK_TOGGLES 在核心腳本後載入，因此這裡直接讀取
    // 同一個 localStorage key；未設定時維持預設開啟。關閉後重新整理即可完全不安裝任何 wrapper。
    try {
    ...
    const _offlineClaimOwner = 'oc_' + Date.now().toString(36) + '_' + Math.random().toString(36).slice(2);
    ...
    // 選角畫面不能借用目前記憶體中的 player（那會是上一個角色），因此用存檔摘要的永久 enSeed
    // 直接定位同一份 checkpoint。舊存檔尚未寫入 enSeed 時，沿用 loadGame 的決定論補種公式。
    function _offlineIdentityForCharacter(meta) {
        if (!meta || typeof meta !== 'object') return '';
        let raw = String(meta.enSeed || '').trim();
        if (!raw && typeof _seedHash === 'function' && meta.name && meta.cls) {
            raw = 'es' + _seedHash(String(meta.name) + '|' + String(meta.cls) + '|lz').toString(36);
        }
        if (!raw && meta.slot && meta.cls) raw = String(meta.slot) + '|' + String(meta.name || '') + '|' + String(meta.cls);
        return raw ? encodeURIComponent(raw) : '';
    }

    function _offlineStoreKeyForCharacter(kind, meta) {
        let id = _offlineIdentityForCharacter(meta);
        return id ? OFFLINE_STORE_PREFIX + kind + '_' + id : '';
    }
    ...
    // 供選角畫面使用的唯讀狀態 API。它只回傳顯示所需欄位，不修改 checkpoint，也不提前結算收益。
    // `ready` 的判斷刻意與 _offlineSettle 使用同一份 eligible/profile/claim 資料，綠燈就代表真的可累積。
    function _offlineStatusForCharacter(meta) {
        let checkpoint = _offlineReadJson(_offlineStoreKeyForCharacter('checkpoint', meta));
        if (!checkpoint) return { state: 'none', map: '', mapName: '', elapsedMs: 0, capMs: OFFLINE_MAX_MS };

        let now = _offlineNow();
        let cpActive = Math.max(0, Math.floor(_offlineFinite(checkpoint.lastActive, 0)));
        if (cpActive > now + 5 * 60 * 1000) cpActive = now;
        let snapshot = checkpoint.snapshot && typeof checkpoint.snapshot === 'object' ? checkpoint.snapshot : null;
        let display = checkpoint.display && typeof checkpoint.display === 'object' ? checkpoint.display : null;
        let claim = _offlineReadJson(_offlineStoreKeyForCharacter('claim', meta));
        // pending 只是跨分頁的短暫互斥鎖，尚未代表獎勵已成功寫入角色存檔。
        let claimAt = claim && claim.status !== 'pending' ? Math.max(0, Math.floor(_offlineFinite(claim.claimedUntil, 0))) : 0;
        // checkpoint 與角色存檔一起成功後，其內部 claim 仍可在外部索引暫時寫入失敗時阻止選角畫面誤報。
        let snapshotClaimAt = snapshot ? Math.max(0, Math.floor(_offlineFinite(snapshot.claimedUntil, 0))) : 0;
        claimAt = Math.max(claimAt, snapshotClaimAt);
        if (claimAt > now + 5 * 60 * 1000) claimAt = now;

        if (snapshot && snapshot.eligible === true) {
            let profile = _offlineProfile(snapshot.profile);
            let map = snapshot.map ? String(snapshot.map) : '';
            if (profile && map && profile.map === map && profile.killsPerMin > 0) {
                if (claimAt > 0 && claimAt >= cpActive) {
                    return { state: 'claimed', map: map, mapName: String(snapshot.mapName || profile.mapName || _offlineMapName(map)), elapsedMs: 0, capMs: OFFLINE_MAX_MS };
                }
                let from = Math.max(0, Math.floor(_offlineFinite(snapshot.awaySince, cpActive)), cpActive, claimAt);
                if (from > now) from = now;
                return {
                    state: 'ready',
                    map: map,
                    mapName: String(snapshot.mapName || profile.mapName || _offlineMapName(map)),
                    elapsedMs: Math.max(0, now - from),
                    capMs: OFFLINE_MAX_MS
                };
            }
        }

        let fallbackMap = String((display && display.map) || (meta && meta.currentMap) || '');
        let stateName = display && display.state ? String(display.state) : '';
        if (!stateName) {
            if (fallbackMap && _offlineValidHuntMap(fallbackMap)) stateName = 'sampling';
            else if (fallbackMap && String(fallbackMap).indexOf('town_') !== 0) stateName = 'restricted';
            else stateName = 'none';
        }
        return {
            state: stateName,
            map: fallbackMap,
            mapName: String((display && display.mapName) || (fallbackMap ? _offlineMapName(fallbackMap) : '')),
            elapsedMs: 0,
            capMs: OFFLINE_MAX_MS
        };
    }

    window.offlineHuntStatusForCharacter = _offlineStatusForCharacter;
    ...
    function _offlineEnsureState() {
        ...
        player.offlineHunt = {
            ...
            claimedUntil: Math.max(0, Math.floor(_offlineFinite(raw.claimedUntil, 0))),
    ...
    // 每次載入角色或切換地圖後都必須用「本次執行階段」重新證明戰力，不能只靠永久保存的舊效率樣本。
    // 一般狩獵維持介面承諾的 15 秒＋3 隻；純頭目房沒有一般怪，改成本次至少實際擊敗 1 隻頭目。
    function _offlineRuntimeQualified(now, profile, map, difficulty) {
        let rt = _offlineRuntime;
        map = String(map || '');
        difficulty = difficulty || _offlineCurrentDifficulty();
        if (!rt || rt.map !== map || rt.difficulty !== difficulty) return false;
        if (profile && profile.bossRoom === true) {
            return rt.bossKills > 0 && rt.lastBossAt > 0 && now - rt.lastBossAt <= OFFLINE_RECENT_KILL_MS;
        }
        return rt.firstKillAt > 0 && rt.kills >= OFFLINE_SAMPLE_MIN_KILLS &&
            now - rt.firstKillAt >= OFFLINE_SAMPLE_MIN_MS && rt.lastKillAt > 0 &&
            now - rt.lastKillAt <= OFFLINE_RECENT_KILL_MS;
    }

    function _offlineRuntimeStale(now, profile, map, difficulty) {
        let rt = _offlineRuntime;
        map = String(map || '');
        difficulty = difficulty || _offlineCurrentDifficulty();
        if (!rt || rt.map !== map || rt.difficulty !== difficulty) return false;
        let last = profile && profile.bossRoom === true ? rt.lastBossAt : rt.lastKillAt;
        return last > 0 && now - last > OFFLINE_RECENT_KILL_MS;
    }
    ...
    function _offlineCanSnapshot(now, profile) {
        ...
        // 受僱只代表其他角色正在使用這份戰力快照；來源角色本身仍可建立自己的離線狩獵。
        // 傭兵端是否能取得經驗由 mercenarySourceOwnProgressActive 單獨處理，不能在此封鎖整份離線收益。
        ...
        return _offlineRuntimeQualified(now, profile, mapState.current, difficulty);
    }

    function _offlineSnapshotDisplay(now, profile, eligible) {
        let map = typeof mapState !== 'undefined' && mapState ? String(mapState.current || '') : '';
        let mapName = map ? _offlineMapName(map) : '';
        if (eligible) return { state: 'ready', map: map, mapName: mapName };
        if (typeof player !== 'undefined' && player && player.dead) return { state: 'dead', map: map, mapName: mapName };
        if (typeof state === 'undefined' || !state || !state.running) return { state: 'none', map: map, mapName: mapName };
        if (!_offlineValidHuntMap(map)) return { state: String(map).indexOf('town_') === 0 ? 'none' : 'restricted', map: map, mapName: mapName };
        ...
        if (_offlineRuntimeStale(now, profile, map, difficulty)) {
            return { state: 'stale', map: map, mapName: mapName };
        }
        if (!_offlineRuntimeQualified(now, profile, map, difficulty)) return { state: 'sampling', map: map, mapName: mapName };
        return { state: 'blocked', map: map, mapName: mapName };
    }
    ...
    function _offlineRecordBossKill(mob, map, expGain, goldGain, petExpGain, allyExpGain, now) {
        ...
        let difficulty = _offlineCurrentDifficulty();
        if (!_offlineRuntime || _offlineRuntime.map !== String(map) || _offlineRuntime.difficulty !== difficulty) _offlineResetRuntime(map);
        _offlineRuntime.bossKills++;
        _offlineRuntime.lastBossAt = now;
        ...
        try {
            if (typeof _offlineOriginalSaveGame === 'function' && _offlineOriginalSaveGame() === true) {
    ...
    function _offlinePrepareSnapshot(now, closingPage) {
        ...
        let checkpoint = {
            ...
            display: _offlineSnapshotDisplay(activityNow, profile, eligible)
        };
    ...
    function _offlineClaimTime(value, now) {
        let t = Math.max(0, Math.floor(_offlineFinite(value, 0)));
        return t > now + 5 * 60 * 1000 ? now : t;
    }

    function _offlineReadClaimAt(now) {
        let obj = _offlineReadJson(_offlineStoreKey('claim'));
        let external = obj && obj.status !== 'pending' ? _offlineClaimTime(obj.claimedUntil, now) : 0;
        // 與獎勵一起存進角色資料的 committed 時間才是原子單一真相；外部 claim 只負責跨分頁互斥與選角顯示。
        let internal = typeof player !== 'undefined' && player && player.offlineHunt ?
            _offlineClaimTime(player.offlineHunt.claimedUntil, now) : 0;
        return Math.max(external, internal);
    }

    function _offlineAcquireClaim(now) {
        let key = _offlineStoreKey('claim');
        if (!key) return null;
        let previous = _offlineReadJson(key);
        if (previous && previous.status === 'pending' && previous.owner !== _offlineClaimOwner &&
            _offlineClaimTime(previous.lockUntil, now) > now) return null;
        let token = _offlineClaimOwner + '_' + now.toString(36) + '_' + Math.random().toString(36).slice(2);
        let pending = { v: OFFLINE_VERSION, status: 'pending', owner: _offlineClaimOwner, token: token,
            claimedUntil: now, lockUntil: now + OFFLINE_CLAIM_LOCK_MS };
        if (!_offlineWriteJson(key, pending)) return null;
        let verify = _offlineReadJson(key);
        if (!verify || verify.status !== 'pending' || verify.token !== token) return null;
        return {
            key: key,
            token: token,
            previous: previous,
            previousPlayerClaim: typeof player !== 'undefined' && player && player.offlineHunt ?
                Math.max(0, Math.floor(_offlineFinite(player.offlineHunt.claimedUntil, 0))) : 0,
            checkpointKey: _offlineStoreKey('checkpoint'),
            previousCheckpoint: _offlineReadJson(_offlineStoreKey('checkpoint'))
        };
    }
    ...
    function _offlineReleaseClaim(handle, restoreCheckpoint) {
        if (!handle) return false;
        let current = _offlineReadJson(handle.key);
        let owned = current && current.status === 'pending' && current.token === handle.token;
        if (owned) {
    ...
    function _offlineApplyAllyExp(amount) {
        ...
        player.allies.forEach(a => {
            ...
            // 來源角色正以自己的分頁或離線快照累積進度時，傭兵只提供戰力、不再取得第二份經驗。
            if (typeof mercenarySourceOwnProgressActive === 'function' && mercenarySourceOwnProgressActive(a)) return;
    ...
    function _offlineRollMobLoot(mob, kills, map, loot) {
        ...
        let dropMult = dropBase * classic * party * fixedDrop;
        let oldSherine = _sherineLootCtx;
        let oldForceBless = _forceBless;
        try {
            _sherineLootCtx = mob.sherine ? { mad: !!mob.sherineMad } : null;
            if (typeof MOB_DROPS !== 'undefined') _offlineRollTable(mob, kills, MOB_DROPS, dropBase * party * fixedDrop, loot, true);
    ...
    function _offlineSettle(reason) {
        ...
        try {
            ...
            // 先取得短效跨分頁鎖；真正的領取時間與獎勵一起寫入角色存檔後才提交。
            let claimHandle = _offlineAcquireClaim(now);
            if (!claimHandle) return false;
            saved.claimedUntil = Math.max(Math.floor(_offlineFinite(saved.claimedUntil, 0)), now);
            try {
    ...
    function _offlineLockAfterDeath() {
        ...
        _offlineWriteJson(_offlineStoreKey('checkpoint'), {
            ...
            lastActive: now,
            snapshot: JSON.parse(JSON.stringify(st)),
            display: { state: 'dead', map: String(typeof mapState !== 'undefined' && mapState ? mapState.current || '' : ''), mapName: _offlineMapName(typeof mapState !== 'undefined' && mapState ? mapState.current : '') }
        });
        return changed;
    }
    ...
    if (typeof _offlineOriginalSaveGame === 'function') {
        window.saveGame = function () {
            ...
            // 回到選角後角色已由 offlinePrepareCharacterSelect 完成快照並設為 detached；此時 js/13 較早註冊的
            // visibilitychange 最終存檔仍會呼叫 saveGame，不可再用 state.running=false 覆蓋剛成功的掛網 checkpoint。
            if (!_offlineInternalSave && !_offlineLoading && !_offlineRoleDetached && typeof player !== 'undefined' && player && player.cls) {
    ...
    window.offlinePrepareCharacterSelect = function () {
        ...
        try {
            if (_offlineOriginalSaveGame() === true) _offlineCommitDeferredClaim();
        } catch (e) {}
        finally {
