...
(function () {
    ...
    function d2rOverviewPanel() {
        ...
        let all = typeof d2rEquipTotals === 'function' ? d2rEquipTotals(player) : {}, groups=[
            ['攻擊／節奏','md,rd,mg,mh,rh,gh,as,fcr,mc,rc,gc,mcd,rcd,gcd,fpen,wpen,epen,apen,sdm,und,dem,dra,hsk,opn,exe,ber,fmp,shp,kfu,frz,brn,psn,bld,ctl,deb,fbr,wfr,eps,wbl,fsi,wsi,esi,asi'],
            ['防禦／受擊反應','hp,mp,hpp,mpp,ac,mr,er,dr,rf,rw,re,ra,rn,abr,fhr,bdr,pdr,mdr,ldr,adr,udr,ddr,blk,mgd,rcv,sav,bar,rip,ksh,krc'],
            ['資源／狩獵','mf,wt,hpr,mpr,pot,gf,xf,nd,bd,kh,km'],
            ['命中／觸發特效','ff,fw,fa,fe,ph,pm,kx,ts,pi,ks'],
            ['玩法改變','sp,fh']
        ];
        let summary=groups.map(([name,codes])=>{let rows=codes.split(',').filter(k=>(Number(all[k])||0)>0).map(k=>`${(typeof D2R_SHORT_LABEL!=='undefined'&&D2R_SHORT_LABEL[k])||(D2R_AFFIX_LABEL&&D2R_AFFIX_LABEL[k])||k} ${k==='ac'?'-':'+'}${Number(all[k]).toFixed(Number(all[k])%1?1:0)}${['as','fcr','fhr','mc','rc','gc','mcd','rcd','gcd','hpp','mpp','pot','abr','gf','xf','nd','bd','kx','ts','pi','ks','sp','fh','fpen','wpen','epen','apen','sdm','bdr','und','dem','dra','hsk','opn','exe','pdr','mdr','ldr','adr','udr','ddr','blk','mgd','rcv','sav','bar','rip','ber','fmp','shp','ksh','krc','kfu','frz','brn','psn','bld','ctl','deb','fbr','wfr','eps','wbl','fsi','wsi','esi','asi'].includes(k)?'%':''}`);return `<div class="gc-d2r-group"><b>${name}</b><span>${rows.length?rows.join('｜'):'目前沒有'}</span></div>`;}).join('');
    ...
    window.renderGrowthCenter = function () {
        ...
        let body = tab==='panacea'?panaceaRows():tab==='loadout'?loadoutRows():tab==='expedition'?expeditionRows():tab==='repair'?repairPanel():tab==='socket'?socketPanel():tab==='gems'?gemPanel():tab==='runes'?runePanel():tab==='alchemy'?alchemyPanel():tab==='orbs'?(typeof orbRenderPanel==='function'?orbRenderPanel():'<p>寶珠系統尚未載入。</p>'):tab==='d2r'?d2rOverviewPanel():tab==='enchant'?enchantPanel():rebirthPanel();
        el.innerHTML=`<style>
        ...
        #growth-center-modal .gc-orb-summary{display:flex;gap:10px;align-items:center;flex-wrap:wrap;padding:12px;margin-bottom:12px;background:#312e81;border:1px solid #a855f7;border-radius:10px}.gc-orb-summary>b{color:#f0abfc}.gc-orb-summary>span{flex:1;color:#c4b5fd}.gc-orb-slots,.gc-orb-grid,.gc-orb-presets{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:16px}.gc-orb-grid{grid-template-columns:repeat(2,1fr)}.gc-orb-slot,.gc-orb-card,.gc-orb-preset{display:grid;gap:7px;padding:12px;background:#1e293b;border:1px solid #475569;border-radius:10px}.gc-orb-slot{border-color:#a16207}.gc-orb-preset{border-color:#7e22ce}.gc-orb-preset>div{display:grid;gap:4px}.gc-orb-preset>span,.gc-orb-card>div{display:flex;gap:7px;align-items:center;flex-wrap:wrap}.gc-orb-slot small,.gc-orb-card small{color:#94a3b8}.gc-orb-card header{display:flex;justify-content:space-between;gap:8px}.gc-orb-card p{color:#e2e8f0}.gc-orb-card details p{color:#c4b5fd}.gc-orb-card.locked{opacity:.68}.gc-orbs>h3{color:#f0abfc;font-weight:bold;margin-bottom:8px}
        @media(max-width:700px){#growth-center-modal .gc-row,#growth-center-modal .gc-card,.gc-enchant-item{grid-template-columns:1fr}.gc-stat-buttons,.gc-d2r-grid,.gc-orb-grid,.gc-orb-slots,.gc-orb-presets{grid-template-columns:1fr}.gc-d2r-group{grid-template-columns:1fr}#growth-center-modal .gc-repair-action{flex-direction:column;align-items:stretch}#growth-center-modal .gc-repair-action button{width:100%;min-height:48px;font-size:16px}#growth-center-modal .gc-repair-action span{text-align:center;font-size:13px}#growth-center-modal .gc-alchemy-fuse{grid-template-columns:1fr}#growth-center-modal .gc-alchemy-fuse>span{text-align:center}#growth-center-modal .gc-alchemy-fuse button,#growth-center-modal .gc-alchemy-fuse select{width:100%;min-height:46px}#growth-center-modal .gc-alchemy-status{flex-direction:column}.gc-orb-summary{align-items:stretch;flex-direction:column}.gc-orb-summary button,.gc-orb-slot button,.gc-orb-card button,.gc-orb-preset button{min-height:46px}.gc-orb-card header{flex-direction:column}}
        </style><div class="gc-shell"><div class="gc-head"><h2>♾️ 成長中心</h2><button onclick="closeGrowthCenter()">✕ 關閉</button></div>
        <div class="gc-tabs">${[['panacea','萬能藥'],['loadout','裝備預設'],['expedition','傭兵遠征'],['d2r','D2R 總覽'],['enchant','鐵匠附魔'],['repair','裝備維修'],['socket','裝備開孔'],['gems','寶石'],['runes','符文'],['alchemy','符文鍊金'],['orbs','🔮 寶珠'],['rebirth','無限轉生']].map(x=>`<button class="${tab===x[0]?'active':''}" onclick="growthTab('${x[0]}')">${x[1]}</button>`).join('')}
