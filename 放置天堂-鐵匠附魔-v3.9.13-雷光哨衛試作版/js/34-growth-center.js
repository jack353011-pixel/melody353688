...
(function () {
    ...
    function d2rOverviewPanel() {
        ...
        let combatDefs=[['fpen','火焰穿透',30],['wpen','寒冰穿透',30],['epen','大地穿透',30],['apen','風雷穿透',30],['sdm','異常增傷',40],['bdr','頭目減傷',25],['und','不死獵殺',35],['dem','惡魔獵殺',35],['dra','屠龍',35],['hsk','破硬皮',30],['opn','先制增傷',30],['exe','處決增傷',40]];
        ...
        let summary=groups.map(([name,codes])=>{let rows=codes.split(',').filter(k=>(Number(all[k])||0)>0).map(k=>`${(typeof D2R_SHORT_LABEL!=='undefined'&&D2R_SHORT_LABEL[k])||(D2R_AFFIX_LABEL&&D2R_AFFIX_LABEL[k])||k} ${k==='ac'?'-':'+'}${Number(all[k]).toFixed(Number(all[k])%1?1:0)}${['as','mc','rc','gc','mcd','rcd','gcd','hpp','mpp','pot','abr','gf','xf','nd','bd','kx','ts','pi','ks','sp','fh','fpen','wpen','epen','apen','sdm','bdr','und','dem','dra','hsk','opn','exe'].includes(k)?'%':''}`);return `<div class="gc-d2r-group"><b>${name}</b><span>${rows.length?rows.join('｜'):'目前沒有'}</span></div>`;}).join('');
