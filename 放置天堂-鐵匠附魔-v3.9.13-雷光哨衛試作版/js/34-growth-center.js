...
(function () {
    ...
    function d2rOverviewPanel() {
        ...
        let combatDefs=[['fpen','火焰穿透',30],['wpen','寒冰穿透',30],['epen','大地穿透',30],['apen','風雷穿透',30],['sdm','異常增傷',40],['bdr','頭目減傷',25],['und','不死獵殺',35],['dem','惡魔獵殺',35],['dra','屠龍',35],['hsk','破硬皮',30],['opn','先制增傷',30],['exe','處決增傷',40],['pdr','物理減傷',20],['mdr','魔法減傷',20],['ldr','瀕死減傷',25],['adr','異常減傷',25],['udr','不死減傷',25],['ddr','惡魔減傷',25]];
        ...
        let all = typeof d2rEquipTotals === 'function' ? d2rEquipTotals(player) : {}, groups=[
            ['攻擊','md,rd,mg,mh,rh,gh,as,mc,rc,gc,mcd,rcd,gcd,fpen,wpen,epen,apen,sdm,und,dem,dra,hsk,opn,exe'],
            ['防禦','hp,mp,hpp,mpp,ac,mr,er,dr,rf,rw,re,ra,rn,abr,bdr,pdr,mdr,ldr,adr,udr,ddr'],
            ['資源／狩獵','mf,wt,hpr,mpr,pot,gf,xf,nd,bd,kh,km'],
            ['命中／觸發特效','ff,fw,fa,fe,ph,pm,kx,ts,pi,ks'],
            ['玩法改變','sp,fh']
        ];
        let summary=groups.map(([name,codes])=>{let rows=codes.split(',').filter(k=>(Number(all[k])||0)>0).map(k=>`${(typeof D2R_SHORT_LABEL!=='undefined'&&D2R_SHORT_LABEL[k])||(D2R_AFFIX_LABEL&&D2R_AFFIX_LABEL[k])||k} ${k==='ac'?'-':'+'}${Number(all[k]).toFixed(Number(all[k])%1?1:0)}${['as','mc','rc','gc','mcd','rcd','gcd','hpp','mpp','pot','abr','gf','xf','nd','bd','kx','ts','pi','ks','sp','fh','fpen','wpen','epen','apen','sdm','bdr','und','dem','dra','hsk','opn','exe','pdr','mdr','ldr','adr','udr','ddr'].includes(k)?'%':''}`);return `<div class="gc-d2r-group"><b>${name}</b><span>${rows.length?rows.join('｜'):'目前沒有'}</span></div>`;}).join('');
