...
function flow(id){return typeof buildFlowSource==='function'?buildFlowSource(player,id):null}
function equippedCore(){let i=player&&player.eq&&player.eq.wpn,d=i&&DB.items[i.id];return d&&['hydra','static','orb','chain'].includes(d.core)?d.core:''}
...
function activate(id,before){
 let x=ext();
 if(id===FB){let w=flow('hydra'),cdr=(w&&w.cdr||0)+x.scd;(mapState.mobs||[]).forEach(m=>{if(m&&!m._dead&&before.has(String(m.uid))&&before.get(String(m.uid))>m.curHp&&m.curHp>0)m.heat79=state.ticks+30});if(w&&player.hydraCd79<=0){player.hydraCd79=Math.max(40,Math.ceil(12000/(100+cdr)));player.hydra79={left:Math.round(60*(1+x.hd/100)),shot:1,anim:0};logCombat('【九頭蛇流派】三首火蛇出現！','magic','summon')}}
 if(ICE_IDS.includes(id)){let w=flow('orb'),cdr=(w&&w.cdr||0)+x.scd;(mapState.mobs||[]).forEach(m=>{if(!m||m._dead||!before.has(String(m.uid)))return;let delta=before.get(String(m.uid))-m.curHp;if(delta>0&&m.heat79>state.ticks&&m.curHp>0){let d=Math.max(1,Math.floor(delta*(m.boss?.15:.30)));m.curHp-=d;delete m.heat79;kill(m);logCombat('【熱衝擊】'+m.n+' 額外 '+d+' 傷害。','magic')}});if(id===ICE&&w&&player.orbCd79<=0){player.orbCd79=Math.max(30,Math.ceil(8000/(100+cdr)));player.orb79={left:40,shot:1,bonus:x.os};logCombat('【冰封球流派】冰箭凝聚成冰封球！','magic','summon')}}
 if(id===LIT){let exact=equippedCore(),mode=exact==='static'||exact==='chain'?exact:(player.arcaneLightningNext79==='chain'?'chain':'static');player.arcaneLightningNext79=mode==='static'?'chain':'static';let w=flow(mode),cdr=(w&&w.cdr||0)+x.scd;if(mode==='static'&&w&&player.staticCd79<=0){player.staticCd79=Math.max(40,Math.ceil(10000/(100+cdr)));player.staticFx79=state.ticks+14;(mapState.mobs||[]).forEach(m=>{if(!m||m._dead||m.curHp<=1)return;let floor=m.boss?Math.ceil((m.hp||1)*.5):1;if(m.curHp<=floor)return;let d=Math.floor(m.curHp*(m.boss?.02:.08)*(1+x.st/100)*(1-magicAttrDefense(m,'wind')));d=Math.max(0,Math.min(d,m.curHp-floor));m.curHp-=d});logCombat('【靜電立場】雷電圓環削減敵人目前生命。','magic')}if(mode==='chain'&&w&&player.chainCd79<=0){player.chainCd79=Math.max(25,Math.ceil(6000/(100+cdr)));chainLightning()}}
