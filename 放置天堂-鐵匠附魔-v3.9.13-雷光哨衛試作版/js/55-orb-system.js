...
function orbIncomingDamage(owner,dmg){
    dmg=Math.max(0,Math.floor(Number(dmg)||0));let orbs=orbEnsure(owner),guard=orbEquipped(owner,'guard',orbs);
    if(!dmg||!guard||guard.id!=='orb_iron')return dmg;
    return Math.max(1,Math.floor(dmg*(1-orbIronPct(guard.level)/100)));
}
...
function orbDropPoolForMob(mob,slot){
    ...
    if(!slot||slot==='resonance')ids.push((race==='惡魔'||mob&&mob.un||mr>=30)?'orb_echo':'orb_hunter');
    if(!slot||slot==='guard')ids.push((mob&&mob.hard||race==='巨人'||race==='高崙')?'orb_iron':'orb_devour');
...
function orbRenderPanel(){
    let o=orbEnsure(player),slotRows=Object.keys(ORB_SLOT_NAME).map(slot=>{
    ...
    return `<div class="gc-orbs"><div class="gc-orb-summary"><b>寶珠粉塵：${o.dust.toLocaleString()}</b><span>固定三欄；滿配穩定增傷上限約 10%，回響平均增傷最高 4%。</span>${o.trialClaimed?'':'<button onclick="growthClaimTrialOrbs()">領取試作寶珠</button>'}</div><div class="gc-orb-slots">${slotRows}</div><h3>寶珠收藏</h3><div class="gc-orb-grid">${cards}</div><p class="text-slate-400">前三次擊敗頭目會依序保底核心、共鳴、守護寶珠；之後頭目 12%、強敵 0.2%、一般敵人 0.02%。種類依怪物元素、種族與防禦特性決定；重複寶珠轉為 15 粉塵。寶珠傷害不會遞迴觸發寶珠。</p></div>`;
}
