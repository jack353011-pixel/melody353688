...
(function () {
  ...
  function renderCard() {
    ...
    out += wCard('🎴 三階卡片',
      ...
      wDesc('三階<b>各自獨立判定</b>（同一隻可能只掉普卡、也可能直接掉金卡）。機率<b>固定</b>，<b>不受任何掉落倍率影響</b>——席琳的世界 ×1.5、瘋狂的席琳世界 ×2.25、恩賜怪 ×10 通通不吃，任何狀態下都是上表的機率。') +
  ...
  function eqHelpCards() {
    ...
    var relic = [
      '<b>每隻怪物各有一件專屬遺物</b>（名稱藍字、說明帶【遺物】），擊殺時 <b>0.0001%</b>（百萬分之一）機率掉落；機率會吃掉落倍率：席琳的世界 <b>×1.5</b>、瘋狂的席琳世界 <b>×2.25</b>、恩賜怪 <b>×10</b>。想查哪隻怪掉哪件，到「掉落查詢」搜怪物名。',
  ...
  function renderSherine() {
    var out = '<div class="m-wiki-note">「席琳」是一整套困難模式：開啟世界 → 怪物變強且報酬提高 → 掉<b>席琳結晶</b> → 找神殿的<b>伊奧</b>換<b>席琳遺骸</b> → 遺骸湊組發動席琳套裝效果。<b>套裝效果不會直接附在掉落裝備上。</b><br>📌 作用範圍：席琳的世界強化效果（怪物強化與掉落倍率）也套用在<b>拉斯塔巴德·軍王之室</b>、<b>時空裂痕·底比斯歐西里斯祭壇</b>與<b>時空裂痕·提卡爾庫庫爾坎祭壇</b>的怪物（但恩賜精英不會在這些頭目房出現）。<br>🎮 <b>經典模式角色進不了席琳神殿</b>，整套席琳（世界、結晶、遺骸、套裝效果）都用不到。</div>';

    out += wCard('🩸 席琳的世界（困難模式：一般／瘋狂）',
      wDesc('到「席琳神殿」找 NPC <b>席琳</b>切換世界難度：普通世界創角即開放，<b>席琳世界（煉獄）需 Lv40</b>，<b>瘋狂席琳（地獄）需 Lv70</b>；<b>不需要完成故事</b>，但只能在安全區切換。兩種席琳難度互斥，開啟後畫面變暗紅（瘋狂更紅）。<b>攻城區與血盟敵人不受影響</b>。') +
      wTbl(['項目', '一般席琳', '瘋狂席琳'], [
        ['怪物 HP', '×2.5', '×5'],
        ['怪物 防禦(AC)', '一般怪 −10、頭目 −20', '一般怪 −10、頭目 −20'],
        ['怪物 魔防', '＋25%', '＋50%（<b>最多額外加 150</b>）'],
        ['怪物 命中', '×1.25', '×1.6'],
        ['怪物 額外減傷', '+（怪等÷5）', '+（怪等÷3）'],
        ['怪物 一般攻擊傷害', '×1.7', '×2.6'],
        ['怪物 技能／持續傷害', '×1.7', '×2.6'],
        ['頭目裂界階段', '50% HP 時 1 次', '66%／33% HP 時各 1 次'],
        ['出怪等待', '×0.8', '×0.8'],
        ['你的經驗', '<b style="color:#86efac">×2.5</b>', '<b style="color:#86efac">×4</b>'],
        ['你的金錢', '<b style="color:#86efac">×2</b>', '<b style="color:#86efac">×3</b>'],
        ['掉落機率', '<b style="color:#86efac">×1.5</b>', '<b style="color:#86efac">×2.25</b>'],
        ...
        ['席琳結晶掉率', '基準；頭目 100 次保底', '<b style="color:#86efac">×3</b>；頭目 40 次保底']
      ]) +
      wDesc('魔防已收斂，避免魔法流派受到遠高於物理流派的懲罰：煉獄增加 25%，地獄增加 50% 且最多額外增加 150。例：原本魔防 100 → 地獄 150；原本 400 → 地獄 550。'));

    out += wCard('🩸 席琳能力（不顯示頭上標籤）',
      wDesc('煉獄一般怪隨機取得 1 項、地獄一般怪取得不重複的 2 項；頭目不抽一般怪能力，改用固定裂界階段。能力不顯示在怪物頭上，只用實際戰鬥行為與日誌提示。') +
      wTbl(['能力', '效果'], [
        ['席琳韌性', '第一次受到冰凍／暈眩／石化／沉睡／麻痺時，控制時間減半'],
        ['再生', '每 5 秒恢復 3% HP；受到污濁之水時只恢復 1.5%'],
        ['不屈', '第一次死亡時恢復 15% HP；只會發動一次'],
        ['相位防護', '每 5 秒在物理防護（AC −8）與魔法防護（MR ＋30）之間切換']
      ]) +
      wDesc('<b>裂界階段：</b>頭目跨越生命門檻時解除冰凍／暈眩／石化／沉睡／麻痺，每階段使攻擊間隔縮短 8%；同時首領韌性降低 4 個百分點。它不增加單次傷害，玩家若撐住攻勢，後半段反而更容易突破。席琳頭目最近 5 秒受到任何傷害都會壓低自然回血，物理、魔法、召喚與持續傷害一視同仁。'));

    out += wCard('💎 席琳結晶（換遺骸的鑰匙）',
      wDesc('席琳世界限定材料，本身不能直接用。到席琳神殿找 <b>伊奧</b>，<b>1 顆換 1 件</b>指定部位的席琳遺骸。掉率直接由<b>怪物等級</b>決定，不受隊伍人數放大；煉獄／地獄的頭目保底各自累積：') +
      wTbl(['怪物類型', '每擊殺的掉落機率'], [
      ...
      wTbl(['倍率', '是否影響結晶掉率'], [
        ['席琳世界掉落 ×1.5', '<b style="color:#f87171">不影響</b>（結晶使用自己的機率）'],
        ['恩賜怪 ×10', '<b style="color:#f87171">不影響</b>'],
        ['瘋狂的席琳世界', '<b style="color:#86efac">×3</b>'],
        ['隊伍人數', '<b style="color:#f87171">不影響</b>'],
        ['頭目保底', '煉獄 100 次／地獄 40 次；掉落後該難度歸零'],
    ...
    out += wCard('✨ 席琳恩賜（精英怪）',
      wDesc('每次刷怪都有 1% 機率讓場上一隻怪物獲得「席琳恩賜」（紅綠光暈＋紅底綠字徽章）；恩賜怪更耐打但報酬更高。<b>已恩賜的怪不會再次被選中</b>，同一隻不會重複疊加。') +
      wTbl(['世界難度', '恩賜規則'], [['席琳世界（煉獄）', '每 3 分鐘最多一次、場上最多一隻、頭目不會獲得'], ['瘋狂席琳（地獄）', '沒有 3 分鐘冷卻、可同時多隻、頭目也可能獲得']]) +
  ...
  function renderSunrise() {
    ...
    out += wCard('🔮 妖魂：日出之國的專屬產出',
      ...
      wDesc('妖魂的掉率<b>會吃掉落倍率</b>（席琳的世界 ×1.5、瘋狂的席琳世界 ×2.25、恩賜怪 ×10）。') +
  ...
  var MAGIC_FACT = {
    ...
    sk_icequeen_veil: ['受到的所有傷害 −20%', '僅裝備冰之女王的耳環 Lv8 時可使用；卸下後技能消失'],
    sk_four_seal_resonance: ['全屬性抗性 +15', '龍的一擊發動率 +8%（12% → 20%）', '僅裝備屠龍劍時可使用；卸下後技能消失'],
    sk_vow_echo: ['單體治療，基礎治療骰為 10d10 ×2', '冷卻 8 秒', '僅同時裝備歐林的項鍊與西瑪戒指時可使用；拆散後技能消失'],
    sk_forbidden_resonance: ['魔法傷害 +8、魔防 −15，持續 32 秒', '僅裝備巴列斯魔杖時可使用；卸下後技能與效果一起消失'],
    sk_nameless_king_flame: ['火屬性抗性 +15', '死亡騎士烈炎之劍的地獄火發動率額外 +5%（原始機率仍隨武器強化成長）', '僅裝備死亡騎士的烈炎之劍時可使用；卸下後技能與效果一起消失'],
    sk_lost_frost_medal: ['水屬性抗性 +15', '蕾雅魔杖的冰裂術命中冰凍目標時，碎冰追加傷害從 +100 提升為 +200', '僅同時裝備蕾雅魔杖與蕾雅戒指時可使用；拆下任一件後技能與效果一起消失'],
    sk_kurt_thunder_edict: ['風屬性抗性 +15', '克特之劍與克特之盾觸發的審判落雷，固定傷害加值從 +60 提升為 +120', '僅同時裝備克特之劍與克特之盾時可使用；拆下任一件後技能與效果一起消失'],
